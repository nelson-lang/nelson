...
//=============================================================================
static void AltF4_Command(void)
{
    Nelson::doExit();
}
//=============================================================================
...
WindowsConsole::WindowsConsole(bool _bWithColors)
{
    ...
    SetConsoleMode(Win32InputStream, ENABLE_PROCESSED_INPUT | ENABLE_QUICK_EDIT_MODE | ENABLE_INSERT_MODE |ENABLE_EXTENDED_FLAGS);
    ...
    if (_bWithColors && bHaveHisOwnWindow)
    {
        ...
        FillConsoleOutputAttribute(hConsole,
                                   BACKGROUND_BLUE | BACKGROUND_GREEN | BACKGROUND_RED | BACKGROUND_INTENSITY,	csbi.dwSize.X*csbi.dwSize.Y,
                                   Coord,
                                   &cWritten);
...
//=============================================================================
std::wstring WindowsConsole::getInput(std::wstring prompt)
{
    return getTextLine(prompt, true);
}
//=============================================================================
std::wstring WindowsConsole::getLine(std::wstring prompt)
{
    return getTextLine(prompt, false);
}
//=============================================================================
std::string WindowsConsole::getLine(std::string prompt)
{
    return wstring_to_utf8(getLine(utf8_to_wstring(prompt)));
}
//=============================================================================
size_t WindowsConsole::getTerminalWidth()
{
    CONSOLE_SCREEN_BUFFER_INFO csbi;
    GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi);
...
//=============================================================================
size_t WindowsConsole::getTerminalHeight()
{
    CONSOLE_SCREEN_BUFFER_INFO csbi;
    GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi);
...
//=============================================================================
void WindowsConsole::outputMessage(std::string msg)
{
...
{
    return ((irBuffer.Event.KeyEvent.dwControlKeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED)) != 0);
}
//=============================================================================
bool WindowsConsole::isALTPressed(INPUT_RECORD irBuffer)
{
    return ((irBuffer.Event.KeyEvent.dwControlKeyState & (RIGHT_ALT_PRESSED | LEFT_ALT_PRESSED)) != 0);
}
//=============================================================================
wchar_t WindowsConsole::keysEventsFilter(INPUT_RECORD irBuffer, bool &bIsChar, bool &bIsAction)
{
    DWORD n = 0;
    wchar_t ch = 0;
    bIsChar = false;
...
//=============================================================================
wchar_t WindowsConsole::actionControlKey(void)
{
    if (isCTRL_VKEY(L'X'))
    {
...
//=============================================================================
bool WindowsConsole::isCTRL_VKEY(int VKEY)
{
    return (GetKeyState(VKEY) & 0x80) == 0 ? false : true;
}
//=============================================================================
void WindowsConsole::clearTerminal()
{
    COORD coord;
    DWORD written;
    ...
    ::FillConsoleOutputCharacterW(hConsole,
                                  L' ',
...
//=============================================================================
void WindowsConsole::pasteClipBoard(void)
{
...
//=============================================================================
void WindowsConsole::clearClipBoard(void)
{
    OpenClipboard(NULL);
    EmptyClipboard();
    CloseClipboard();
...
//=============================================================================
bool WindowsConsole::copyToClipBoard(std::wstring txt)
{
    bool bRes = false;
...
//=============================================================================
bool WindowsConsole::hasHisOwnWindow()
{
    return bHaveHisOwnWindow;
}
//=============================================================================
bool WindowsConsole::setConsoleTitle(std::wstring title)
{
    return ::SetConsoleTitleW(title.c_str()) == FALSE ? false : true;
}
//=============================================================================
std::wstring WindowsConsole::getConsoleTitle()
{
    wchar_t title[4096];

...
public:
    Utf8String(void) : _data(), _bufSize(0), _len(0) { }
    explicit Utf8String(UnicodeString const& src) : _data(), _bufSize(0), _len(0)
    {
        assign(src, src.length());
    }

    Utf8String(UnicodeString const& src_, int len_) : _data(), _bufSize(0), _len(0)
    {
        assign(src_, len_);
    }

    void
    assign(UnicodeString const& str_)
    {
        assign(str_, str_.length());
    }

    void
    assign(UnicodeString const& str_, int len_)
    {
        assign(str_.get(), len_);
    }

    void
    assign(char32_t const* str_, int len_)
    {
        int len(len_ * 4);
        realloc(len);
    ...
    void
    assign(Utf8String const& other_)
    {
        realloc(other_._len);
    ...
    bool
    operator!=(Utf8String const& other_)
    {
        return ((other_._len != _len)
            || ((_len != 0) && (memcmp(other_._data.get(), _data.get(), _len) != 0)));
    }

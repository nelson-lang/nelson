...
void
Terminal::write32(char32_t const* text32, int len32)
{
...
void
Terminal::write8(char const* data_, int size_)
{
#ifdef _WIN32
...
int
Terminal::get_screen_columns(void)
{
    int cols(0);
#ifdef _WIN32
...
Terminal::enable_out(void)
{
    ...
    _autoEscape
        = SetConsoleMode(_consoleOut, _origOutMode | ENABLE_VIRTUAL_TERMINAL_PROCESSING) != 0;
#endif
}

void
Terminal::disable_out(void)
{
#ifdef _WIN32
...
void
Terminal::enable_bracketed_paste(void)
{
...
void
Terminal::disable_bracketed_paste(void)
{
...
int
Terminal::reset_raw_mode(void)
{
    if (!_rawMode) {
        return (-1);
    }
#ifdef _WIN32
    SetConsoleMode(_consoleIn,
...
void
Terminal::disable_raw_mode(void)
{
    if (!_rawMode) {
        return;
    }
...
read_unicode_character(int in_fd_)
{
    ...
    while (true) {
        unsigned char c = 0;
        int nread = _read(in_fd_, reinterpret_cast<char*>(&c), 1);
        if (nread <= 0) {
            return 0;
        }
        ...
        if (utf8Count < sizeof(utf8String) - 1) {
            utf8String[utf8Count++] = static_cast<char8_t>(c);
        ...
        // window in the debugger
        {
            if (rec.EventType == KEY_EVENT) {
                ...
                sprintf(buf,
                    ...
                    rec.Event.KeyEvent.uChar.UnicodeChar, rec.Event.KeyEvent.wRepeatCount,
                    rec.Event.KeyEvent.wVirtualKeyCode, rec.Event.KeyEvent.wVirtualScanCode,
...
Terminal::wait_for_input(int long timeout_)
{
    ...
    auto const timeoutBudget(
        timeoutEnabled ? std::chrono::milliseconds(timeout_) : std::chrono::milliseconds::zero());
    ...
    bool stopInputBuffer = false;

    while (true) {
        bool waitKey(true);
        DWORD event(WAIT_TIMEOUT);
        while (waitKey) {
            try {
                ProcessEventsDynamicFunctionWithoutWait();
            } catch (Nelson::Exception const& e) {
    ...
    while (true) {
        ...
        while (waitKey) {
            ...
            FD_ZERO(&readySet);
            FD_SET(_in_fd, &readySet);
            FD_SET(_interrupt[0], &readySet);
            timeval tv { 0, 0 };
            err = select(nfds, &readySet, nullptr, nullptr, &tv);
            if (!eval->commandQueue.isEmpty() || !Nelson::CallbackQueue::getInstance()->isEmpty()) {
                stopInputBuffer = false;
                return (EVENT_TYPE::MESSAGE);
            }
...
Terminal::notify_event(EVENT_TYPE eventType_)
{
    ...
    SetEvent(_interrupt);
#else
    char data((eventType_ == EVENT_TYPE::KEY_PRESS)
            ? 'k'
            : (eventType_ == EVENT_TYPE::MESSAGE ? 'm' : 'r'));
...
Terminal::jump_cursor(int xPos_, int yOffset_)
{
    ...
    snprintf(seq, sizeof seq, "\033[%dG", xPos_ + 1 /* 1-based on VT100 */
    );
    write8(seq, strlen(seq));
#endif
}
...
int
Terminal::install_window_change_handler(void)
{
    struct sigaction sa;

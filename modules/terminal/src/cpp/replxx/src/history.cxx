...
namespace {
    void
    delete_ReplxxHistoryScanImpl(Replxx::HistoryScanImpl* impl_)
    {
        delete impl_;
    }
...
Replxx::HistoryScan::HistoryScan(impl_t impl_) : _impl(std::move(impl_)) { }

bool
Replxx::HistoryScan::next(void)
{
    return (_impl->next());
}
...
Replxx::HistoryEntry const&
Replxx::HistoryScan::get(void) const
{
    return (_impl->get());
}

bool
Replxx::HistoryScanImpl::next(void)
{
    if (_it == _entries.end()) {
        _it = _entries.begin();
    } else {
...
Replxx::HistoryEntry const&
Replxx::HistoryScanImpl::get(void) const
{
    if (_cacheValid) {
        return (_entryCache);
    }
...
History::scan(void) const
{
    return (Replxx::HistoryScan::impl_t(
        new Replxx::HistoryScanImpl(_entries), delete_ReplxxHistoryScanImpl));
}
...
void
History::add(UnicodeString const& line, std::string const& when)
{
    if (_maxSize <= 0) {
        return;
    }
...
public:
    FileLock(std::string const& name_)
        : _path(name_ + ".lock"), _lockFd(::open(_path.c_str(), O_CREAT | O_RDWR, 0600))
    {
...
bool
History::save(std::string const& filename, bool sync_)
{
#ifndef _WIN32
...
void
History::save(std::ostream& histFile)
{
    Utf8String utf8;
    UnicodeString us;
...
namespace {
    ...
    bool
    is_timestamp(std::string const& s)
    {
...
void
History::do_load(std::istream& histFile)
{
    string line;
...
bool
History::load(std::string const& filename)
{
    ifstream histFile(filename);
    if (!histFile) {
        clear();
...
void
History::load(std::istream& histFile)
{
    clear();
    do_load(histFile);
    sort();
...
void
History::set_max_size(int size_)
{
    if (size_ >= 0) {
        _maxSize = size_;
        trim_to_max_size();
...
void
History::reset_yank_iterator(void)
{
    _yankPos = _entries.end();
}

bool
History::next_yank_position(void)
{
...
void
History::jump(bool start_, bool reset_)
{
    if (start_) {
        _current = _entries.begin();
    } else {
...
void
History::save_pos(void)
{
    _previous = _current;
}

void
History::restore_pos(void)
{
    _current = _previous;
}

bool
History::common_prefix_search(
    UnicodeString const& prefix_, int prefixSize_, bool back_, bool ignoreCase)
{
    int step(back_ ? -1 : 1);
    entries_t::iterator it(moved(_current, step, true));
    bool lowerCaseContext(std::none_of(prefix_.begin(), prefix_.end(),
        [](char32_t x) { return iswupper(static_cast<wint_t>(x)); }));
    while (it != _current) {
        if (it->text().starts_with(prefix_.begin(), prefix_.begin() + prefixSize_,
                ignoreCase && lowerCaseContext ? case_insensitive_equal : case_sensitive_equal)) {
            _current = it;
            commit_index();
            return (true);
        }
...
bool
History::move(entries_t::iterator& it_, int by_, bool wrapped_)
{
    if (by_ > 0) {
        for (int i(0); i < by_; ++i) {
            ++it_;
...
History::entries_t::iterator
History::moved(entries_t::iterator it_, int by_, bool wrapped_)
{
...
void
History::erase(entries_t::iterator it_)
{
...
void
History::remove_duplicate(UnicodeString const& line_)
{
    if (!_unique) {
        return;
    }
...
void
History::remove_duplicates(void)
{
    if (!_unique) {
        return;
    }
...
void
History::update_last(UnicodeString const& line_)
{
    if (_unique) {

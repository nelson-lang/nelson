...
int
virtual_render(char32_t const* display_, int size_, int& x_, int& y_, int screenColumns_,
    int promptLen_, char32_t* rendered_, int* renderedSize_, int out_fd_)
{
    char32_t* out(rendered_);
    int visibleCount(0);
    auto render = [&rendered_, &renderedSize_, &out, &visibleCount](
                      char32_t c_, bool visible_, bool renderAttributes_ = true) {
    ...
    if (rendered_ && renderedSize_) {
        *renderedSize_ = (int)(out - rendered_);
    }
    return (visibleCount);
}

char const*
ansi_color(Replxx::Color color_)
{
    ...
    if ((code & static_cast<int unsigned>(Replxx::Color::DEFAULT)) != 0) {
        ...
        static char const* ansiEscapeCodeTemplate
            = has256color ? "\033[0;9%d%s%sm" : "\033[0;1;3%d%s%sm";
        pos = snprintf(colorBuffer, MAX_COLOR_CODE_SIZE, ansiEscapeCodeTemplate,
            fg - static_cast<int>(Replxx::Color::GRAY), underline, bold);
    } else {
        pos = snprintf(
            colorBuffer, MAX_COLOR_CODE_SIZE, "\033[0;38;5;%d%s%sm", fg, underline, bold);
    }
    ...
    if (bg <= static_cast<int unsigned>(Replxx::Color::WHITE)) {
        if (bg <= static_cast<int unsigned>(Replxx::Color::LIGHTGRAY)) {
            ...
            snprintf(colorBuffer + pos, MAX_COLOR_CODE_SIZE - pos, "\033[10%dm",
                bg - static_cast<int>(Replxx::Color::GRAY));
        }
    } else {

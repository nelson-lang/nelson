...
template <typename T>
T*
HandleEsc(HANDLE out_, T* p, T* end)
{
    if (*p == '[') {
        int code = 0;
            ...
            if ('0' <= c && c <= '9') {
                ...
                case 97:
                    WIN_ATTR._consoleColor
                        = FOREGROUND_GREEN | FOREGROUND_RED | FOREGROUND_BLUE | thisBackground;
                    break;
                case 101:
    ...
    SetConsoleTextAttribute(out_, WIN_ATTR._consoleAttribute | WIN_ATTR._consoleColor);

    return p;
}

int
win_write(HANDLE out_, bool autoEscape_, char const* str_, int size_)
{
    int count(0);
    if (isConsoleHandle(out_)) {
        DWORD nWritten(0);
        if (autoEscape_) {

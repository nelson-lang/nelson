...
namespace std {
template <> struct hash<replxx::UnicodeString>
{
    std::size_t
    operator()(replxx::UnicodeString const& us_) const
    {
        std::size_t h(0);
...
public:
    ...
    public:
        Entry(std::string const& timestamp_, UnicodeString const& text_)
            : _timestamp(timestamp_), _text(text_), _scratch(text_)
        {
        }
        std::string const&
        ...
        void
        set_scratch(UnicodeString const& s)
        {
            _scratch = s;
        }
        void
        ...
        bool
        operator<(Entry const& other_) const
        {
...
public:
    History(void);
    void
    add(UnicodeString const& line, std::string const& when = now_ms_str());
    bool
    save(std::string const& filename, bool);
    void
    save(std::ostream& histFile);
    bool
    load(std::string const& filename);
    void
    load(std::istream& histFile);
    void
    clear(void);
    void
    set_max_size(int len);
    void
    set_unique(bool unique_)
    {
        _unique = unique_;
        remove_duplicates();
    }
    ...
    reset_yank_iterator();
    bool
    next_yank_position(void);
    void
    reset_recall_most_recent(void)
    {
    ...
    void
    update_last(UnicodeString const&);
    void
    drop_last(void);
    bool
    is_last(void);
    ...
    move(bool);
    void
    set_current_scratch(UnicodeString const& s)
    {
        _current->set_scratch(s);
    }
    void
    reset_scratches(void)
    {
    ...
    jump(bool, bool = true);
    bool
    common_prefix_search(UnicodeString const&, int, bool, bool);
    int
    size(void) const
    {
...
private:
    ...
    History&
    operator=(History const&)
        = delete;
    bool
    move(entries_t::iterator&, int, bool = false);
    entries_t::iterator
    moved(entries_t::iterator, int, bool = false);
    void erase(entries_t::iterator);
    void
    trim_to_max_size(void);
    void
    remove_duplicate(UnicodeString const&);
    void

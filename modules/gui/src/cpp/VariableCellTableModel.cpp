...
VariableCellTableModel::formatCellForDisplay(const QVariant& cellData) const
{
    ...
    // Handle different Qt types
    /* if (cellData.type() == QVariant::Double) {
        double val = cellData.toDouble();
        if (std::isnan(val)) {
            return QVariant(tr("<missing>"));
        }
        return QVariant(QString::number(val, 'g', 6));
    } else if (cellData.type() == QVariant::String) {
        return cellData;
    }*/
...
//=============================================================================
VariableCellTableModel::CellType
VariableCellTableModel::getCellType(int row, int col) const
{
    // Check edit data first
    const int idx = indexToFlat(row, col);
    if (idx >= 0 && idx < m_editData.size() && m_editData[idx].isValid()) {
        return inferCellType(m_editData[idx]);
    }
    ...
    if (row < m_rows && col < m_cols && hasOriginalData()) {
        ...
        const int arrayIdx = row + col * m_rows;
        if (ptr && arrayIdx < m_array.getElementCount()) {
            return arrayToCellType(ptr[arrayIdx]);
        }
    }
...
//=============================================================================
VariableCellTableModel::CellType
VariableCellTableModel::inferCellType(const QVariant& cellData) const
{
    if (!cellData.isValid()) {
        return CellType::Empty;
    }
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    int typeId = cellData.typeId();
#else
    int typeId = QMetaType::type(cellData.typeName());
#endif
    switch (typeId) {
    case QMetaType::Bool:
        return CellType::Logical;
    ...
    case QMetaType::ULongLong:
        return CellType::UInt64;

    case QMetaType::Int:
        return CellType::Int32;

    case QMetaType::UInt:
        return CellType::UInt32;

    case QMetaType::Long:
        // Long size is platform dependent, typically 32 or 64 bit
        return (sizeof(long) == 8) ? CellType::Int64 : CellType::Int32;
    ...
    case QMetaType::UChar:
        return CellType::Int8; // or UInt8 for UChar
...
//=============================================================================
VariableCellTableModel::CellType
VariableCellTableModel::arrayToCellType(const ArrayOf& cellArray) const
{
    if (cellArray.isEmpty()) {
        return CellType::Empty;
    }

    // For non-scalar arrays, return Other unless it's a string array
    if (!cellArray.isScalar()) {
        switch (cellArray.getDataClass()) {
        case NLS_CHAR:
            return CellType::String; // Character arrays can be strings
        default:
            return CellType::Other;
        }
    }
...
VariableCellTableModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    // Parse the input value to determine what type of cell to create
    ArrayOf newCellValue = parseInputToArrayOf(value.toString(), row, col);
    ...
    if (!setCellValue(row, col, newCellValue)) {
        return false;
    }
...
//=============================================================================
bool
VariableCellTableModel::expandDisplayOptimized(int row, int col)
{
...
//=============================================================================
bool
VariableCellTableModel::resizeArrayOptimized(int row, int col)
{
...
//=============================================================================
void
VariableCellTableModel::resizeEditDataOptimized(int oldRows, int oldCols)
{
...
//=============================================================================
bool
VariableCellTableModel::pasteData(int startRow, int startCol, const QString& text)
{

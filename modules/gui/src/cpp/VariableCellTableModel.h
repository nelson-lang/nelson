...
namespace Nelson {
//=============================================================================
class VariableCellTableModel : public VariableAbstractTableModel
{
    Q_OBJECT
...
public:
    enum class CellType
    {
        ...
        Empty, // Empty/uninitialized cells
        Other // Other data types not covered above
    };

    explicit VariableCellTableModel(
        const QString& name, ArrayOf array, Evaluator* evaluator, QObject* parent = nullptr);
    ~VariableCellTableModel() override = default;
    ...
    bool
    pasteData(int startRow, int startCol, const QString& text) override;
    bool
    ...
    bool
    isCellScalarOrEditable(int row, int col) const;
...
private:
    ...
    bool
    expandDisplayOptimized(int row, int col);
    bool
    resizeArrayOptimized(int row, int col);
    void
    resizeEditDataOptimized(int oldRows, int oldCols);
    ...
    QVariant
    formatCellForDisplay(const QVariant& cellData) const;
    QVariant
    formatCellForDisplay(const ArrayOf& cellArray) const;

    CellType
    inferCellType(const QVariant& cellData) const;
    ...
    CellType
    arrayToCellType(const ArrayOf& cellArray) const;

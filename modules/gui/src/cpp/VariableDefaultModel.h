...
public:
    ...
    QString
    getPlainContent() const;

    QString
    formatArrayContent(const ArrayOf& array) const;

    bool
    setContentFromText(const QString& text);
    ...
    bool
    replaceText(const QString& text, int start = 0, int length = -1);
...
private:
    ...
    // Content validation
    bool
    isValidTextContent(const QString& text) const;
    bool
    canParseAsNumeric(const QString& text) const;
    bool
    canParseAsString(const QString& text) const;

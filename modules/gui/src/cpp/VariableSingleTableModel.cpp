...
namespace Nelson {
//=============================================================================
VariableSingleTableModel::VariableSingleTableModel(
...
{
    if (!m_evaluator) {
        qWarning() << "VariableSingleTableModel: Context is null for variable" << name;
    }

    if (!validateArrayState()) {
        qWarning() << "VariableSingleTableModel: Invalid array state for variable" << name;
    }
...
//=============================================================================
int
VariableSingleTableModel::rowCount(const QModelIndex& parent) const
{
    return parent.isValid() ? 0 : m_rows_display;
}
//=============================================================================
int
VariableSingleTableModel::columnCount(const QModelIndex& parent) const
{
    return parent.isValid() ? 0 : m_cols_display;
}
//=============================================================================
QVariant
VariableSingleTableModel::data(const QModelIndex& index, int role) const
{
...
//=============================================================================
bool
VariableSingleTableModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    // Only support single arrays for now
    if (!m_array.isSingleType(false)) {
        emit errorOccurred(tr("Can only edit single arrays"));
        return false;
    }
    ...
    if (resultArray.isComplex() && !m_array.isComplex()) {
        m_array.promoteType(NLS_SCOMPLEX);
    } else if (!resultArray.isComplex() && m_array.isComplex()) {
        resultArray.promoteType(NLS_SCOMPLEX);
    }

    if (m_array.isEmpty()) {
        if (m_array.getDataClass() == NLS_SCOMPLEX) {
            m_array = ArrayOf::complexConstructor(0, 0);
        } else {
            m_array = ArrayOf::singleConstructor(0);
        }
    }
    ...
    if (resultArray.isComplex()) {
        std::complex<single> cplxValue = resultArray.getContentAsSingleComplexScalar();
        updateEditData(row, col, QVariant(formatDisplayValue(cplxValue)));
    } else {
        single numericValue = resultArray.getContentAsSingleScalar();
...
//=============================================================================
bool
VariableSingleTableModel::expandArrayIfNeeded(int row, int col)
{
...
//=============================================================================
bool
VariableSingleTableModel::expandDisplay(int row, int col)
{
...
//=============================================================================
void
VariableSingleTableModel::resizeEditData(int oldRows, int oldCols)
{
...
//=============================================================================
void
VariableSingleTableModel::schedulePeristenceUpdate()
{
    if (!m_persistenceTimer) {
        ...
        connect(m_persistenceTimer, &QTimer::timeout, this,
            &VariableSingleTableModel::performBatchedPersistence);
    }
...
//=============================================================================
bool
VariableSingleTableModel::resizeArray(int row, int col)
{
    if (row < m_rows && col < m_cols) {
    ...
    const size_t MAX_ELEMENTS = 50000000; // ~400MB for single
    ...
    try {
        ...
        if (cls != NLS_SINGLE && cls != NLS_SCOMPLEX) {
            return false; // Unsupported type
        }

        const size_t oldSize = static_cast<size_t>(m_rows) * m_cols;
        const size_t elementSize = (cls == NLS_SCOMPLEX) ? 2 : 1;
        const size_t totalNewSize = newSize * elementSize;

        Dimensions newDims(newRows, newCols);
        single* ptr = (single*)ArrayOf::allocateArrayOf(cls, newDims.getElementCount());
        ArrayOf newArray(cls, newDims, ptr);
        single* newData = (single*)newArray.getDataPointer();

        // Initialize with zeros
        std::fill(newData, newData + totalNewSize, 0.0);

        // Copy old data column-wise
        if (const single* oldData = (const single*)m_array.getDataPointer()) {
            for (int c = 0; c < m_cols; ++c) {
                ...
                std::memcpy(newData + newColStart, oldData + oldColStart,
                    sizeof(single) * m_rows * elementSize);
            }
        }
...
//=============================================================================
Qt::ItemFlags
VariableSingleTableModel::flags(const QModelIndex& index) const
{
    if (!index.isValid()) {
    ...
    // Only allow editing of single arrays
    if (m_array.isSingleType(false)) {
...
//=============================================================================
QVariant
VariableSingleTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
//=============================================================================
void
VariableSingleTableModel::refreshFromArray(ArrayOf& value)
{
    m_array = value;
    beginResetModel();
    initializeDimensions();
...
//=============================================================================
bool
VariableSingleTableModel::pasteDataFromClipboard(int startRow, int startCol, const QString& text)
{
    if (text.isEmpty()) {
        return false;
    }
...
//=============================================================================
bool
VariableSingleTableModel::pasteDataFromExcel(int startRow, int startCol, const QString& text)
{
    if (text.isEmpty()) {
        return false;
    }
...
//=============================================================================
bool
VariableSingleTableModel::canPasteAt(int startRow, int startCol, const QString& text) const
{
...
//=============================================================================
QSize
VariableSingleTableModel::getPasteSize(const QString& text) const
{
    const PasteData pasteData = parsePasteText(text);
...
//=============================================================================
void
VariableSingleTableModel::initializeDimensions()
{
...
//=============================================================================
void
VariableSingleTableModel::initializeEditData()
{
...
//=============================================================================
bool
VariableSingleTableModel::validateArrayState() const
{
    return !m_variableName.isEmpty();
}
//=============================================================================
QVariant
VariableSingleTableModel::getDisplayValue(int row, int col) const
{
    ...
    if (row < m_rows && col < m_cols && hasOriginalData()) {
        const single* ptr = static_cast<const single*>(m_array.getDataPointer());
...
//=============================================================================
bool
VariableSingleTableModel::hasOriginalData() const
{
    return m_array.isSingleType(false) && m_array.getDataPointer() != nullptr;
}
//=============================================================================
bool
VariableSingleTableModel::isValidPosition(int row, int col) const
{
    return row >= 0 && col >= 0 && row < m_rows_display && col < m_cols_display;
}
//=============================================================================
bool
VariableSingleTableModel::expandDisplayIfNeeded(int row, int col)
{
...
//=============================================================================
bool
VariableSingleTableModel::resizeArrayIfNeeded(int row, int col)
{
    if (row < m_rows && col < m_cols) {
        return true; // No resize needed
    }
    ...
    try {
        ...
        auto newData = std::make_unique<single[]>(newSize);
        std::fill(newData.get(), newData.get() + newSize, 0.0);

        // Copy existing data
        if (const single* oldData = static_cast<const single*>(m_array.getDataPointer())) {
            for (int c = 0; c < m_cols; ++c) {
        ...
        if (single* arrayData = (single*)(m_array.getDataPointer())) {
            std::memcpy(arrayData, newData.get(), sizeof(single) * newSize);
        } else {
...
//=============================================================================
bool
VariableSingleTableModel::setArrayValue(int row, int col, ArrayOf& value)
{
    if (m_array.getDataClass() == NLS_SINGLE && value.isComplex()) {
        m_array.promoteType(NLS_SCOMPLEX);
    }

    single* data = (single*)(m_array.getDataPointer());
    if (!data) {
    ...
    if (value.isComplex()) {
        std::complex<single> c = value.getContentAsSingleComplexScalar();
        data[2 * idx] = c.real();
        data[2 * idx + 1] = c.imag();
    } else {
        data[idx] = value.getContentAsSingleScalar();
    }
    if (m_array.allReal()) {
        m_array.promoteType(NLS_SINGLE);
    }
    return true;
}
//=============================================================================
void
VariableSingleTableModel::updateEditData(int row, int col, const QVariant& value)
{
    const int idx = indexToFlat(row, col);
...
//=============================================================================
QString
VariableSingleTableModel::formatDisplayValue(std::complex<single> value) const
{
...
//=============================================================================
void
VariableSingleTableModel::persistChangesToContext()
{
...
//=============================================================================
int
VariableSingleTableModel::indexToFlat(int row, int col) const
{
    if (!isValidPosition(row, col)) {
        return -1;
    }
...
std::pair<int, int>
VariableSingleTableModel::flatToIndex(int flatIndex) const
{
...
//=============================================================================
VariableSingleTableModel::PasteData
VariableSingleTableModel::parsePasteText(const QString& text) const
{
    PasteData result;
...
//=============================================================================
bool
VariableSingleTableModel::validatePasteData(const PasteData& data, int startRow, int startCol) const
{
...
//=============================================================================
bool
VariableSingleTableModel::applyPasteData(const PasteData& data, int startRow, int startCol)
{
    if (!data.isValid) {
        return false;
    }
    ...
    for (int r = 0; r < data.rows; ++r) {
        for (int c = 0; c < data.cols && c < data.cells[r].size(); ++c) {
            ...
            if (value.isComplex()) {
                value.promoteType(NLS_SCOMPLEX);
            } else {
                value.promoteType(NLS_SINGLE);
            }
            if (setArrayValue(targetRow, targetCol, value)) {
                if (value.isComplex()) {
                    std::complex<single> cplxValue = value.getContentAsSingleComplexScalar();
                    updateEditData(targetRow, targetCol, QVariant(formatDisplayValue(cplxValue)));
                } else {
                    single numericValue = value.getContentAsSingleScalar();
                    updateEditData(
...
//=============================================================================
void
VariableSingleTableModel::forceViewUpdate()
{
    beginResetModel();
    endResetModel();
    emit modelChanged();
...
//=============================================================================
bool
VariableSingleTableModel::isStructureCompatible(const ArrayOf& value)
{
    return value.getDataClass() == NLS_SINGLE || value.getDataClass() == NLS_SCOMPLEX;
}
//=============================================================================
bool
VariableSingleTableModel::insertRowAt(int row)
{
    ...
    try {
        size_t total = newRows * cols;
        ArrayOf newArray;
        if (m_array.getDataClass() == NLS_SCOMPLEX) {
            single* ptr = (single*)ArrayOf::allocateArrayOf(NLS_SCOMPLEX, total);
            std::complex<single>* dst = reinterpret_cast<std::complex<single>*>(ptr);
            ...
            single* ptr = (single*)ArrayOf::allocateArrayOf(NLS_SINGLE, total);
            single* dst = ptr;
...
//=============================================================================
bool
VariableSingleTableModel::deleteRowAt(int row)
{
    if (row < 0) {
        return false;
    }
...
//=============================================================================
bool
VariableSingleTableModel::deleteColumnAt(int col)
{
    if (col < 0) {
        return false;
    }
...
//=============================================================================
QString
VariableSingleTableModel::getSelectedDataAsText(const QModelIndexList& selectedIndexes) const
{
...
//=============================================================================
ArrayOf
VariableSingleTableModel::createArrayFromSelection(const QModelIndexList& selectedIndexes) const
{
    ...
    if (m_array.getDataClass() == NLS_SINGLE) {
        single* ptr = (single*)ArrayOf::allocateArrayOf(NLS_SINGLE, newDims.getElementCount());
        newVar = ArrayOf(NLS_SINGLE, newDims, ptr);

        for (int r = bounds.minRow; r <= bounds.maxRow; ++r) {
            for (int c = bounds.minCol; c <= bounds.maxCol; ++c) {
                QModelIndex idx = index(r, c);
                QVariant val = data(idx, Qt::DisplayRole);
                single num = val.toDouble();
                ptr[(r - bounds.minRow) + (c - bounds.minCol) * newDims.getRows()] = num;
            }
        }
    } else if (m_array.getDataClass() == NLS_SCOMPLEX) {
        single* ptr = (single*)ArrayOf::allocateArrayOf(NLS_SCOMPLEX, newDims.getElementCount());
        std::complex<single>* complexPtr = reinterpret_cast<std::complex<single>*>(ptr);
        newVar = ArrayOf(NLS_SCOMPLEX, newDims, ptr);

        for (int r = bounds.minRow; r <= bounds.maxRow; ++r) {
            for (int c = bounds.minCol; c <= bounds.maxCol; ++c) {
                single* ptrModel = (single*)m_array.getDataPointer();
                std::complex<single>* complexPtrModel
                    = reinterpret_cast<std::complex<single>*>(ptrModel);
...
//=============================================================================
bool
VariableSingleTableModel::isValidSelectionForExtraction(
    const QModelIndexList& selectedIndexes) const
{
    return !selectedIndexes.isEmpty()
        && (m_array.getDataClass() == NLS_SINGLE || m_array.getDataClass() == NLS_SCOMPLEX);
}
//=============================================================================
bool
VariableSingleTableModel::shouldDisplayAsTable() const
{
    return (m_array.getDataClass() == NLS_SINGLE || m_array.getDataClass() == NLS_SCOMPLEX)
        && !m_array.isSparse() && m_array.is2D();
}
//=============================================================================
VariableSingleTableModel::SelectionBounds
VariableSingleTableModel::getSelectionBounds(const QModelIndexList& selectedIndexes) const
{

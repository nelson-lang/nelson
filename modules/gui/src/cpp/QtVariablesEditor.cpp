...
QtVariablesEditor::refreshVariable(const QString& variableName)
{
    ...
    if (tableView && shouldBeTableView) {
        ...
        bool needDoubleModel = value.isDoubleType();
        bool needStringModel = value.isStringArray();
        ...
        if (needDoubleModel) {
            ...
            newModel = new VariableStringTableModel(variableName, value, m_evaluator, tableView);
        }
        if (newModel) {
...
QtVariablesEditor::pasteDataFromClipboard()
{
    ...
    try {
        if (modelAbstract) {
            pasteSuccess = modelAbstract->pasteData(startRow, startCol, clipboardText);
        }
    } catch (const std::exception& e) {
        QMessageBox::critical(this, TR("Paste Error"),
...
QtVariablesEditor::pasteExcelDataFromClipboard()
{
    ...
    bool pasteSuccess = false;
    try {
        pasteSuccess = modelAbstract->pasteData(startRow, startCol, processedText);
...
QtVariablesEditor::preprocessExcelData(const QString& rawData)
{
    ...
    // Handle Excel-specific formatting issues

    // 1. Replace Excel's Unicode minus sign (U+2212) with regular hyphen-minus
    processedData.replace(QChar(0x2212), QChar('-'));

    // 2. Handle Excel's representation of empty cells
    processedData.replace("\t\t", "\t\t"); // Keep double tabs as-is for now

    // 3. Handle Excel number formatting - remove thousand separators in some locales
    // This is locale-dependent, but we'll handle common cases
    ...
    // 4. Handle Excel scientific notation formatting
    processedData.replace("E+", "e+");
    processedData.replace("E-", "e-");

    // 5. Handle Excel percentage values (remove % symbol and convert)
    ...
    // 6. Handle Excel date/time formats - convert to ISO format if detected
    // This is a simplified approach; a full implementation would need more sophisticated date
    // parsing
    QRegularExpression datePattern("(\\d{1,2})/(\\d{1,2})/(\\d{4})");
    processedData.replace(datePattern, "\\3-\\1-\\2"); // MM/DD/YYYY to YYYY-MM-DD

    // 7. Handle Excel boolean values
    processedData.replace("TRUE", "true", Qt::CaseInsensitive);
    processedData.replace("FALSE", "false", Qt::CaseInsensitive);

    // 8. Clean up any extra whitespace that Excel might add

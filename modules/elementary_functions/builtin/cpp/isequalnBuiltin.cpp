    ...
    Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    std::string overloadTypeName = overloadFunctionName(destinationTypeName, "isequaln");
...
generic_isequalnBuiltin(const ArrayOfVector& args, NelsonType commonType)
{
    ArrayOfVector retval;
    for (size_t k = 1; k < args.size(); k++) {
        ...
        bool res = IsEqual(A, B, false, complexTypeOrNot, true);
        if (!res) {
    ...
    Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    FunctionDef* funcDef = nullptr;
    std::string overloadTypeName = "builtin_cell_isequaln";
    if (!FunctionsInMemory::getInstance()->find(overloadTypeName, funcDef)) {
        Context* context = eval->getContext();
        context->lookupFunction(overloadTypeName, funcDef);
    }

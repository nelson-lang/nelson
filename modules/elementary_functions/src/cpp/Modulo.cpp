...
namespace Nelson {
	...
	//=============================================================================
	template <class T>
	T nelson_mod_integer(T x, T y)
	{
		return y != 0 ? x % y : x;
	}
	//=============================================================================
	template <class T>
	T nelson_mod(T x, T y)
	{
		T res = x;
		if (y != 0)
		{
	...
	//=============================================================================
	template <class T>
	void modfunc(indexType N, T* C, const T*A, int stride1, const T*B, int stride2)
	{
...
#pragma omp parallel for
	...
	//=============================================================================
	template <class T>
	void modfunc_integer(indexType N, T* C, const T*A, int stride1, const T*B, int stride2)
	{
		if ((stride1 == 1) && (stride2 == 1))
		{
#if defined(__NLS_WITH_OPENMP)
...
#pragma omp parallel for
	...
	//=============================================================================
	ArrayOf Modulo(ArrayOf A, ArrayOf B)
    {
		ArrayOf M;
		...
		else if (A.isIntegerType() || B.isIntegerType())
		{
			...
			if (classA == classB)
			{
				...
				case NLS_INT8:
				{
					...
					modfunc_integer<int8>(Clen, (int8*)Cp, (int8*)A.getDataPointer(), Astride, (int8*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_UINT8:
				{
					Cp = new_with_exception<uint8>(Clen);
					modfunc_integer<uint8>(Clen, (uint8*)Cp, (uint8*)A.getDataPointer(), Astride, (uint8*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_INT16:
				{
					Cp = new_with_exception<int16>(Clen);
					modfunc_integer<int16>(Clen, (int16*)Cp, (int16*)A.getDataPointer(), Astride, (int16*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_UINT16:
				{
					Cp = new_with_exception<uint16>(Clen);
					modfunc_integer<uint16>(Clen, (uint16*)Cp, (uint16*)A.getDataPointer(), Astride, (uint16*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_INT32:
				{
					Cp = new_with_exception<int32>(Clen);
					modfunc_integer<int32>(Clen, (int32*)Cp, (int32*)A.getDataPointer(), Astride, (int32*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_UINT32:
				{
					Cp = new_with_exception<uint32>(Clen);
					modfunc_integer<uint32>(Clen, (uint32*)Cp, (uint32*)A.getDataPointer(), Astride, (uint32*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_INT64:
				{
					Cp = new_with_exception<int64>(Clen);
					modfunc_integer<int64>(Clen, (int64*)Cp, (int64*)A.getDataPointer(), Astride, (int64*)B.getDataPointer(), Bstride);
				}
				break;
				case NLS_UINT64:
				{
					Cp = new_with_exception<uint64>(Clen);
					modfunc_integer<uint64>(Clen, (uint64*)Cp, (uint64*)A.getDataPointer(), Astride, (uint64*)B.getDataPointer(), Bstride);
				}
				break;
				default:
				{
		...
		else
		{
			...
			if (A.isSingleType() || B.isSingleType())
			{
				...
				modfunc<double>(Clen, (double*)Cp, (double*)A.getDataPointer(), Astride, (double*)B.getDataPointer(), Bstride);

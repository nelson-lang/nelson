...
namespace Nelson {
    //=============================================================================
    template <class T>
    boost::container::vector<indexType> nanIndices(T *values, indexType nbElements)
    {
    ...
    //=============================================================================
    template <class T>
    boost::container::vector<indexType> infIndices(T *values, indexType nbElements)
    {
        boost::container::vector<indexType> res;
        for (indexType k = 0; k < nbElements; k++)
        {
    ...
    //=============================================================================
    template <class T>
    boost::container::vector<indexType> ninfIndices(T *values, indexType nbElements)
    {
        boost::container::vector<indexType> res;
        for (indexType k = 0; k < nbElements; k++)
        {
    ...
    //=============================================================================
    template <class T>
    bool IsApprox(T *ptrA, T *ptrB, Dimensions dims, double precision)
    {
        bool bRes = false;
        ...
        if (!dims.is2D())
        {
            Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> matA(ptrA, 1, dims.getElementCount());
            Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> matB(ptrB, 1, dims.getElementCount());
            bRes = matA.isApprox(matB, (T)precision);
        }
        else
        {
            Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> matA(ptrA, dims.getRows(), dims.getColumns());
            Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> matB(ptrB, dims.getRows(), dims.getColumns());
            bRes = matA.isApprox(matB, (T)precision);
        }
        return bRes;
    }
    //=============================================================================
    bool IsApprox(ArrayOf A, ArrayOf B, double precision)
    {
        bool bRes = false;

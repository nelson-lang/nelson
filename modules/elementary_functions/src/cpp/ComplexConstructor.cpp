...
namespace Nelson {
    //=============================================================================
    template <class T> void ComplexArrayArray(T *A, T *B, T *C, size_t len)
    {
        size_t l = 0;
    ...
    //=============================================================================
    template <class T> void ComplexScalarArray(T A, T *B, T *C, size_t len)
    {
        size_t l = 0;
        for (size_t k = 0; k < len; k++)
        {
    ...
    //=============================================================================
    template <class T> void ComplexArrayScalar(T *A, T B, T *C, size_t len)
    {
        size_t l = 0;
        for (size_t k = 0; k < len; k++)
        {
    ...
    //=============================================================================
    ArrayOf ComplexConstructor(ArrayOf arrayA, ArrayOf arrayB)
    {
        ArrayOf res;
        ...
        if ((arrayA.getDataClass() == NLS_SCOMPLEX) ||
                (arrayA.getDataClass() == NLS_DCOMPLEX) ||
                (arrayB.getDataClass() == NLS_SCOMPLEX) ||
                (arrayB.getDataClass() == NLS_DCOMPLEX))
        {
            throw Exception(_W("Input arguments must be real."));
        }
        Class DestinationClass = NLS_DCOMPLEX;
        if ((arrayA.getDataClass() == NLS_SINGLE) ||
                (arrayB.getDataClass() == NLS_SINGLE))
        {
            DestinationClass = NLS_SCOMPLEX;
        }
        if (!(arrayA.getDimensions().equals(arrayB.getDimensions()) || arrayA.isScalar() || arrayB.isScalar()))
        {
    ...
    //=============================================================================
    ArrayOf ComplexConstructor(ArrayOf arrayA)
    {
        ArrayOf res;
        if (arrayA.isSparse())
        {
            throw Exception(_W("Undefined function '") + utf8_to_wstring(ClassName(arrayA)) + L"_complex'");
        }

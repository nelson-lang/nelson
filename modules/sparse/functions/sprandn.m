...
function R = sprandn(varargin)
  ...
  switch nargin()
    case 1
      R = sprandn_case1(varargin{1});
    case 2
      error(_('Invalid number of input arguments.'));
    case 3
      R = sprandn_case3(varargin{1}, varargin{2}, varargin{3});
    otherwise
      error(_('Invalid number of input arguments.'));
    end
end
%=============================================================================
function R = sprandn_case3(m_in, n_in, density_in)
  numRows = double(m_in);
  numCols = double(n_in);
  ...
  if numNonZerosWanted > 0
    ...
    rowIndices = pairs(:,1);
    colIndices = pairs(:,2);
    randomValues = randn(length(rowIndices), 1, 'double');
...
%=============================================================================
function R = sprandn_case1(S)
  if ~ismatrix(S)
    error(_('Input must be a matrix.'));
  end
  [numRows, numCols] = size(S);
  [rowIndices, colIndices] = find(S);
  R = sparse(rowIndices, colIndices, randn(numel(rowIndices), 1, 'double'), numRows, numCols);
end
%=============================================================================

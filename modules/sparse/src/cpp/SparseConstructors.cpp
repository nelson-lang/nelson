...
namespace Nelson {
    //=============================================================================
    ArrayOf SparseConstructor(indexType m, indexType n)
    {
        ArrayOf res;
        try
        {
            Dimensions dims(m, n);
            Eigen::SparseMatrix<double, 0, signedIndexType> *spmat = new Eigen::SparseMatrix<double, 0, signedIndexType>(m, n);
    ...
    //=============================================================================
    ArrayOf SparseConstructor(ArrayOf a)
    {
        ArrayOf res;
        if (a.isSparse())
        {
    ...
    //=============================================================================
    ArrayOf SparseConstructor(ArrayOf I, ArrayOf J, ArrayOf V)
    {
        ArrayOf res;
        ...
        void *spmat = Eigen_makeSparseFromIJV(V.getDataClass(), rows, cols, olen, ip, istride, jp, jstride, V.getDataPointer(), vstride, bScalarV);
        res = ArrayOf(V.getDataClass(), dim, spmat, true);
        return res;
    }
    //=============================================================================
    ArrayOf SparseConstructor(ArrayOf I, ArrayOf J, ArrayOf V, indexType m, indexType n)
    {
        ArrayOf res;
        ...
        void *spmat = Eigen_CopyResizeSparseMatrix(res.getDataClass(), res.getSparseDataPointer(), dims.getRows(), dims.getColumns(), m, n);
        return ArrayOf(V.getDataClass(), newdims, spmat, true);
    }
    //=============================================================================
    ArrayOf SparseConstructor(ArrayOf I, ArrayOf J, ArrayOf V, indexType m, indexType n, indexType nnz)
    {
        ...
        switch (res.getDataClass())
        {
            case NLS_LOGICAL:
            {
                Eigen::SparseMatrix<logical, 0, signedIndexType> *spmat = (Eigen::SparseMatrix<logical, 0, signedIndexType>*) res.getSparseDataPointer();
            ...
            case NLS_DCOMPLEX:
            {
                Eigen::SparseMatrix<doublecomplex, 0, signedIndexType> *spmat = (Eigen::SparseMatrix<doublecomplex, 0, signedIndexType>*) res.getSparseDataPointer();
                if (nnz >= (indexType)spmat->nonZeros())
                {
                    spmat->reserve(nnz);
                    spmat->finalize();

...
namespace Nelson {
    //=============================================================================
    ArrayOf SparseRealPart(ArrayOf a)
    {
        ArrayOf res;
        if (!a.isSparse())
        {
        ...
        switch (a.getDataClass())
        {
            case NLS_LOGICAL:
            {
                try
                {
                    ...
                    Eigen::SparseMatrix<logical, 0, signedIndexType> *spmatSRC = (Eigen::SparseMatrix<logical, 0, signedIndexType>*) a.getSparseDataPointer();
                    for (indexType k = 0; k < (indexType)spmatSRC->outerSize(); ++k)
                    {
                        for (Eigen::SparseMatrix<logical, 0, signedIndexType>::InnerIterator it(*spmatSRC, k); it; ++it)
                        {

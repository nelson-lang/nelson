...
namespace Nelson {
    ...
    //=============================================================================
    static std::string printNumber(double number, OutputFormatDisplay currentFormat)
    {
    ...
    //=============================================================================
    static std::string printNumber(double realpart, double imagpart, OutputFormatDisplay currentFormat)
    {
        std::string strNumber = "";
        if (imagpart != 0.0)
        {
            if (imagpart > 0.0)
            {
                strNumber = printNumber(realpart, currentFormat) + " + " + printNumber(imagpart, currentFormat) + "i";
            }
            else
            {
                strNumber = printNumber(realpart, currentFormat) + " - " + printNumber(abs(imagpart), currentFormat) + "i";
            }
        }
        else
        {
    ...
    void SparseDoubleDisplay(Evaluator *eval, ArrayOf a)
    {
        ...
        if (io)
        {
            ...
            if ((rows == 0) || (cols == 0))
            {
                std::string msg = str(boost::format("\tAll zero sparse: %d-by-%d\n") % (int)rows % (int)cols);
                ...
                else
                {
                    ...
                    for (indexType k = 0; k < (indexType)spMat->outerSize(); ++k)
                    {
                        ...
                        for (Eigen::SparseMatrix<double, 0, signedIndexType>::InnerIterator it(*spMat, k); it; ++it)
                        {
                            ...
                            std::string strNumber = printNumber(it.value(), eval->getCurrentOutputFormatDisplay());
                            std::string msg = str(boost::format("\t(%d,%d)\t%s\n") % (it.row() + 1) % (it.col() + 1) % strNumber.c_str());
                            io->outputMessage(msg);
                        }
                    }
                }
                ...
                else
                {
                    ...
                    for (indexType k = 0; k < (indexType)spMat->outerSize(); ++k)
                    {
                        ...
                        for (Eigen::SparseMatrix<doublecomplex, 0, signedIndexType>::InnerIterator it(*spMat, k); it; ++it)
                        {
                            ...
                            std::string strNumber = printNumber(it.value().real(), it.value().imag(), eval->getCurrentOutputFormatDisplay());
                            std::string msg = str(boost::format("\t(%d,%d)\t%s\n") % (it.row() + 1) % (it.col() + 1) % strNumber.c_str());
                            io->outputMessage(msg);
                        }
                    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (indexType k = 0; k < (indexType)spMat->outerSize(); ++k)
                    {
                        ...
                        for (Eigen::SparseMatrix<logical, 0, signedIndexType>::InnerIterator it(*spMat, k); it; ++it)
                        {
                            if (eval->GetInterruptPending())
                            {
                                break;

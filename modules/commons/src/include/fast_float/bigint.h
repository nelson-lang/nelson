...
template <uint16_t size> struct stackvec
{
    ...
    FASTFLOAT_CONSTEXPR20
    void
    resize_unchecked(size_t new_len, limb value) noexcept
    {
        if (new_len > len()) {
    ...
    FASTFLOAT_CONSTEXPR14 bool
    nonzero(size_t index) const noexcept
    {
        while (index < len()) {
            if (rindex(index) != 0) {
                return true;
...
template <uint16_t size>
FASTFLOAT_CONSTEXPR20 bool
large_add_from(stackvec<size>& x, limb_span y, size_t start) noexcept
{
...
template <typename = void> struct pow5_tables
{
    ...
    constexpr static limb large_power_of_5[] = { 4279965485U, 329373468U, 4020270615U, 2137533757U,
        4287402176U, 1057042919U, 1071430142U, 2440757623U, 381945767U, 46164893U };
#endif
};
...
struct bigint : pow5_tables<>
{
    ...
    FASTFLOAT_CONSTEXPR20 uint64_t
    hi64(bool& truncated) const noexcept
    {
#ifdef FASTFLOAT_64BIT_LIMB
        if (vec.len() == 0) {

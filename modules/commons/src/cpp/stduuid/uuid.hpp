...
namespace detail {
    ...
    is_hex(TChar const ch) noexcept
    {
...
public:
    ...
    [[nodiscard]] inline span<std::byte const, 16>
    as_bytes() const
    {
...
private:
    ...
    friend std::basic_string<CharT, Traits, Allocator>
    to_string(uuid const& id);
...
static uuid uuid_namespace_x500 { { 0x6b, 0xa7, 0xb8, 0x14, 0x9d, 0xad, 0x11, 0xd1, 0x80, 0xb4,
...
public:
    ...
    operator()()
    {
        ...
        std::array<uint8_t, 16> arrbytes = { { bytes.byte0, bytes.byte1, bytes.byte2, bytes.byte3,
            bytes.byte4, bytes.byte5, bytes.byte6, bytes.byte7, bytes.byte8, bytes.byte9,
...
class uuid_time_generator
{
    ...
    [[nodiscard]] static unsigned short
    get_clock_sequence()
    {
...
template <> struct hash<uuids::uuid>
{
    ...
    operator()(argument_type const& uuid) const
    {
        ...
        uint64_t l = static_cast<uint64_t>(uuid.data[0]) << 56
            | static_cast<uint64_t>(uuid.data[1]) << 48 | static_cast<uint64_t>(uuid.data[2]) << 40
            | static_cast<uint64_t>(uuid.data[3]) << 32 | static_cast<uint64_t>(uuid.data[4]) << 24
            | static_cast<uint64_t>(uuid.data[5]) << 16 | static_cast<uint64_t>(uuid.data[6]) << 8
            | static_cast<uint64_t>(uuid.data[7]);
        uint64_t h = static_cast<uint64_t>(uuid.data[8]) << 56
            | static_cast<uint64_t>(uuid.data[9]) << 48 | static_cast<uint64_t>(uuid.data[10]) << 40

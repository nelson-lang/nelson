...
//=============================================================================
ArrayOfVector Nelson::MemoryGateway::memoryBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    if (nLhs == 0)
    {
        ...
        if (io)
        {
            ...
            msg = _W("Physical Memory used by Nelson:") + L" " + std::to_wstring(val) + L" MB" + L" (" + std::to_wstring((uint64)getTotalPhysicalMemoryByNelson()) + L" bytes)" + L"\n";
            io->outputMessage(msg);
            val = (unsigned int)(getTotalVirtualMemoryByNelson() * 9.54e-7);
            msg = _W("Virtual Memory used by Nelson:") + L" " + std::to_wstring(val) + L" MB" + L" (" + std::to_wstring((uint64)getTotalVirtualMemoryByNelson()) + L" bytes)" + L"\n";
            io->outputMessage(msg);
            val = (unsigned int)(getTotalPhysicalMemory() * 9.54e-7);
            msg = _W("Physical Memory:") + L" " + std::to_wstring(val) + L" MB" + L" (" + std::to_wstring((uint64)getTotalPhysicalMemory()) + L" bytes)" + L"\n";
            io->outputMessage(msg);
            val = (unsigned int)(getTotalVirtualMemory() * 9.54e-7);
            msg = _W("Virtual Memory:") + L" " + std::to_wstring(val) + L" MB" + L" (" + std::to_wstring((uint64)getTotalVirtualMemory()) + L" bytes)" + L"\n";
            io->outputMessage(msg);
        }
    }
    if (nLhs > 0)
    ...
    if (nLhs > 1)
    {
        ...
        fieldvalues1.push_back(ArrayOf::doubleConstructor(getTotalVirtualMemory() - getTotalVirtualMemoryUsed()));
        ...
        fieldnames1.push_back(L"Available");
        fieldnames1.push_back(L"Total");
        fieldvalues1.push_back(ArrayOf::doubleConstructor(getTotalPhysicalMemory() - getTotalPhysicalMemoryUsed()));

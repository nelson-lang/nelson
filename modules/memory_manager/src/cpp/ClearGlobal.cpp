...
namespace Nelson {
//=============================================================================
static bool callClearHandle(Evaluator *eval, Scope *scope,
                            std::string variable) {
  bool res = false;
  ArrayOf val;
    ...
    if (val.isHandle()) {
      ...
      for (indexType k = 0; k < dimsVal.getElementCount(); k++) {
        ...
        HandleGenericObject *hlObj =
            HandleManager::getInstance()->getPointer(hl);
        if (hlObj) {
...
//=============================================================================
bool ClearPersistentVariable(Evaluator *eval, std::wstring variable) {
    return ClearPersistentVariable(eval, wstring_to_utf8(variable));
}
//=============================================================================
bool ClearPersistentVariable(Evaluator *eval, std::string variable) {
  bool res = false;
  FuncPtr func;

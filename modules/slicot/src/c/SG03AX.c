...
/* Subroutine */ EXPORTSYMBOL int sg03ax_(trans, n, a, lda, e, lde, x, ldx, scale, info, trans_len)
char *trans;
integer *n;
doublereal *a;
integer *lda;
...
L60:
            if (ll + lb <= *n)
            {
                ...
                if (ll > 1)
                {
                    ...
                    i__1 = lh - kh + 1;
                    dgemm_("T", "N", &i__1, &lb, &kb, &c_b11, &e[kl + kh * e_dim1], lde, tm, &c__2, &c_b11, &x[kh + ll * x_dim1], ldx, 1L, 1L);
                    if (kb == 2)
                    {
                        daxpy_(&lb, &e[kl + kl * e_dim1], tm, &c__2, &x[kl + ll * x_dim1], ldx);
                    }
                }
                ...
                {
                    dimmat = 1;
                    mat[0] = a[ll + ll * a_dim1] * a[kl + kl * a_dim1] - e[ll + ll * e_dim1] * e[kl + kl * e_dim1];
                ...
                if (kl < ll)
                {
                    ...
                    dgemv_("N", &kb, &lb, &c_b11, &x[kl + ll * x_dim1], ldx, &e[ll + lh * e_dim1], &c__1, &c_b12, &tm[(lb << 1) - 2], &c__1, 1L);
                    i__1 = lh - kh;
                    dgemm_("T", "N", &i__1, &lb, &kb, &c_b11, &e[kl + (kh + 1) * e_dim1], lde, tm, &c__2, &c_b11, &x[kh + 1 + ll * x_dim1], ldx, 1L, 1L);
                }
                goto L60;
            }
            /*           END WHILE 60 */
...
L140:
            if (kl > 1)
            {
                ...
                if (kh < *n)
                {
                    ...
                    i__1 = ll - kl + 1;
                    dgemm_("N", "T", &kb, &i__1, &lb, &c_b11, tm, &c__2, &e[kl + ll * e_dim1], lde, &c_b11, &x[kl + kl * x_dim1], ldx, 1L, 1L);
                    if (lb == 2)
                    {
                        daxpy_(&kb, &e[lh + lh * e_dim1], &tm[2], &c__1, &x[kl + lh * x_dim1], &c__1);
                    }
                }
                /*              Solve small Sylvester equations of order at most (2,2). */
                if (kb == 1 && lb == 1)

...
EXPORTSYMBOL /* Subroutine */ int fb01rd_(jobk, multbq, n, m, p, s, lds, a, lda, b, ldb, q, ldq, c__, ldc, r__, ldr, k, ldk, tol, iwork, dwork, ldwork, info, jobk_len, multbq_len)
char *jobk, *multbq;
integer *n, *m, *p;
doublereal *s;
integer *lds;
...
/* Computing MAX */
	i__1 = 2, i__2 = pn * *n + *n, i__1 = max(i__1,i__2), i__2 = pn * *n + (*p << 1), i__1 = max(i__1,i__2), i__2 = *n * (*n + *m + 2), i__1 = max(i__1,i__2), i__2 = *p * 3;
/* Computing MAX */
	i__3 = 1, i__4 = pn * *n + *n, i__3 = max(i__3,i__4), i__4 = pn * *n + (*p << 1), i__3 = max(i__3,i__4), i__4 = *n * (*n + *m + 2);
...
/*     following subroutine, as returned by ILAENV.) */
    ...
    i__1 = min(*n,*p);
    dlacpy_("Lower", p, &i__1, &c__[c_offset], ldc, &dwork[1], &pn, 5L);
    i__1 = min(*n,*p);
    dlacpy_("Full", n, &i__1, &a[a_offset], lda, &dwork[*p + 1], &pn, 4L);
    if (*n > *p) {
	i__1 = *n - *p;
	dlacpy_("Lower", n, &i__1, &a[(*p + 1) * a_dim1 + 1], lda, &dwork[*p * pn + *p + 1], &pn, 5L);
    }
...
/*     Workspace: need (P+N+1)*N. */
	...
	i__2 = *n - i__ + 1;
	dtrmv_("Lower", "No transpose", "Non-unit", &i__2, &dwork[ii], &pn, &dwork[pl], &c__1, 5L, 12L, 8L);
	i__2 = *n - i__ + 1;
...
/*     Compute [ A2  A4 ] x S. */
    ...
    dtrmm_("Right", "Lower", "No transpose", "Non-unit", p, n, &c_b23, &s[s_offset], lds, &dwork[*n + 1], &pn, 5L, 5L, 12L, 8L);
...
/*     Workspace: need (N+P)*N + 2*P. */
    ...
    mb04ld_("Lower", p, n, n, &r__[r_offset], ldr, &dwork[1], &pn, &dwork[*p + 1], &pn, &k[k_offset], ldk, &dwork[itau], &dwork[jwork], 5L);
/* Computing MAX */
...
/*     following computations, and make room for B x Q. */
    ...
    dlacpy_("Full", n, n, &dwork[*p + 1], &pn, &dwork[1], n, 4L);
...
/*     Workspace: need N*(M+N). */
    ...
    if (! lmultb) {
	dtrmm_("Right", "Lower", "No transpose", "Non-unit", n, m, &c_b23, &q[q_offset], ldq, &dwork[1], n, 5L, 5L, 12L, 8L);
    }
...
/* Computing MAX */
    ...
    i__2 = max(i__3,0);
    i__4 = *ldwork - jwork + 1;
    mb04jd_(n, &i__1, &i__2, &c__0, &dwork[1], n, &dwork[1], n, &dwork[itau], &dwork[jwork], &i__4, info);
/* Computing MAX */
...
/*        Workspace: need 3*P. */
	...
	mb02od_("Right", "Lower", "No transpose", "Non-unit", "1-norm", n, p, &c_b23, &r__[r_offset], ldr, &k[k_offset], ldk, &rcond, tol, &iwork[1], &dwork[1], info, 5L, 5L, 12L, 8L, 6L);
	if (*info == 0) {
/* Computing MAX */

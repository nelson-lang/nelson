...
/* Subroutine */ EXPORTSYMBOL int ab08nd_(equil, n, m, p, a, lda, b, ldb, c__, ldc, d__, ldd, nu, rank, dinfz, nkror, nkrol, infz, kronr, kronl, af, ldaf, bf, ldbf, tol, iwork, dwork, ldwork, info, equil_len)
char *equil;
integer *n, *m, *p;
doublereal *a;
integer *lda;
...
ftnlen equil_len;
{
        ...
        else /* if(complicated condition) */
        {
            ...
            else
            {
                ...
                if (lquery)
                {
                    svlmax = 0.;
                    ninfz = 0;
                    ab08nx_(n, m, p, p, &c__0, &svlmax, &bf[bf_offset], ldbf, &ninfz, &infz[1], &kronl[1], &mu, nu, nkrol, tol, &iwork[1], &dwork[1], &c_n1, info);
                    /* Computing MAX */
                    i__1 = i__, i__2 = (integer) dwork[1];
                    wrkopt = max(i__1,i__2);
                    i__1 = *m - ii;
                    ab08nx_(n, &ii, m, &i__1, &ii, &svlmax, &af[af_offset], ldaf, &ninfz, &infz[1], &kronl[1], &mu, nu, nkrol, tol, &iwork[1], &dwork[1], &c_n1, info);
                    /* Computing MAX */
                    i__1 = wrkopt, i__2 = (integer) dwork[1];
                    wrkopt = max(i__1,i__2);
                    i__1 = *n + ii;
                    nb = ilaenv_(&c__1, "DGERQF", " ", &ii, &i__1, &c_n1, &c_n1, 6L, 1L);
                    /* Computing MAX */
                    i__1 = wrkopt, i__2 = ii + ii * nb;
                    wrkopt = max(i__1,i__2);
                    /* Computing MIN */
                    i__3 = *n + ii;
                    i__1 = 64, i__2 = ilaenv_(&c__1, "DORMRQ", "RT", n, &i__3, &ii, &c_n1, 6L, 2L);
                    nb = min(i__1,i__2);
                    /* Computing MAX */
    ...
    *nkror = 0;
    ...
    /*                                    ( D  C ) */
    dlacpy_("Full", &nn, &mm, &b[b_offset], ldb, &bf[bf_offset], ldbf, 4L);
    if (pp > 0)
    {
        dlacpy_("Full", &pp, &mm, &d__[d_offset], ldd, &bf[nn + 1 + bf_dim1], ldbf, 4L);
    }
    if (nn > 0)
    {
        dlacpy_("Full", &nn, &nn, &a[a_offset], lda, &bf[(mm + 1) * bf_dim1 + 1], ldbf, 4L);
        if (pp > 0)
        {
            dlacpy_("Full", &pp, &nn, &c__[c_offset], ldc, &bf[nn + 1 + (mm + 1) * bf_dim1], ldbf, 4L);
        }
    }
    ...
    {
        maxred = 0.;
        tb01id_("A", &nn, &mm, &pp, &maxred, &bf[(mm + 1) * bf_dim1 + 1], ldbf, &bf[bf_offset], ldbf, &bf[nn + 1 + (mm + 1) * bf_dim1], ldbf, &dwork[1], info, 1L);
        wrkopt = *n;
    }
    ...
    i__1 = nn + pp;
    i__2 = nn + mm;
    svlmax = dlange_("Frobenius", &i__1, &i__2, &bf[bf_offset], ldbf, &dwork[1], 9L);
    ...
    sigma = 0;
    ninfz = 0;
    ab08nx_(&nn, &mm, &pp, &ro, &sigma, &svlmax, &bf[bf_offset], ldbf, &ninfz, &infz[1], &kronl[1], &mu, nu, nkrol, &toler, &iwork[1], &dwork[1], ldwork, info);
    /* Computing MAX */
    i__1 = wrkopt, i__2 = (integer) dwork[1];
    wrkopt = max(i__1,i__2);
    *rank = mu;
    ...
    if (numu != 0)
    {
        ...
        for (i__ = 1; i__ <= i__1; ++i__)
        {
            dcopy_(&mnu, &bf[i__ + bf_dim1], ldbf, &af[(numu1 - i__) * af_dim1 + 1], &c_n1);
            /* L80: */
        }
        if (mu != mm)
        {
            ...
            ro = pp - mm;
            sigma = mm;
            ab08nx_(&nn, &mm, &pp, &ro, &sigma, &svlmax, &af[af_offset], ldaf, &ninfz, &infz[1], &kronr[1], &mu, nu, nkror, &toler, &iwork[1], &dwork[1], ldwork, info);
            /* Computing MAX */
            i__1 = wrkopt, i__2 = (integer) dwork[1];
            wrkopt = max(i__1,i__2);
        }
        if (*nu != 0)
        {
            ...
            dlaset_("Full", nu, &mu, &c_b30, &c_b30, &bf[bf_offset], ldbf, 4L);
            dlaset_("Full", nu, nu, &c_b30, &c_b34, &bf[(mu + 1) * bf_dim1 + 1], ldbf, 4L);
            if (*rank != 0)
            {
                nu1 = *nu + 1;
                i1 = *nu + mu;
                ...
                i__1 = *ldwork - mu;
                dtzrzf_(&mu, &i1, &af[nu1 + af_dim1], ldaf, &dwork[1], &dwork[mu + 1], &i__1, info);
                /* Computing MAX */
                ...
                /*                         prefer MIN(M,P) + N*NB. */
                i__1 = *ldwork - mu;
                dormrz_("Right", "Transpose", nu, &i1, &mu, nu, &af[nu1 + af_dim1], ldaf, &dwork[1], &af[af_offset], ldaf, &dwork[mu + 1], &i__1, info, 5L, 9L);
                /* Computing MAX */
                i__1 = wrkopt, i__2 = (integer) dwork[mu + 1] + mu;
                wrkopt = max(i__1,i__2);
                i__1 = *ldwork - mu;
                dormrz_("Right", "Transpose", nu, &i1, &mu, nu, &af[nu1 + af_dim1], ldaf, &dwork[1], &bf[bf_offset], ldbf, &dwork[mu + 1], &i__1, info, 5L, 9L);
            }
            /*           Move AF and BF in the first columns. This assumes that */
            /*           DLACPY moves column by column. */
            dlacpy_("Full", nu, nu, &af[(mu + 1) * af_dim1 + 1], ldaf, &af[af_offset], ldaf, 4L);
            if (*rank != 0)
            {
                dlacpy_("Full", nu, nu, &bf[(mu + 1) * bf_dim1 + 1], ldbf, &bf[bf_offset], ldbf, 4L);
            }
        }
    }

...
EXPORTSYMBOL /* Subroutine */ int ab08nd_(equil, n, m, p, a, lda, b, ldb, c__, ldc, d__, ldd, nu,
    rank, dinfz, nkror, nkrol, infz, kronr, kronl, af, ldaf, bf, ldbf, tol, iwork, dwork, ldwork,
    info, equil_len) char* equil;
integer *n, *m, *p;
doublereal* a;
integer* lda;
doublereal* b;
...
ftnlen equil_len;
{
            ...
            if (*ldbf < max(i__1, i__2)) {
                *info = -24;
                ...
                if (lquery) {
                    svlmax = 0.;
                    ninfz = 0;
                    ab08nx_(n, m, p, p, &c__0, &svlmax, &bf[bf_offset], ldbf, &ninfz, &infz[1],
                        &kronl[1], &mu, nu, nkrol, tol, &iwork[1], &dwork[1], &c_n1, info);
                    /* Computing MAX */
                    i__1 = i__, i__2 = (integer)dwork[1];
                    wrkopt = max(i__1, i__2);
                    i__1 = *m - ii;
                    ab08nx_(n, &ii, m, &i__1, &ii, &svlmax, &af[af_offset], ldaf, &ninfz, &infz[1],
                        &kronl[1], &mu, nu, nkrol, tol, &iwork[1], &dwork[1], &c_n1, info);
                    /* Computing MAX */
    ...
    *nkror = 0;
    ...
    if (lequil && nn > 0 && pp > 0) {
        maxred = 0.;
        tb01id_("A", &nn, &mm, &pp, &maxred, &bf[(mm + 1) * bf_dim1 + 1], ldbf, &bf[bf_offset],
    ...
    sigma = 0;
    ninfz = 0;
    ab08nx_(&nn, &mm, &pp, &ro, &sigma, &svlmax, &bf[bf_offset], ldbf, &ninfz, &infz[1], &kronl[1],
        &mu, nu, nkrol, &toler, &iwork[1], &dwork[1], ldwork, info);
    /* Computing MAX */
    i__1 = wrkopt, i__2 = (integer)dwork[1];
    wrkopt = max(i__1, i__2);
    *rank = mu;
    ...
    if (numu != 0) {
        ...
        if (mu != mm) {
            ...
            ro = pp - mm;
            sigma = mm;
            ab08nx_(&nn, &mm, &pp, &ro, &sigma, &svlmax, &af[af_offset], ldaf, &ninfz, &infz[1],
                &kronr[1], &mu, nu, nkror, &toler, &iwork[1], &dwork[1], ldwork, info);
            /* Computing MAX */
            i__1 = wrkopt, i__2 = (integer)dwork[1];
            wrkopt = max(i__1, i__2);
        }
        if (*nu != 0) {
            ...
            if (*rank != 0) {
                ...
                /*                         prefer MIN(M,P) + N*NB. */
                i__1 = *ldwork - mu;
                dormrz_("Right", "Transpose", nu, &i1, &mu, nu, &af[nu1 + af_dim1], ldaf, &dwork[1],
                    &af[af_offset], ldaf, &dwork[mu + 1], &i__1, info, 5L, 9L);
                /* Computing MAX */
                i__1 = wrkopt, i__2 = (integer)dwork[mu + 1] + mu;
                wrkopt = max(i__1, i__2);
                i__1 = *ldwork - mu;
                dormrz_("Right", "Transpose", nu, &i1, &mu, nu, &af[nu1 + af_dim1], ldaf, &dwork[1],
                    &bf[bf_offset], ldbf, &dwork[mu + 1], &i__1, info, 5L, 9L);
            }

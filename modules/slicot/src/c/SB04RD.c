...
EXPORTSYMBOL /* Subroutine */ int sb04rd_(abschu, ula, ulb, n, m, a, lda, b, ldb, c__, ldc, tol,
    ...
    *info = 0;
    ...
    if (!labscb && !labscs && !lsame_(abschu, "A", 1L, 1L)) {
        ...
        *info = -11;
        ...
        || *ldwork < (maxmn << 1) * ((maxmn << 1) + 4) && !(labscs && lula && lulb)) {
        *info = -15;
    }
    if (*info != 0) {
        /*        Error return. */
    ...
    if (labscs && lula && lulb) {
        ...
        sb04py_("NoTranspose", "NoTranspose", &c__1, n, m, &a[a_offset], lda, &b[b_offset], ldb,
            &c__[c_offset], ldc, &scale, &dwork[1], info, 11L, 11L);
        if (scale != 1.) {
            *info = 1;
        }
        return 0;
    ...
    *(unsigned char*)abschr = *(unsigned char*)abschu;
    ...
    L20:
        if ((iend - i__) * fwd >= 0) {
            ...
            if (istep == 1) {
                sb04rw_(abschr, ulb, n, m, &c__[c_offset], ldc, &i__, &b[b_offset], ldb,
                    &a[a_offset], lda, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04ry_("R", ula, n, &a[a_offset], lda, &b[i__ + i__ * b_dim1], &dwork[jwork],
                ...
                ipincr = i__ + incr;
                sb04rv_(abschr, ulb, n, m, &c__[c_offset], ldc, &ipincr, &b[b_offset], ldb,
                    &a[a_offset], lda, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04rx_("R", ula, n, &a[a_offset], lda, &b[ipincr + ipincr * b_dim1],
    ...
    L40:
        if ((iend - i__) * fwd >= 0) {
            ...
            if (istep == 1) {
                sb04rw_(abschr, ula, n, m, &c__[c_offset], ldc, &i__, &a[a_offset], lda,
                    &b[b_offset], ldb, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04ry_("C", ulb, m, &b[b_offset], ldb, &a[i__ + i__ * a_dim1], &dwork[jwork],
                    &tol1, &iwork[1], &dwork[1], &ldw, info, 1L, 1L);
                if (*info == 1) {
                    return 0;
                }
                ...
                ipincr = i__ + incr;
                sb04rv_(abschr, ula, n, m, &c__[c_offset], ldc, &ipincr, &a[a_offset], lda,
                    &b[b_offset], ldb, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04rx_("C", ulb, m, &b[b_offset], ldb, &a[ipincr + ipincr * a_dim1],

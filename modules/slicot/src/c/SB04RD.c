...
/* Subroutine */ EXPORTSYMBOL int sb04rd_(abschu, ula, ulb, n, m, a, lda, b, ldb, c__, ldc, tol, iwork, dwork, ldwork, info, abschu_len, ula_len, ulb_len)
char *abschu, *ula, *ulb;
integer *n, *m;
doublereal *a;
integer *lda;
...
ftnlen ulb_len;
{
    ...
    extern /* Subroutine */ EXPORTSYMBOL int sb04py_(), sb04rv_(), sb04rw_();
    static integer istep;
    extern /* Subroutine */ EXPORTSYMBOL int sb04rx_(), sb04ry_();
    ...
    *info = 0;
    ...
    else if (*ldwork < *n << 1 || *ldwork < (maxmn << 1) * ((maxmn << 1) + 4) && ! (labscs && lula && lulb))
    {
        *info = -15;
    }
    if (*info != 0)
    ...
    if (labscs && lula && lulb)
    {
        ...
        sb04py_("NoTranspose", "NoTranspose", &c__1, n, m, &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset], ldc, &scale, &dwork[1], info, 11L, 11L);
        if (scale != 1.)
        {
            *info = 1;
        }
...
L20:
        if ((iend - i__) * fwd >= 0)
        {
            ...
            if (istep == 1)
            {
                sb04rw_(abschr, ulb, n, m, &c__[c_offset], ldc, &i__, &b[b_offset], ldb, &a[a_offset], lda, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04ry_("R", ula, n, &a[a_offset], lda, &b[i__ + i__ * b_dim1], &dwork[jwork], &tol1, &iwork[1], &dwork[1], &ldw, info, 1L, 1L);
                if (*info == 1)
                {
                    return 0;
                }
            ...
            {
                ipincr = i__ + incr;
                sb04rv_(abschr, ulb, n, m, &c__[c_offset], ldc, &ipincr, &b[b_offset], ldb, &a[a_offset], lda, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04rx_("R", ula, n, &a[a_offset], lda, &b[ipincr + ipincr * b_dim1], &b[ipincr + 1 + ipincr * b_dim1], &b[ipincr + (ipincr + 1) * b_dim1], &b[ipincr + 1 + (ipincr + 1) * b_dim1], &dwork[jwork], &tol1, &iwork[1], &dwork[1], &ldw, info, 1L, 1L);
                if (*info == 1)
                {
                    return 0;
                }
                dcopy_(n, &dwork[jwork], &c__2, &c__[ipincr * c_dim1 + 1], &c__1);
                dcopy_(n, &dwork[jwork + 1], &c__2, &c__[(ipincr + 1) * c_dim1 + 1], &c__1);
            }
            i__ += fwd * istep;
            goto L20;
        }
...
L40:
        if ((iend - i__) * fwd >= 0)
        {
            ...
            if (istep == 1)
            {
                sb04rw_(abschr, ula, n, m, &c__[c_offset], ldc, &i__, &a[a_offset], lda, &b[b_offset], ldb, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04ry_("C", ulb, m, &b[b_offset], ldb, &a[i__ + i__ * a_dim1], &dwork[jwork], &tol1, &iwork[1], &dwork[1], &ldw, info, 1L, 1L);
                if (*info == 1)
                {
                    return 0;
                }
            ...
            {
                ipincr = i__ + incr;
                sb04rv_(abschr, ula, n, m, &c__[c_offset], ldc, &ipincr, &a[a_offset], lda, &b[b_offset], ldb, &dwork[jwork], &dwork[1], 1L, 1L);
                sb04rx_("C", ulb, m, &b[b_offset], ldb, &a[ipincr + ipincr * a_dim1], &a[ipincr + 1 + ipincr * a_dim1], &a[ipincr + (ipincr + 1) * a_dim1], &a[ipincr + 1 + (ipincr + 1) * a_dim1], &dwork[jwork], &tol1, &iwork[1], &dwork[1], &ldw, info, 1L, 1L);
                if (*info == 1)
                {
                    return 0;
                }
                dcopy_(m, &dwork[jwork], &c__2, &c__[ipincr + c_dim1], ldc);
                dcopy_(m, &dwork[jwork + 1], &c__2, &c__[ipincr + 1 + c_dim1], ldc);
            }
            i__ += fwd * istep;
            goto L40;
        }

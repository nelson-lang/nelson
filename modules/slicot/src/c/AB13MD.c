...
/* Subroutine */ EXPORTSYMBOL int ab13md_(fact, n, z__, ldz, m, nblock, itype, x, bound, d__, g, iwork, dwork, ldwork, zwork, lzwork, info, fact_len)
char *fact;
integer *n;
doublecomplex *z__;
...
ftnlen fact_len;
{
    ...
    extern /* Subroutine */ EXPORTSYMBOL int zgees_(), dcopy_(), zgemm_();
    static doublereal hnorm, snorm, svlam;
    static logical gtest, bwork[1];
    static doublereal t1;
    static integer izwrk;
    static doublereal t2, t3, znorm;
    extern /* Subroutine */ EXPORTSYMBOL int dsysv_(), zcopy_(), zgemv_();
    static doublereal hnorm1, dlambd, ynorm1, hn, ynorm2, znorm2;
    ...
    extern /* Subroutine */ EXPORTSYMBOL int zlascl_();
    static integer lzamax;
    extern /* Subroutine */ EXPORTSYMBOL int dsycon_(), zgetrf_();
    static doublereal colsum;
    extern /* Subroutine */ EXPORTSYMBOL int zgesvd_(), zgetri_(), zlacpy_();
    ...
    *info = 0;
    ...
    lzamax = 0;
    /*     Set D = In, G = 0. */
    dlaset_("Full", n, &c__1, &c_b11, &c_b11, &d__[1], n, 4L);
...
L120:
    ...
    c__ = 1.;
    /*     Set H. */
    dlaset_("Full", &mt, &mt, &c_b15, &c_b11, &dwork[iw11 + 1], &mt, 4L);
    iter = -1;
    /*     Main iteration loop. */
L390:
    ++iter;
...
L810:
    ...
    if (mr > 0)
    {
        ...
        for (i__ = 1; i__ <= i__1; ++i__)
        {
            ...
            dwork[iw10 + *m - 1 + i__] = 1. / (d__1 * d__1) + 1. / (d__2 * d__2);
            /* L980: */
        }
    }
    i__1 = mt;
    ...
    /*           Compute norm( H ). */
L1110:
    hnorm = dlange_("F", &mt, &mt, &dwork[iw11 + 1], &mt, &dwork[1], 1L);
    /*           Compute rcond( H ). */
    dlacpy_("Full", &mt, &mt, &dwork[iw11 + 1], &mt, &dwork[iw31 + 1], &mt, 4L);
    ...
    dcopy_(&mt, &dwork[iw26 + 1], &c__1, &dwork[iw27 + 1], &c__1);
    dsytrs_("U", &mt, &c__1, &dwork[iw31 + 1], &mt, &iwork[1], &dwork[iw27 + 1], &mt, &info2, 1L);
...
L1600:
    ...
    i__1 = mt + 1;
    i__2 = mt + 1;
    c__ = dlange_("F", &i__1, &c__1, &dwork[iw33 + 1], &i__2, &dwork[1], 1L);
    /*        Set x = yw(2:mt+1). */

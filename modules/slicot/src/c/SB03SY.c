...
EXPORTSYMBOL /* Subroutine */ int sb03sy_(job, trana, lyapun, n, t, ldt, u, ldu, xa, ldxa, sepd, thnorm, iwork, dwork, ldwork, info, job_len, trana_len, lyapun_len)
...
ftnlen lyapun_len;
{
    ...
    integer t_dim1, t_offset, u_dim1, u_offset, xa_dim1, xa_offset, i__1, i__2;
    /* Local variables */
...
L10:
        ...
        if (kase != 0)
        {
            ...
            if (kase == 1)
            {
                ...
                sb03mx_(trana, n, &t[t_offset], ldt, &dwork[1], n, &scale, &dwork[itmp], &info2, 1L);
            }
            else
            {
                /*              Solve op(T)*W*op(T)' - W = scale*RHS. */
                sb03mx_(tranat, n, &t[t_offset], ldt, &dwork[1], n, &scale, &dwork[itmp], &info2, 1L);
            }
            if (info2 > 0)
            {
                *info = *n + 1;
...
L20:
        ...
        if (kase != 0)
        {
            ...
            dsyr2k_(uplo, tranat, n, n, &c_b22, &dwork[1], n, &xa[xa_offset], ldxa, &c_b21, &dwork[itmp], n, 1L, 1L);

...
EXPORTSYMBOL /* Subroutine */ int ab09nd_(dico, job, equil, ordsel, n, m, p, nr, alpha, a, lda, b,
    ldb, c__, ldc, d__, ldd, ns, hsv, tol1, tol2, iwork, dwork, ldwork, iwarn, info, dico_len,
    job_len, equil_len, ordsel_len) char *dico,
    *job, *equil, *ordsel;
integer *n, *m, *p, *nr;
doublereal *alpha, *a;
integer* lda;
...
ftnlen ordsel_len;
{
    ...
    *ns = *n - nu;
    ...
    /*                prefer larger. */
    i__1 = *ldwork - kw + 1;
    ab09bx_(dico, job, ordsel, ns, m, p, &nra, &a[nu1 + nu1 * a_dim1], lda, &b[nu1 + b_dim1], ldb,
        &c__[nu1 * c_dim1 + 1], ldc, &d__[d_offset], ldd, &hsv[1], &dwork[kt], n, &dwork[kti], n,
        tol1, tol2, &iwork[1], &dwork[kw], &i__1, iwarn, &ierr, 1L, 1L, 1L);

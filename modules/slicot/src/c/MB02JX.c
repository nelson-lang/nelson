...
EXPORTSYMBOL /* Subroutine */ int mb02jx_(job, k, l, m, n, tc, ldtc, tr, ldtr, rnk, q, ldq, r__, ldr, jpvt, tol1, tol2, dwork, ldwork, info, job_len)
char *job;
integer *k, *l, *m, *n;
doublereal *tc;
integer *ldtc;
...
ftnlen job_len;
{
    ...
    *info = 0;
    ...
    if (compq)
    {
        ...
        /* Computing MAX */
        i__3 = mk, i__4 = (*n - 1) **l;
        i__1 = 3, i__2 = (mk + (*n - 1) **l) * (*l + (*k << 1)) + *l * 9 + max(i__3,i__4);
    ...
    else
    {
        /* Computing MAX */
        /* Computing MAX */
        i__3 = (*n - 1) **l * (*l + (*k << 1) + 1) + *l * 9, i__4 = mk * (*l + 1) + *l;
    ...
    {
        /*        Catch M*K <= L. */
        dlacpy_("All", &mk, l, &tc[tc_offset], ldtc, &dwork[1], &mk, 3L);
        pdw = mk **l + 1;
        jwork = pdw + mk;
        i__1 = *ldwork - jwork + 1;
        dgeqrf_(&mk, l, &dwork[1], &mk, &dwork[pdw], &dwork[jwork], &i__1, &ierr);
        /* Computing MAX */
        i__1 = wrkopt, i__2 = (integer) dwork[jwork] + jwork - 1;
        wrkopt = max(i__1,i__2);
        ma02ad_("Upper part", &mk, l, &dwork[1], &mk, &r__[r_offset], ldr, 10L);
        i__1 = *ldwork - jwork + 1;
        dorgqr_(&mk, &mk, &mk, &dwork[1], &mk, &dwork[pdw], &dwork[jwork], &i__1, &ierr);
        /* Computing MAX */
        i__1 = wrkopt, i__2 = (integer) dwork[jwork] + jwork - 1;
        wrkopt = max(i__1,i__2);
        if (compq)
        {
            dlacpy_("All", &mk, &mk, &dwork[1], &mk, &q[q_offset], ldq, 3L);
        }
        pdw = mk * mk + 1;
        if (*n > 1)
        {
            i__1 = *n - 1;
            i__2 = *ldwork - pdw + 1;
            mb02kd_("Row", "Transpose", k, l, m, &i__1, &mk, &c_b10, &c_b11, &tc[tc_offset], ldtc, &tr[tr_offset], ldtr, &dwork[1], &mk, &r__[*l + 1 + r_dim1], ldr, &dwork[pdw], &i__2, &ierr, 3L, 9L);
        }
        /* Computing MAX */
    ...
    if (compq)
    {
        dlacpy_("All", &mk, l, &tc[tc_offset], ldtc, &q[q_offset], ldq, 3L);
        i__1 = *ldwork - *l;
        dgeqp3_(&mk, l, &q[q_offset], ldq, &jpvt[1], &dwork[1], &dwork[*l + 1], &i__1, &ierr);
        /* Computing MAX */
        ...
        if (ltol1 < 0.)
        {
            ...
            dlaset_("All", l, &c__1, &temp, &temp, &dwork[*l + 1], &c__1, 3L);
            for (i__ = 1; i__ <= 5; ++i__)
            {
                dtrmv_("Upper", "NonTranspose", "NonUnit", l, &q[q_offset], ldq, &dwork[*l + 1], &c__1, 5L, 12L, 7L);
                dtrmv_("Upper", "Transpose", "NonUnit", l, &q[q_offset], ldq, &dwork[*l + 1], &c__1, 5L, 9L, 7L);
                nrm = dnrm2_(l, &dwork[*l + 1], &c__1);
                d__1 = 1. / nrm;
                dscal_(l, &d__1, &dwork[*l + 1], &c__1);
                /* L30: */
            }
            ltol1 = sqrt(nrm * dlamch_("Epsilon", 7L));
        }
        i__ = *l;
L40:
        ...
        rrnk = i__;
        rrdf = *l - rrnk;
        ma02ad_("Upper", &rrnk, l, &q[q_offset], ldq, &r__[r_offset], ldr, 5L);
        if (rrnk > 1)
        {
            i__1 = *l - 1;
            i__2 = rrnk - 1;
        ...
        i__1 = *ldwork - *l;
        dorgqr_(&mk, l, &rrnk, &q[q_offset], ldq, &dwork[1], &dwork[*l + 1], &i__1, &ierr);
        /* Computing MAX */
        i__1 = wrkopt, i__2 = (integer) dwork[*l + 1] + *l;
        wrkopt = max(i__1,i__2);
        if (*n > 1)
        {
            i__1 = *n - 1;
            mb02kd_("Row", "Transpose", k, l, m, &i__1, &rrnk, &c_b10, &c_b11, &tc[tc_offset], ldtc, &tr[tr_offset], ldtr, &q[q_offset], ldq, &r__[*l + 1 + r_dim1], ldr, &dwork[1], ldwork, &ierr, 3L, 9L);
            /* Computing MAX */
    ...
    else
    {
        ...
        dlacpy_("All", &mk, l, &tc[tc_offset], ldtc, &dwork[1], &mk, 3L);
        i__1 = *ldwork - jwork + 1;
        dgeqp3_(&mk, l, &dwork[1], &mk, &jpvt[1], &dwork[pdw], &dwork[jwork], &i__1, &ierr);
        /* Computing MAX */
        i__1 = wrkopt, i__2 = (integer) dwork[jwork] + jwork - 1;
        wrkopt = max(i__1,i__2);
        if (ltol1 < 0.)
        {
            ...
            /*           matrix with 5 power iterations. */
            temp = 1. / sqrt((doublereal) (*l));
            dlaset_("All", l, &c__1, &temp, &temp, &dwork[jwork], &c__1, 3L);
            for (i__ = 1; i__ <= 5; ++i__)
            {
                dtrmv_("Upper", "NonTranspose", "NonUnit", l, &dwork[1], &mk, &dwork[jwork], &c__1, 5L, 12L, 7L);
                dtrmv_("Upper", "Transpose", "NonUnit", l, &dwork[1], &mk, &dwork[jwork], &c__1, 5L, 9L, 7L);
...
L60:
        ...
        rrdf = *l - rrnk;
        ma02ad_("Upper part", &rrnk, l, &dwork[1], &mk, &r__[r_offset], ldr, 10L);
        if (rrnk > 1)
        {
            i__1 = *l - 1;
            i__2 = rrnk - 1;
        ...
        i__1 = *ldwork - jwork + 1;
        dorgqr_(&mk, l, &rrnk, &dwork[1], &mk, &dwork[pdw], &dwork[jwork], &i__1, &ierr);
        /* Computing MAX */
        i__1 = wrkopt, i__2 = (integer) dwork[jwork] + jwork - 1;
        wrkopt = max(i__1,i__2);
        if (*n > 1)
        {
            i__1 = *n - 1;
            i__2 = *ldwork - pdw + 1;
            mb02kd_("Row", "Transpose", k, l, m, &i__1, &rrnk, &c_b10, &c_b11, &tc[tc_offset], ldtc, &tr[tr_offset], ldtr, &dwork[1], &mk, &r__[*l + 1 + r_dim1], ldr, &dwork[pdw], &i__2, &ierr, 3L, 9L);
            /* Computing MAX */
            i__1 = wrkopt, i__2 = (integer) dwork[pdw] + pdw - 1;
            wrkopt = max(i__1,i__2);
        }
    ...
    if (ltol2 < 0.)
    {
        ltol2 = (doublereal) (*n **l) * sqrt(dlamch_("Epsilon", 7L));
    }
    i__1 = *l;
    for (j = 1; j <= i__1; ++j)
    {
        dcopy_(&rrnk, &r__[j + r_dim1], ldr, &r__[*l + jpvt[j] + (rrnk + 1) * r_dim1], ldr);
        /* L70: */
    }
    if (*n > 2)
    {
        i__1 = (*n - 2) **l;
        dlacpy_("All", &i__1, &rrnk, &r__[*l + 1 + r_dim1], ldr, &r__[(*l << 1) + 1 + (rrnk + 1) * r_dim1], ldr, 3L);
    }
    ...
    if (rrdf > 0)
    {
        i__1 = min(rrdf,*k);
        i__2 = (*n - 1) **l;
        ma02ad_("All", &i__1, &i__2, &tr[tr_offset], ldtr, &r__[*l + 1 + ((rrnk << 1) + 1) * r_dim1], ldr, 3L);
    }
    if (*k > rrdf)
    {
        i__1 = *k - rrdf;
        i__2 = (*n - 1) **l;
        i__3 = (*n - 1) **l;
        ma02ad_("All", &i__1, &i__2, &tr[rrdf + 1 + tr_dim1], ldtr, &dwork[1], &i__3, 3L);
    }
    ...
    i__1 = (*n - 1) **l;
    i__2 = (*n - 1) **l;
    dlacpy_("All", &i__1, &rrnk, &r__[*l + 1 + r_dim1], ldr, &dwork[pnr], &i__2, 3L);
    ...
    if (compq)
    {
        ...
        pdw = pnq + mk * (rrnk + *k);
        dlacpy_("All", &mk, &rrnk, &q[q_offset], ldq, &dwork[pnq], &mk, 3L);
        if (*m > 1)
        {
            i__1 = (*m - 1) **k;
            dlacpy_("All", &i__1, &rrnk, &q[q_offset], ldq, &q[*k + 1 + (rrnk + 1) * q_dim1], ldq, 3L);
        }
        dlaset_("All", k, &rrnk, &c_b11, &c_b11, &q[(rrnk + 1) * q_dim1 + 1], ldq, 3L);
        if (rrdf > 0)
        {
            dlaset_("All", &mk, &rrdf, &c_b11, &c_b10, &q[((rrnk << 1) + 1) * q_dim1 + 1], ldq, 3L);
        }
        /* Computing MAX */
        i__2 = 0, i__3 = *k - rrdf;
        i__1 = max(i__2,i__3);
        dlaset_("All", &rrdf, &i__1, &c_b11, &c_b11, &dwork[pdq], &mk, 3L);
        i__1 = *m **k - rrdf;
        /* Computing MAX */
        i__3 = 0, i__4 = *k - rrdf;
        i__2 = max(i__3,i__4);
        dlaset_("All", &i__1, &i__2, &c_b11, &c_b10, &dwork[pdq + rrdf], &mk, 3L);
        dlaset_("All", &mk, k, &c_b11, &c_b11, &dwork[pnq + mk * rrnk], &mk, 3L);
    }
    else
    {
    ...
    *rnk = rrnk;
    ...
    for (i__ = *l + 1; i__4 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__4)
    {
        ...
        mb02cu_("Deficient", &kk, &pp, &i__2, &c_n1, &r__[i__ + (*rnk + 1) * r_dim1], ldr, &dwork[ppr], &i__3, &dwork[pnr], &i__5, &rrnk, &jpvt[i__], &dwork[pdw], &ltol1, &dwork[pdw + *l * 5], &i__6, &ierr, 9L);
        if (ierr != 0)
        {
...
L110:
        ...
        if (len > kk)
        {
            ...
            mb02cv_("Deficient", "NoStructure", &kk, &i__2, &pp, &i__3, &c_n1, &rrnk, &r__[i__ + (*rnk + 1) * r_dim1], ldr, &dwork[ppr], &i__5, &dwork[pnr], &i__6, &r__[i__ + kk + (*rnk + 1) * r_dim1], ldr, &dwork[ppr + kk], &i__7, &dwork[pnr + kk], &i__8, &dwork[pdw], &dwork[pdw + *l * 5], &i__9, &ierr, 9L, 11L);
        }
        ...
        if (compq)
        {
            ...
            i__5 = (*n - 1) **l;
            i__6 = *ldwork - pdw - *l * 5 + 1;
            mb02cv_("Deficient", "NoStructure", &kk, &mk, &pp, &i__2, &c_n1, &rrnk, &r__[i__ + (*rnk + 1) * r_dim1], ldr, &dwork[ppr], &i__3, &dwork[pnr], &i__5, &q[(*rnk + 1) * q_dim1 + 1], ldq, &dwork[pdq], &mk, &dwork[pnq], &mk, &dwork[pdw], &dwork[pdw + *l * 5], &i__6, &ierr, 9L, 11L);
        }
        ...
        for (j = kk; j >= i__2; --j)
        {
            if ((d__1 = r__[i__ + j - 1 + (*rnk + j) * r_dim1], abs(d__1)) <= ltol1)
            {
                ++nzc;
            }
            /* L140: */
        ...
        for (j = rrnk + 1; j <= i__2; ++j)
        {
            ...
            i__3 = len - j - gap;
            d__1 = -dwork[pt + j - 1];
            daxpy_(&i__3, &d__1, &dwork[pt + j], &c__1, &r__[i__ + j + (*rnk + j) * r_dim1], &c__1);
            i__3 = len - j - gap;
            if (dnrm2_(&i__3, &r__[i__ + j + (*rnk + j) * r_dim1], &c__1) > ltol2 * abs(temp))
            {
                /*              Unlucky case: */
        ...
        rrdf = kk - rrnk;
        i__2 = i__ - 1;
        dlaset_("All", &i__2, &rrnk, &c_b11, &c_b11, &r__[(*rnk + 1) * r_dim1 + 1], ldr, 3L);
        i__2 = *l - 1;
        i__3 = rrnk - 1;
        dlaset_("Upper", &i__2, &i__3, &c_b11, &c_b11, &r__[i__ + (*rnk + 2) * r_dim1], ldr, 5L);
        ...
        if (! last)
        {
            ...
            dlaset_("All", l, &rrdf, &c_b11, &c_b11, &r__[i__ + (*rnk + rrnk + 1) * r_dim1], ldr, 3L);
            ...
            if (rrdf - nzc > 0 && nzc > 0)
            {
                ...
                {
                    i__3 = len - *l;
                    dcopy_(&i__3, &r__[i__ + *l + (j + rrdf - nzc) * r_dim1], &c__1, &r__[i__ + *l + j * r_dim1], &c__1);
                    /* L160: */
                }
            }
            ...
            if (cpcol > 0)
            {
                ...
                for (j = i__; j <= i__2; ++j)
                {
                    dcopy_(&cpcol, &r__[j + (*rnk + 1) * r_dim1], ldr, &r__[jpvt[j] + *l + (*rnk + rrnk + nzc + 1) * r_dim1], ldr);
                    /* L170: */
                }
                if (len > *l << 1)
                {
                    i__2 = len - (*l << 1);
                    dlacpy_("All", &i__2, &cpcol, &r__[i__ + *l + (*rnk + 1) * r_dim1], ldr, &r__[i__ + (*l << 1) + (*rnk + rrnk + nzc + 1) * r_dim1], ldr, 3L);
                }
            }
            ppr += *l;
            ...
            if (cpcol > 0)
            {
                i__2 = len - *l;
                i__3 = (*n - 1) **l;
                dlacpy_("All", &i__2, &cpcol, &dwork[ppr], &i__3, &r__[i__ + *l + (*rnk + (rrnk << 1) + nzc + 1) * r_dim1], ldr, 3L);
                ppr += cpcol * (*n - 1) **l;
            }
            ...
            if (compq)
            {
                if (rrdf - nzc > 0 && nzc > 0)
                {
                    ...
                    for (j = *rnk + rrnk + 1; j <= i__2; ++j)
                    {
                        dcopy_(&mk, &q[(j + rrdf - nzc) * q_dim1 + 1], &c__1, &q[j * q_dim1 + 1], &c__1);
                        /* L180: */
                    }
                }
                /* Computing MIN */
                ...
                if (cpcol > 0)
                {
                    dlaset_("All", k, &cpcol, &c_b11, &c_b11, &q[(*rnk + rrnk + nzc + 1) * q_dim1 + 1], ldq, 3L);
                    if (*m > 1)
                    {
                        i__2 = (*m - 1) **k;
                        dlacpy_("All", &i__2, &cpcol, &q[(*rnk + 1) * q_dim1 + 1], ldq, &q[*k + 1 + (*rnk + rrnk + nzc + 1) * q_dim1], ldq, 3L);
                    }
                }
                /* Computing MIN */
                i__2 = *k - rdef, i__3 = kk - rrnk - nzc;
                ...
                if (cpcol > 0)
                {
                    dlacpy_("All", &mk, &cpcol, &dwork[pdq], &mk, &q[(*rnk + (rrnk << 1) + nzc + 1) * q_dim1 + 1], ldq, 3L);
                    pdq += cpcol * mk;
                }
                pnq += (rrdf - nzc) * mk;
            }

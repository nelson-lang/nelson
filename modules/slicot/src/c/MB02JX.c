...
EXPORTSYMBOL /* Subroutine */ int mb02jx_(job, k, l, m, n, tc, ldtc, tr, ldtr, rnk, q, ldq, r__,
    ldr, jpvt, tol1, tol2, dwork, ldwork, info, job_len) char* job;
integer *k, *l, *m, *n;
doublereal* tc;
integer* ldtc;
doublereal* tr;
...
ftnlen job_len;
{
    ...
    *info = 0;
    ...
    if (mk <= *l) {
        ...
        if (*n > 1) {
            i__1 = *n - 1;
            i__2 = *ldwork - pdw + 1;
            mb02kd_("Row", "Transpose", k, l, m, &i__1, &mk, &c_b10, &c_b11, &tc[tc_offset], ldtc,
                &tr[tr_offset], ldtr, &dwork[1], &mk, &r__[*l + 1 + r_dim1], ldr, &dwork[pdw],
                &i__2, &ierr, 3L, 9L);
        }
        /* Computing MAX */
    ...
    if (compq) {
        ...
        if (ltol1 < 0.) {
            ...
            for (i__ = 1; i__ <= 5; ++i__) {
                dtrmv_("Upper", "NonTranspose", "NonUnit", l, &q[q_offset], ldq, &dwork[*l + 1],
                    &c__1, 5L, 12L, 7L);
                dtrmv_("Upper", "Transpose", "NonUnit", l, &q[q_offset], ldq, &dwork[*l + 1], &c__1,
                    5L, 9L, 7L);
    ...
    L40:
        ...
        if (*n > 1) {
            i__1 = *n - 1;
            mb02kd_("Row", "Transpose", k, l, m, &i__1, &rrnk, &c_b10, &c_b11, &tc[tc_offset], ldtc,
        ...
        if (ltol1 < 0.) {
            ...
            for (i__ = 1; i__ <= 5; ++i__) {
                dtrmv_("Upper", "NonTranspose", "NonUnit", l, &dwork[1], &mk, &dwork[jwork], &c__1,
                    5L, 12L, 7L);
                dtrmv_("Upper", "Transpose", "NonUnit", l, &dwork[1], &mk, &dwork[jwork], &c__1, 5L,
                    9L, 7L);
    ...
    if (*n > 2) {
        i__1 = (*n - 2) * *l;
        dlacpy_("All", &i__1, &rrnk, &r__[*l + 1 + r_dim1], ldr,
            &r__[(*l << 1) + 1 + (rrnk + 1) * r_dim1], ldr, 3L);
    }
    /*     2nd column of the generator. */
    if (rrdf > 0) {
        i__1 = min(rrdf, *k);
        i__2 = (*n - 1) * *l;
        ma02ad_("All", &i__1, &i__2, &tr[tr_offset], ldtr,
            &r__[*l + 1 + ((rrnk << 1) + 1) * r_dim1], ldr, 3L);
    }
    if (*k > rrdf) {
        i__1 = *k - rrdf;
        i__2 = (*n - 1) * *l;
    ...
    if (compq) {
        ...
        if (*m > 1) {
            i__1 = (*m - 1) * *k;
            dlacpy_(
                "All", &i__1, &rrnk, &q[q_offset], ldq, &q[*k + 1 + (rrnk + 1) * q_dim1], ldq, 3L);
        }
    ...
    *rnk = rrnk;
    ...
    for (i__ = *l + 1; i__4 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__4) {
        ...
        mb02cu_("Deficient", &kk, &pp, &i__2, &c_n1, &r__[i__ + (*rnk + 1) * r_dim1], ldr,
            &dwork[ppr], &i__3, &dwork[pnr], &i__5, &rrnk, &jpvt[i__], &dwork[pdw], &ltol1,
            &dwork[pdw + *l * 5], &i__6, &ierr, 9L);
        if (ierr != 0) {
        ...
        if (len > kk) {
            ...
            mb02cv_("Deficient", "NoStructure", &kk, &i__2, &pp, &i__3, &c_n1, &rrnk,
                &r__[i__ + (*rnk + 1) * r_dim1], ldr, &dwork[ppr], &i__5, &dwork[pnr], &i__6,
                &r__[i__ + kk + (*rnk + 1) * r_dim1], ldr, &dwork[ppr + kk], &i__7,
                &dwork[pnr + kk], &i__8, &dwork[pdw], &dwork[pdw + *l * 5], &i__9, &ierr, 9L, 11L);
        }
        ...
        if (compq) {
            ...
            i__5 = (*n - 1) * *l;
            i__6 = *ldwork - pdw - *l * 5 + 1;
            mb02cv_("Deficient", "NoStructure", &kk, &mk, &pp, &i__2, &c_n1, &rrnk,
                &r__[i__ + (*rnk + 1) * r_dim1], ldr, &dwork[ppr], &i__3, &dwork[pnr], &i__5,
                &q[(*rnk + 1) * q_dim1 + 1], ldq, &dwork[pdq], &mk, &dwork[pnq], &mk, &dwork[pdw],
                &dwork[pdw + *l * 5], &i__6, &ierr, 9L, 11L);
        }
        ...
        if (!last) {
            ...
            dlaset_(
                "All", l, &rrdf, &c_b11, &c_b11, &r__[i__ + (*rnk + rrnk + 1) * r_dim1], ldr, 3L);
            ...
            if (rrdf - nzc > 0 && nzc > 0) {
                ...
                for (j = *rnk + rrnk + 1; j <= i__2; ++j) {
                    i__3 = len - *l;
                    dcopy_(&i__3, &r__[i__ + *l + (j + rrdf - nzc) * r_dim1], &c__1,
                        &r__[i__ + *l + j * r_dim1], &c__1);
                    /* L160: */
                }
            }
            ...
            if (cpcol > 0) {
                i__2 = i__ + *l - 1;
                for (j = i__; j <= i__2; ++j) {
                    dcopy_(&cpcol, &r__[j + (*rnk + 1) * r_dim1], ldr,
                        &r__[jpvt[j] + *l + (*rnk + rrnk + nzc + 1) * r_dim1], ldr);
                    /* L170: */
                }
                if (len > *l << 1) {
                    i__2 = len - (*l << 1);
                    dlacpy_("All", &i__2, &cpcol, &r__[i__ + *l + (*rnk + 1) * r_dim1], ldr,
                        &r__[i__ + (*l << 1) + (*rnk + rrnk + nzc + 1) * r_dim1], ldr, 3L);
                }
            }
            ppr += *l;
            ...
            if (cpcol > 0) {
                i__2 = len - *l;
                i__3 = (*n - 1) * *l;
                dlacpy_("All", &i__2, &cpcol, &dwork[ppr], &i__3,
                    &r__[i__ + *l + (*rnk + (rrnk << 1) + nzc + 1) * r_dim1], ldr, 3L);
                ppr += cpcol * (*n - 1) * *l;
            }
            ...
            if (compq) {
                if (rrdf - nzc > 0 && nzc > 0) {
                    ...
                    i__2 = *rnk + rrnk + cpcol;
                    for (j = *rnk + rrnk + 1; j <= i__2; ++j) {
                        dcopy_(&mk, &q[(j + rrdf - nzc) * q_dim1 + 1], &c__1, &q[j * q_dim1 + 1],
                            &c__1);
                        /* L180: */
                    }
                }
                ...
                cpcol = min(i__2, i__3);
                if (cpcol > 0) {
                    dlaset_("All", k, &cpcol, &c_b11, &c_b11,
                        &q[(*rnk + rrnk + nzc + 1) * q_dim1 + 1], ldq, 3L);
                    if (*m > 1) {
                        i__2 = (*m - 1) * *k;
                        dlacpy_("All", &i__2, &cpcol, &q[(*rnk + 1) * q_dim1 + 1], ldq,
                            &q[*k + 1 + (*rnk + rrnk + nzc + 1) * q_dim1], ldq, 3L);
                    }
                }
                /* Computing MIN */
                i__2 = *k - rdef, i__3 = kk - rrnk - nzc;
                cpcol = min(i__2, i__3);
                if (cpcol > 0) {
                    dlacpy_("All", &mk, &cpcol, &dwork[pdq], &mk,
                        &q[(*rnk + (rrnk << 1) + nzc + 1) * q_dim1 + 1], ldq, 3L);
                    pdq += cpcol * mk;
                }
                pnq += (rrdf - nzc) * mk;
            }

...
EXPORTSYMBOL /* Subroutine */ int tf01md_(n, m, p, ny, a, lda, b, ldb, c__, ldc, d__, ldd, u, ldu, x, y, ldy, dwork, info)
integer *n, *m, *p, *ny;
doublereal *a;
integer *lda;
doublereal *b;
...
/*        Non-dynamic system: compute the output vectors. */
	...
	if (*m == 0) {
	    dlaset_("Full", p, ny, &c_b4, &c_b4, &y[y_offset], ldy, 4L);
	} else {
	    dgemm_("No transpose", "No transpose", p, ny, m, &c_b8, &d__[d_offset], ldd, &u[u_offset], ldu, &c_b4, &y[y_offset], ldy, 12L, 12L);
	}
	return 0;
    }
    ...
    i__1 = *ny;
    for (ik = 1; ik <= i__1; ++ik) {
	dgemv_("No transpose", p, n, &c_b8, &c__[c_offset], ldc, &x[1], &c__1, &c_b4, &y[ik * y_dim1 + 1], &c__1, 12L);

	dgemv_("No transpose", n, n, &c_b8, &a[a_offset], lda, &x[1], &c__1, &c_b4, &dwork[1], &c__1, 12L);
	dgemv_("No transpose", n, m, &c_b8, &b[b_offset], ldb, &u[ik * u_dim1 + 1], &c__1, &c_b8, &dwork[1], &c__1, 12L);

	dcopy_(n, &dwork[1], &c__1, &x[1], &c__1);
/* L10: */
    }

    dgemm_("No transpose", "No transpose", p, ny, m, &c_b8, &d__[d_offset], ldd, &u[u_offset], ldu, &c_b8, &y[y_offset], ldy, 12L, 12L);

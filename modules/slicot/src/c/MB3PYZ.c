...
/* Subroutine */ EXPORTSYMBOL int mb3pyz_(m, n, a, lda, rcond, svlmax, rank, sval, jpvt, tau, dwork, zwork, info)
integer *m, *n;
doublecomplex *a;
integer *lda;
...
integer *info;
{
    ...
    extern /* Subroutine */ EXPORTSYMBOL int xerbla_(), zlarfg_(), zlacgv_();
...
L20:
    if (*rank < k)
    {
        ...
        else
        {
            ...
            zlaic1_(&c__2, rank, &zwork[ismin], &smin, &zwork[jwork], &a[mki + nki * a_dim1], &sminpr, &s1, &c1);
            zlaic1_(&c__1, rank, &zwork[ismax], &smax, &zwork[jwork], &a[mki + nki * a_dim1], &smaxpr, &s2, &c2);
        }
            ...
            if (*svlmax **rcond <= sminpr)
            {
                if (smaxpr **rcond <= sminpr)
                {
                    if (mki > 1)
                    {
                        ...
                        i__1 = mki - 1;
                        zlarf_("Right", &i__1, &nki, &a[mki + a_dim1], lda, &tau[i__], &a[a_offset], lda, &zwork[jwork], 5L);
                        i__1 = mki + nki * a_dim1;

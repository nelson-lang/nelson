...
EXPORTSYMBOL /* Subroutine */ int td03ay_(mwork, pwork, index, dcoeff, lddcoe, ucoeff, lduco1, lduco2, n, a, lda, b, ldb, c__, ldc, d__, ldd, info)
...
integer *ldd, *info;
{
    ...
    integer a_dim1, a_offset, b_dim1, b_offset, c_dim1, c_offset, d_dim1, d_offset, dcoeff_dim1, dcoeff_offset, ucoeff_dim1, ucoeff_dim2, ucoeff_offset, i__1, i__2;
    doublereal d__1;
...
/*           row I of U(s). */
		...
		dcopy_(mwork, &ucoeff[i__ + (k * ucoeff_dim2 + 1) * ucoeff_dim1], lduco1, &b[ia + b_dim1], ldb);
		daxpy_(mwork, &temp, &ucoeff[i__ + (ucoeff_dim2 + 1) * ucoeff_dim1], lduco1, &b[ia + b_dim1], ldb);
/* L10: */
	    }
...
/*           also. */
	...
	dcopy_(mwork, &ucoeff[i__ + (ucoeff_dim2 + 1) * ucoeff_dim1], lduco1, &d__[i__ + d_dim1], ldd);

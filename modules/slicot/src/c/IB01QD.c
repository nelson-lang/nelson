...
/* Subroutine */ EXPORTSYMBOL int ib01qd_(jobx0, job, n, m, l, nsmp, a, lda, c__, ldc, u, ldu, y, ldy, x0, b, ldb, d__, ldd, tol, iwork, dwork, ldwork, iwarn, info, jobx0_len, job_len)
char *jobx0, *job;
integer *n, *m, *l, *nsmp;
doublereal *a;
integer *lda;
...
ftnlen job_len;
{
    ...
    double pow_dd();
    /* Local variables */
    static integer igam, inih, lddw, irem, lnob, ncol, rank;
    static logical ncyc;
    ...
    extern /* Subroutine */ EXPORTSYMBOL int mb04od_(), mb01td_(), mb02ud_();
    extern logical lsame_();
    extern /* Subroutine */ EXPORTSYMBOL int dgemv_();
    static doublereal rcond;
    extern /* Subroutine */ EXPORTSYMBOL int mb04oy_();
    ...
    static integer minwrk, maxwrk, minwls, n2m, ias, ini, igs, ldr;
    static doublereal dum[1];
    static integer iny, isv, ncp1, ldw2, ldw3;
    /*     SLICOT RELEASE 5.0. */
    ...
    for (icycle = 1; icycle <= i__1; ++icycle)
    {
            ...
            for (j = 1; j <= i__3; ++j)
            {
                ...
                if (j < iexpon)
                {
                    ...
                    if (*n > 1)
                    {
                        ...
                        i__4 = *n + 1;
                        i__5 = *n + 1;
                        dcopy_(&i__2, &dwork[ia + 1], &i__4, &dwork[ias + 1], &i__5);
                    }
        ...
        if (*m > 0 && withd)
        {
            ...
            if (first)
            {
                ...
                /*                         prefer v + M*NB. */
                i__2 = *ldwork - jwork + 1;
                dgeqrf_(&nobs, m, &u[u_offset], ldu, &dwork[itauu], &dwork[jwork], &i__2, &ierr);
                /*                                                  ij */

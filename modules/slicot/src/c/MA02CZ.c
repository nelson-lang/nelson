...
EXPORTSYMBOL /* Subroutine */ int ma02cz_(n, kl, ku, a, lda)
integer *n, *kl, *ku;
doublecomplex *a;
integer *lda;
{
...
/* Computing MIN */
	...
	if (i1 > 0) {
	    i__2 = -lda1;
	    zswap_(&i1, &a[i__ + 1 + a_dim1], &lda1, &a[*n - i1 + 1 + (*n - i1 + 1 - i__) * a_dim1], &i__2);
	}
/* L10: */
    }
...
/* Computing MIN */
	...
	if (i1 > 0) {
	    i__2 = -lda1;
	    zswap_(&i1, &a[(i__ + 1) * a_dim1 + 1], &lda1, &a[*n - i1 + 1 - i__ + (*n - i1 + 1) * a_dim1], &i__2);
	}
/* L20: */
    }
...
/*     Pertranspose the diagonal. */
    ...
    if (i1 > 0) {
	i__1 = -lda1;
	zswap_(&i1, &a[a_dim1 + 1], &lda1, &a[*n - i1 + 1 + (*n - i1 + 1) * a_dim1], &i__1);
    }

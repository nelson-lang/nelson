...
EXPORTSYMBOL /* Subroutine */ int mb01ud_(side, trans, m, n, alpha, h__, ldh, a, lda, b, ldb, info, side_len, trans_len)
char *side, *trans;
integer *m, *n;
...
ftnlen trans_len;
{
    ...
    *info = 0;
    ...
    if (*alpha == 0.)
    {
        ...
        dlaset_("Full", m, n, &c_b9, &c_b9, &b[b_offset], ldb, 4L);
        return 0;
    }
    ...
    dlacpy_("Full", m, n, &a[a_offset], lda, &b[b_offset], ldb, 4L);
    dtrmm_(side, "Upper", trans, "Non-unit", m, n, alpha, &h__[h_offset], ldh, &b[b_offset], ldb, 1L, 5L, 1L, 8L);
    ...
    if (lside)
    {
        ...
        if (ltrans)
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                ...
                {
                    b[i__ + j * b_dim1] += *alpha * h__[i__ + 1 + h_dim1] * a[i__ + 1 + j * a_dim1];
                    /* L10: */
                }
                /* L20: */
            }
        ...
        else
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                ...
                {
                    b[i__ + j * b_dim1] += *alpha * h__[i__ + h_dim1] * a[i__ - 1 + j * a_dim1];
                    /* L30: */
                }
                /* L40: */
            }
    ...
    else
    {
        if (ltrans)
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                if (h__[j + 1 + j * h_dim1] != 0.)
                {
                    ...
                    daxpy_(m, &d__1, &a[j * a_dim1 + 1], &c__1, &b[(j + 1) * b_dim1 + 1], &c__1);
                }
                /* L50: */
            }
        }
        else
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                ...
                {
                    d__1 = *alpha * h__[j + 1 + j * h_dim1];
                    daxpy_(m, &d__1, &a[(j + 1) * a_dim1 + 1], &c__1, &b[j * b_dim1 + 1], &c__1);
                }
                /* L60: */
            }
        }

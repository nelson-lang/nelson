    ...
    *info = 0;
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
        ...
        if (nu != 0.) {
            ...
            i__2 = *n - i__;
            daxpy_(&i__2, &mu, &qg[i__ + 1 + i__ * qg_dim1], &c__1, &dwork[1], &c__1);
        ...
        a[i__ + 1 + (i__ + 1) * a_dim1] = c__ * temp + s * qg[i__ + 1 + (i__ + 1) * qg_dim1];
        qg[i__ + 1 + (i__ + 2) * qg_dim1] = c__ * ttemp - s * temp;
        qg[i__ + 1 + (i__ + 1) * qg_dim1] = -s * temp + c__ * qg[i__ + 1 + (i__ + 1) * qg_dim1];
        ttemp = -s * ttemp - c__ * temp;
        temp = a[i__ + 1 + (i__ + 1) * a_dim1];
        qg[i__ + 1 + (i__ + 1) * qg_dim1] = c__ * qg[i__ + 1 + (i__ + 1) * qg_dim1] + s * ttemp;
        a[i__ + 1 + (i__ + 1) * a_dim1] = c__ * temp + s * qg[i__ + 1 + (i__ + 2) * qg_dim1];
        qg[i__ + 1 + (i__ + 2) * qg_dim1] = -s * temp + c__ * qg[i__ + 1 + (i__ + 2) * qg_dim1];
        cs[(i__ << 1) - 1] = c__;
        cs[i__ * 2] = s;
        ...
        if (nu != 0.) {
            ...
            i__2 = *n - i__;
            daxpy_(&i__2, &mu, &a[i__ + 1 + i__ * a_dim1], &c__1, &dwork[1], &c__1);
            /*           Apply the transformation as a rank-2 update: */
            /*                QG := QG - v * w' - w * v'. */
            i__2 = *n - i__;

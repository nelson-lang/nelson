...
L10:
    ...
    L50:
        ...
        for (k = l; k <= i__2; ++k) {
            ...
            w[0] = 1.;
            i__3 = i2 - k;
            dlarfx_("Left", &nr, &i__3, w, &tauw, &b[k + (k + 1) * b_dim1], ldb, &dwork[1], 4L);
            /*           Apply reflector V from the left to transform the rows of the */
            /*           matrix A in columns K:I2. */
            i__3 = i2 - k + 1;
            dlarfx_("Left", &nr, &i__3, v, &tauv, &a[k + k * a_dim1], lda, &dwork[1], 4L);

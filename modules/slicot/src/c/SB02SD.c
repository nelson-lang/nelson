...
EXPORTSYMBOL /* Subroutine */ int sb02sd_(job, fact, trana, uplo, lyapun, n, a, lda, t, ldt, u, ldu,
    g, ldg, q, ldq, x, ldx, sepd, rcond, ferr, iwork, dwork, ldwork, info, job_len, fact_len,
    trana_len, uplo_len, lyapun_len) char *job,
...
ftnlen lyapun_len;
{
    ...
    integer a_dim1, a_offset, g_dim1, g_offset, q_dim1, q_offset, t_dim1, t_offset, u_dim1,
    ...
    *info = 0;
    ...
    if (update || nofact) {
        ...
        dsymm_("Left", uplo, n, n, &c_b18, &g[g_offset], ldg, &x[x_offset], ldx, &c_b18,
            &dwork[ixbs + 1], n, 4L, 1L);
        if (notrna) {
            /*                                   -1 */
    ...
    if (nofact) {
        ...
        i__1 = *ldwork - lwa - (*n << 1);
        dgees_(sjob, "Not ordered", select_, n, &t[t_offset], ldt, &sdim, &dwork[lwa + 1],
            &dwork[lwa + *n + 1], &u[u_offset], ldu, &dwork[lwa + (*n << 1) + 1], &i__1, bwork,
            info, 1L, 11L);
        if (*info > 0) {
            if (lwa > 0) {
                i__1 = *n << 1;
    ...
    if (update) {
        dgemm_("NoTranspose", trana, n, n, n, &c_b18, &x[x_offset], ldx, &dwork[1], n, &c_b17,
            &dwork[ixma + 1], n, 11L, 1L);
    } else {
        mb01ud_("Right", trana, n, n, &c_b18, &t[t_offset], ldt, &x[x_offset], ldx,
            &dwork[ixma + 1], n, &info2, 5L, 1L);
    }
    if (!jobe) {
        ...
        /*                  LWR = 0,   otherwise. */
        sb03sy_("Both", trana, lyapun, n, &t[t_offset], ldt, &u[u_offset], ldu, &dwork[ixma + 1], n,
            sepd, &thnorm, &iwork[1], &dwork[1], &ixma, info, 4L, 1L, 1L);
        /* Computing MAX */
        /* Computing MAX */
        i__3 = 3, i__4 = nn << 1;
    ...
    L30:
        ...
        if (kase != 0) {
            ...
            if (dlansy_("1-norm", "Upper", n, &dwork[1], n, &dwork[ixbs + 1], 6L, 5L)
                >= dlansy_("1-norm", "Lower", n, &dwork[1], n, &dwork[ixbs + 1], 6L, 5L)) {
            ...
            mb01ru_(loup, tranat, n, n, &c_b17, &c_b18, &dwork[1], n, &dwork[ixma + 1], n,
                &dwork[1], n, &dwork[ixbs + 1], &nn, &info2, 1L, 1L);
            i__1 = *n + 1;
            dscal_(n, &c_b51, &dwork[1], &i__1);
            if (update) {
                /*              Transform the right-hand side: RHS := U'*RHS*U. */
                mb01ru_(loup, "Transpose", n, n, &c_b17, &c_b18, &dwork[1], n, &u[u_offset], ldu,
                    &dwork[1], n, &dwork[ixbs + 1], &nn, &info2, 1L, 9L);
                i__1 = *n + 1;
                dscal_(n, &c_b51, &dwork[1], &i__1);
            }
            ...
            if (kase == 1) {
                ...
                sb03mx_(trana, n, &t[t_offset], ldt, &dwork[1], n, &scale, &dwork[ixbs + 1], &info2,
                    1L);
            } else {
                /*              Solve op(T)*W*op(T)' - W = scale*RHS. */
                sb03mx_(tranat, n, &t[t_offset], ldt, &dwork[1], n, &scale, &dwork[ixbs + 1],
                    &info2, 1L);
            }
            if (update) {
                /*              Transform back to obtain the solution: Z := U*Z*U', with */
                /*              Z = Y or Z = W. */
                mb01ru_(loup, "No transpose", n, n, &c_b17, &c_b18, &dwork[1], n, &u[u_offset], ldu,
                    &dwork[1], n, &dwork[ixbs + 1], &nn, &info2, 1L, 12L);
                i__1 = *n + 1;
                dscal_(n, &c_b51, &dwork[1], &i__1);
                /*              Fill in the remaining triangle of the symmetric matrix. */
                ma02ed_(loup, n, &dwork[1], n, 1L);
        ...
        if (tmax <= 1.) {
            ...
            denom = 1. / tmax * (qnorm / tmax) + *sepd / tmax * (anorm / tmax) * thnorm
    ...
    if (!jobc) {
        ...
        if (update) {
            mb01rx_("Left", uplo, tranat, n, n, &c_b18, &c_b18, &dwork[ires + 1], n, &a[a_offset],
                lda, &dwork[ixma + 1], n, &info2, 4L, 1L, 1L);
        } else {
            mb01ry_("Left", uplo, tranat, n, &c_b18, &c_b18, &dwork[ires + 1], n, &t[t_offset], ldt,
                &dwork[ixma + 1], n, &dwork[iwrk + 1], &info2, 4L, 1L, 1L);
            dsymm_("Left", uplo, n, n, &c_b18, &g[g_offset], ldg, &dwork[ixma + 1], n, &c_b17,
                &dwork[ixbs + 1], n, 4L, 1L);
            mb01rx_("Left", uplo, "Transpose", n, n, &c_b18, &c_b18, &dwork[ires + 1], n,
                &dwork[ixma + 1], n, &dwork[ixbs + 1], n, &info2, 4L, 1L, 9L);
        }
        ...
        if (lower) {
            ...
            for (j = 1; j <= i__1; ++j) {
                ...
                for (i__ = j; i__ <= i__2; ++i__) {
                    dwork[ires + (j - 1) * *n + i__] = temp
                            * ((d__1 = q[i__ + j * q_dim1], abs(d__1))
                                  + (d__2 = x[i__ + j * x_dim1], abs(d__2)))
                        + (d__3 = dwork[ires + (j - 1) * *n + i__], abs(d__3));
                    /* L80: */
                }
                /* L90: */
            }
        ...
        if (update) {
            ...
            for (j = 1; j <= i__1; ++j) {
                i__2 = *n;
                for (i__ = 1; i__ <= i__2; ++i__) {
                    dwork[iabs + (j - 1) * *n + i__]
                        = (d__1 = dwork[iabs + (j - 1) * *n + i__], abs(d__1));
                    /* L120: */
                }
                /* L130: */
            }
            dgemm_("NoTranspose", trana, n, n, n, &c_b18, &dwork[ixbs + 1], n, &dwork[iabs + 1], n,
                &c_b17, &dwork[ixma + 1], n, 11L, 1L);
            mb01rx_("Left", uplo, tranat, n, n, &c_b18, &epsn, &dwork[ires + 1], n,
                &dwork[iabs + 1], n, &dwork[ixma + 1], n, &info2, 4L, 1L, 1L);
        } else {
            i__1 = *n;
            for (j = 1; j <= i__1; ++j) {
                /* Computing MIN */
            ...
            mb01ud_("Right", trana, n, n, &c_b18, &dwork[iabs + 1], n, &dwork[ixbs + 1], n,
                &dwork[ixma + 1], n, &info2, 5L, 1L);
            mb01ry_("Left", uplo, tranat, n, &c_b18, &epsn, &dwork[ires + 1], n, &dwork[iabs + 1],
                n, &dwork[ixma + 1], n, &dwork[iwrk + 1], &info2, 4L, 1L, 1L);
        }
        if (lower) {
            i__1 = *n;
            for (j = 1; j <= i__1; ++j) {
        ...
        if (update) {
            mb01ru_(uplo, tranat, n, n, &c_b18, &epst, &dwork[ires + 1], n, &dwork[ixma + 1], n,
                &dwork[iabs + 1], n, &dwork[ixbs + 1], &nn, &info2, 1L, 1L);
            /* Computing MAX */
            /* Computing MAX */
            i__3 = 3, i__4 = nn << 1;
            ...
            dsymm_("Left", uplo, n, n, &c_b18, &dwork[iabs + 1], n, &dwork[ixma + 1], n, &c_b17,
                &dwork[ixbs + 1], n, 4L, 1L);
            mb01ry_("Left", uplo, tranat, n, &c_b18, &epst, &dwork[ires + 1], n, &dwork[ixma + 1],
                n, &dwork[ixbs + 1], n, &dwork[iwrk + 1], &info2, 4L, 1L, 1L);
            /* Computing MAX */
            /* Computing MAX */
            i__3 = 3, i__4 = nn << 1;
        ...
        sb03sx_(trana, uplo, lyapun, n, &xanorm, &t[t_offset], ldt, &u[u_offset], ldu,
            &dwork[ires + 1], n, ferr, &iwork[1], &dwork[ixbs + 1], &ixma, info, 1L, 1L, 1L);
    }
    dwork[1] = (doublereal)wrkopt;
    return 0;
    /* *** Last line of SB02SD *** */

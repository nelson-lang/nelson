...
EXPORTSYMBOL /* Subroutine */ int sb10sd_(n, m, np, ncon, nmeas, a, lda, b, ldb, c__, ldc, d__, ldd,
    ak, ldak, bk, ldbk, ck, ldck, dk, lddk, x, ldx, y, ldy, rcond, tol, iwork, dwork, ldwork, bwork,
    info) integer *n,
    *m, *np, *ncon, *nmeas;
doublereal* a;
integer* lda;
...
integer* info;
{
    ...
    integer a_dim1, a_offset, ak_dim1, ak_offset, b_dim1, b_offset, bk_dim1, bk_offset, c_dim1,
        ...
        y_dim1, y_offset, i__1, i__2, i__3, i__4;
    /* Builtin functions */
    double sqrt();
    /* Local variables */
    ...
    *info = 0;
    if (*n < 0) {
        ...
        *info = -25;
        ...
        i__1 = 1, i__2 = *n * 14 * *n + *n * 6 + max(i__3, i__4), i__1 = max(i__1, i__2),
        i__2 = m2 * (*n + m2 + max(3, m1)), i__1 = max(i__1, i__2), i__2 = np2 * (*n + np2 + 3);
    ...
    dgemm_("N", "N", n, n, &m2, &c_b7, &b[(m1 + 1) * b_dim1 + 1], ldb, &c__[nd1 + 1 + c_dim1], ldc,
    ...
    i__3 = *n << 1;
    i__4 = *ldwork - iwrk + 1;
    sb02od_("D", "G", "N", "L", "Z", "S", n, &m2, &np1, &ak[ak_offset], ldak, &dwork[iwg], n,
        &dwork[iwq], n, &dwork[iwrk], m, &dwork[iwrk], n, &rcond2, &x[x_offset], ldx, &dwork[iwr],
        &dwork[iwi], &dwork[iwb], &dwork[iws], &i__1, &dwork[iwt], &i__2, &dwork[iwu], &i__3, &toll,
    ...
    iwrk = iwv + *n * *n;
    i__1 = *ldwork - iwrk + 1;
    sb02sd_("C", "N", "N", "L", "O", n, &ak[ak_offset], ldak, &dwork[iwc], n, &dwork[iwv], n,
        &dwork[iwg], n, &dwork[iwq], n, &x[x_offset], ldx, &sepd, &rcond[3], &ferr, &iwork[1],
    ...
    iwrk = iw2 + m2 * m2;
    /*     Compute B2'*X2 . */
    dgemm_("T", "N", &m2, n, n, &c_b8, &b[(m1 + 1) * b_dim1 + 1], ldb, &x[x_offset], ldx, &c_b12,
        &dwork[1], &m2, 1L, 1L);
    /*     Compute Im2 + B2'*X2*B2 . */
    dlaset_("L", &m2, &m2, &c_b12, &c_b8, &dwork[iw2], &m2, 1L);
    mb01rx_("Left", "Lower", "N", &m2, n, &c_b8, &c_b8, &dwork[iw2], &m2, &dwork[1], &m2,
        &b[(m1 + 1) * b_dim1 + 1], ldb, &info2, 4L, 5L, 1L);
    ...
    dgemm_("N", "N", &m2, n, n, &c_b7, &dwork[1], &m2, &a[a_offset], lda, &c_b7, &ck[ck_offset],
        ldck, 1L, 1L);
    ...
    dgemm_("N", "N", &m2, &m1, n, &c_b7, &dwork[1], &m2, &b[b_offset], ldb, &c_b7, &dwork[iwrk],
        &m2, 1L, 1L);
    ...
    dlacpy_("Full", n, n, &a[a_offset], lda, &ak[ak_offset], ldak, 4L);
    dgemm_("N", "N", n, n, &np2, &c_b7, &b[(nd2 + 1) * b_dim1 + 1], ldb, &c__[np1 + 1 + c_dim1],
    ...
    i__3 = *n << 1;
    i__4 = *ldwork - iwrk + 1;
    sb02od_("D", "G", "N", "U", "Z", "S", n, &np2, &m1, &ak[ak_offset], ldak, &dwork[iwg], n,
        &dwork[iwq], n, &dwork[iwrk], m, &dwork[iwrk], n, &rcond2, &y[y_offset], ldy, &dwork[iwr],
        &dwork[iwi], &dwork[iwb], &dwork[iws], &i__1, &dwork[iwt], &i__2, &dwork[iwu], &i__3, &toll,
        &iwork[1], &dwork[iwrk], &i__4, &bwork[1], &info2, 1L, 1L, 1L, 1L, 1L, 1L);
    if (info2 > 0) {
        *info = 3;
    ...
    iwrk = iwv + *n * *n;
    i__1 = *ldwork - iwrk + 1;
    sb02sd_("C", "N", "N", "U", "O", n, &ak[ak_offset], ldak, &dwork[iwc], n, &dwork[iwv], n,
        &dwork[iwg], n, &dwork[iwq], n, &y[y_offset], ldy, &sepd, &rcond[4], &ferr, &iwork[1],
        &dwork[iwrk], &i__1, &info2, 1L, 1L, 1L, 1L, 1L);
    if (info2 > 0) {
        rcond[4] = 0.;
    }
    ...
    iwrk = iw2 + np2 * np2;
    /*     Compute Y2*C2' . */
    dgemm_("N", "T", n, &np2, n, &c_b8, &y[y_offset], ldy, &c__[np1 + 1 + c_dim1], ldc, &c_b12,
        &dwork[1], n, 1L, 1L);
    /*     Compute Ip2 + C2*Y2*C2' . */
    dlaset_("U", &np2, &np2, &c_b12, &c_b8, &dwork[iw2], &np2, 1L);
    mb01rx_("Left", "Upper", "N", &np2, n, &c_b8, &c_b8, &dwork[iw2], &np2, &c__[np1 + 1 + c_dim1],
        ldc, &dwork[1], n, &info2, 4L, 5L, 1L);
    ...
    dgemm_("N", "N", n, &np2, n, &c_b8, &a[a_offset], lda, &dwork[1], n, &c_b8, &bk[bk_offset],
        ldbk, 1L, 1L);
    /*     Compute L2 = -( A*Y2*C2' + B1*D21' )*inv( Ip2 + C2*Y2*C2' ) . */
    dtrsm_("R", "U", "N", "N", n, &np2, &c_b7, &dwork[iw2], &np2, &bk[bk_offset], ldbk, 1L, 1L, 1L,
        1L);
    dtrsm_("R", "U", "T", "N", n, &np2, &c_b8, &dwork[iw2], &np2, &bk[bk_offset], ldbk, 1L, 1L, 1L,
        1L);
    /*     Compute F2*Y2*C2' + F0*D21' . */
    dgemm_("N", "N", &m2, &np2, n, &c_b8, &ck[ck_offset], ldck, &dwork[1], n, &c_b8, &dk[dk_offset],
        lddk, 1L, 1L);
    /*     Compute DK = L0 = ( F2*Y2*C2' + F0*D21' )*inv( Ip2 + C2*Y2*C2' ) . */
    dtrsm_("R", "U", "N", "N", &m2, &np2, &c_b8, &dwork[iw2], &np2, &dk[dk_offset], lddk, 1L, 1L,
        1L, 1L);
    dtrsm_("R", "U", "T", "N", &m2, &np2, &c_b8, &dwork[iw2], &np2, &dk[dk_offset], lddk, 1L, 1L,
        1L, 1L);
    /*     Compute CK = F2 - L0*C2 . */
    dgemm_("N", "N", &m2, n, &np2, &c_b7, &dk[dk_offset], lddk, &c__[np1 + 1 + c_dim1], ldc, &c_b8,
        &ck[ck_offset], ldck, 1L, 1L);
    /*     Find AK = A + B2*( F2 - L0*C2 ) + L2*C2 . */
    dlacpy_("Full", n, n, &a[a_offset], lda, &ak[ak_offset], ldak, 4L);
    dgemm_("N", "N", n, n, &m2, &c_b8, &b[(m1 + 1) * b_dim1 + 1], ldb, &ck[ck_offset], ldck, &c_b8,
        &ak[ak_offset], ldak, 1L, 1L);
    dgemm_("N", "N", n, n, &np2, &c_b8, &bk[bk_offset], ldbk, &c__[np1 + 1 + c_dim1], ldc, &c_b8,
        &ak[ak_offset], ldak, 1L, 1L);
    /*     Find BK = -L2 + B2*L0 . */
    dgemm_("N", "N", n, &np2, &m2, &c_b8, &b[(m1 + 1) * b_dim1 + 1], ldb, &dk[dk_offset], lddk,
        &c_b7, &bk[bk_offset], ldbk, 1L, 1L);

...
EXPORTSYMBOL /* Subroutine */ int tb01xz_(jobd, n, m, p, kl, ku, a, lda, b, ldb, c__, ldc, d__, ldd, info, jobd_len)
char *jobd;
integer *n, *m, *p, *kl, *ku;
doublecomplex *a;
integer *lda;
    ...
    *info = 0;
    ...
    else
    {
        ...
        for (j = 1; j <= i__1; ++j)
        {
            j1 = (*n - j) / 2;
            i__2 = -lda1;
            zswap_(&j1, &a[j + 1 + a_dim1], &lda1, &a[*n - j1 + 1 + (*n - j1 + 1 - j) * a_dim1], &i__2);
            /* L20: */
        }
        ...
        for (j = 1; j <= i__1; ++j)
        {
            j1 = (*n - j) / 2;
            i__2 = -lda1;
            zswap_(&j1, &a[(j + 1) * a_dim1 + 1], &lda1, &a[*n - j1 + 1 - j + (*n - j1 + 1) * a_dim1], &i__2);
            /* L30: */
        }

...
/* Subroutine */ EXPORTSYMBOL int ab09bd_(dico, job, equil, ordsel, n, m, p, nr, a, lda, b, ldb, c__, ldc, d__, ldd, hsv, tol1, tol2, iwork, dwork, ldwork, iwarn, info, dico_len, job_len, equil_len, ordsel_len)
...
ftnlen ordsel_len;
{
    ...
    /* Local variables */
    static integer ierr;
    extern /* Subroutine */ EXPORTSYMBOL int tb01id_(), ab09bx_();
    extern logical lsame_();
    extern /* Subroutine */ EXPORTSYMBOL int tb01wd_();
    static integer ki, nn, kr, kt, kw;
    ...
    *iwarn = 0;
    ...
    i__1 = *ldwork - kw + 1;
    tb01wd_(n, m, p, &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset], ldc, &dwork[kt], n, &dwork[kr], &dwork[ki], &dwork[kw], &i__1, &ierr);
    if (ierr != 0)
    {
        *info = 1;
        return 0;
    ...
    kw = kti + nn;
    i__1 = *ldwork - kw + 1;
    ab09bx_(dico, job, ordsel, n, m, p, nr, &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset], ldc, &d__[d_offset], ldd, &hsv[1], &dwork[kt], n, &dwork[kti], n, tol1, tol2, &iwork[1], &dwork[kw], &i__1, iwarn, &ierr, 1L, 1L, 1L);
    if (ierr != 0)
    {
        *info = ierr + 1;
        return 0;

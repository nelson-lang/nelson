...
/* Subroutine */ EXPORTSYMBOL int mb03id_(compq, compu, n, a, lda, c__, ldc, d__, ldd, b, ldb, f, ldf, q, ldq, u1, ldu1, u2, ldu2, neig, iwork, liwork, dwork, ldwork, info, compq_len, compu_len)
char *compq, *compu;
integer *n;
doublereal *a;
integer *lda;
...
ftnlen compu_len;
{
    ...
    integer a_dim1, a_offset, b_dim1, b_offset, c_dim1, c_offset, d_dim1, d_offset, f_dim1, f_offset, q_dim1, q_offset, u1_dim1, u1_offset, u2_dim1, u2_offset, i__1, i__2, i__3, i__4, i__5;
    doublereal d__1;
    /* Builtin functions */
    double log(), d_sign();
    /* Local variables */
    ...
    extern /* Subroutine */ EXPORTSYMBOL int mb03bb_();
    static integer m;
    extern /* Subroutine */ EXPORTSYMBOL int mb03cd_();
    static integer r__;
    extern /* Subroutine */ EXPORTSYMBOL int mb03gd_(), dscal_();
    ...
    static integer iqlole, ihuple, ib1, ib2, ib3, iqlori, iquple, iuuple, izlori, iqupri, izuple, iq1, iq2, iq3, iuupri, izupri, ibs;
    ...
    if (i__ <= m - 1)
    {
        ...
        else
        {
            ...
            if (abs(tmpa) <= prec * abs(u11) && abs(tmpc) <= prec * abs(u12))
            {
                ...
                prd[5] = 0.;
                prd[9] = 0.;
                mb03bb_(&base, &lgbas, &prec, &c__3, idum, &idum[3], &c__1, prd, &c__2, &c__2, &dwork[1], &dwork[3], &dwork[5], &idum[6], &dwork[7], info);
                if (*info != 0)
                {
                    return 0;
                }
    ...
    if (k <= r__)
    {
        if (iwork[is + k] < 0)
        {
            ...
            for (j = k - 1; j >= i__1; --j)
            {
                ...
                i__2 = *ldwork - iwrk1 + 1;
                mb03cd_("Upper", &dim1, &dim2, &prec, &dwork[ic], &sdim, &dwork[ia], &sdim, &dwork[ib], &sdim, &dwork[iq1], &sdim, &dwork[iq2], &sdim, &dwork[iq3], &sdim, &dwork[iwrk1], &i__2, info, 5L);
                if (*info > 0)
                {
                    *info = 2;
                    return 0;
                ...
                i__2 = sdim - 1;
                i__3 = sdim - 1;
                dlacpy_("Upper", &i__2, &i__3, &c__[ib1 + (ib1 + 1) * c_dim1], ldc, &dum[3], &c__3, 5L);
                i__2 = sdim - 1;
                i__3 = sdim - 1;
                dlaset_("Upper", &i__2, &i__3, &c_b35, &c_b35, &c__[ib1 + (ib1 + 1) * c_dim1], ldc, 5L);
                /*              Update A. */
                /*              Workspace: IWRK2 + 2*N - 1. */
                dgemm_("No Transpose", "No Transpose", &nrow, &sdim, &sdim, &c_b14, &a[ib1 * a_dim1 + 1], lda, &dwork[iq1], &sdim, &c_b35, &dwork[iwrk2], &nrow, 12L, 12L);
                dlacpy_("Full", &nrow, &sdim, &dwork[iwrk2], &nrow, &a[ib1 * a_dim1 + 1], lda, 4L);
                dgemm_("Transpose", "No Transpose", &sdim, &ncol, &sdim, &c_b14, &dwork[iq2], &sdim, &a[ib1 + ib1 * a_dim1], lda, &c_b35, &dwork[iwrk2], &sdim, 9L, 12L);
                ...
                /*              Update C. */
                dgemm_("No Transpose", "No Transpose", &ncol, &sdim, &sdim, &c_b14, &c__[ib1 + ib1 * c_dim1], ldc, &dwork[iq3], &sdim, &c_b35, &dwork[iwrk2], &ncol, 12L, 12L);
                dlacpy_("Full", &ncol, &sdim, &dwork[iwrk2], &ncol, &c__[ib1 + ib1 * c_dim1], ldc, 4L);
                dgemm_("Transpose", "No Transpose", &sdim, &nrow, &sdim, &c_b14, &dwork[iq2], &sdim, &c__[ib1 + c_dim1], ldc, &c_b35, &dwork[iwrk2], &sdim, 9L, 12L);
                dlacpy_("Full", &sdim, &nrow, &dwork[iwrk2], &sdim, &c__[ib1 + c_dim1], ldc, 4L);
                i__2 = sdim - 1;
                i__3 = sdim - 1;
                dlacpy_("Upper", &i__2, &i__3, &dum[3], &c__3, &c__[ib1 + (ib1 + 1) * c_dim1], ldc, 5L);
                /*              Update D. */
                dgemm_("No Transpose", "No Transpose", &m, &sdim, &sdim, &c_b14, &d__[ib1 * d_dim1 + 1], ldd, &dwork[iq3], &sdim, &c_b35, &dwork[iwrk2], &m, 12L, 12L);
                dlacpy_("Full", &m, &sdim, &dwork[iwrk2], &m, &d__[ib1 * d_dim1 + 1], ldd, 4L);
                dgemm_("Transpose", "No Transpose", &sdim, &m, &sdim, &c_b14, &dwork[iq2], &sdim, &d__[ib1 + d_dim1], ldd, &c_b35, &dwork[iwrk2], &sdim, 9L, 12L);
                ...
                dgemm_("No Transpose", "No Transpose", &nrow, &sdim, &sdim, &c_b14, &b[ib1 * b_dim1 + 1], ldb, &dwork[iq1], &sdim, &c_b35, &dwork[iwrk2], &ldw, 12L, 12L);
                dlacpy_("Full", &nrow, &sdim, &dwork[iwrk2], &ldw, &b[ib1 * b_dim1 + 1], ldb, 4L);
                dgemm_("Transpose", "No Transpose", &sdim, &ncol, &sdim, &c_b14, &dwork[iq3], &sdim, &b[ib1 + ibs * b_dim1], ldb, &c_b35, &dwork[iwrk2], &sdim, 9L, 12L);
                dlacpy_("Full", &sdim, &ncol, &dwork[iwrk2], &sdim, &b[ib1 + ibs * b_dim1], ldb, 4L);
                /*              Update F. */
                dgemm_("No Transpose", "No Transpose", &nrows, &sdim, &sdim, &c_b14, &f[ib1 * f_dim1 + 1], ldf, &dwork[iq3], &sdim, &c_b35, &dwork[iwrk2], &ldw, 12L, 12L);
                dlacpy_("Full", &nrows, &sdim, &dwork[iwrk2], &ldw, &f[ib1 * f_dim1 + 1], ldf, 4L);
                dgemm_("Transpose", "No Transpose", &sdim, &ncols, &sdim, &c_b14, &dwork[iq3], &sdim, &f[ib1 + ib3 * f_dim1], ldf, &c_b35, &dwork[iwrk2], &sdim, 9L, 12L);
                dlacpy_("Full", &sdim, &ncols, &dwork[iwrk2], &sdim, &f[ib1 + ib3 * f_dim1], ldf, 4L);
                i__2 = *ldwork - iwrk2 + 1;
                mb01ru_("Upper", "Transpose", &sdim, &sdim, &c_b35, &c_b14, &f[ib1 + ib1 * f_dim1], ldf, &dwork[iq3], &sdim, &f[ib1 + ib1 * f_dim1], ldf, &dwork[iwrk2], &i__2, info, 5L, 9L);
                i__2 = *ldf + 1;
                ...
                if (lcmpq)
                {
                    ...
                    dgemm_("No Transpose", "No Transpose", &upds, &sdim, &sdim, &c_b14, &q[ib1 * q_dim1 + 1], ldq, &dwork[iq1], &sdim, &c_b35, &dwork[iwrk2], &upds, 12L, 12L);
                    dlacpy_("Full", &upds, &sdim, &dwork[iwrk2], &upds, &q[ib1 * q_dim1 + 1], ldq, 4L);
                    dgemm_("No Transpose", "No Transpose", &upds, &sdim, &sdim, &c_b14, &q[iupd + (m + ib1) * q_dim1], ldq, &dwork[iq3], &sdim, &c_b35, &dwork[iwrk2], &upds, 12L, 12L);
                    dlacpy_("Full", &upds, &sdim, &dwork[iwrk2], &upds, &q[iupd + (m + ib1) * q_dim1], ldq, 4L);
                }
                if (lcmpu)
                {
                    /*                 Update U1. */
                    dgemm_("No Transpose", "No Transpose", &m, &sdim, &sdim, &c_b14, &u1[ib1 * u1_dim1 + 1], ldu1, &dwork[iq2], &sdim, &c_b35, &dwork[iwrk2], &m, 12L, 12L);
                    dlacpy_("Full", &m, &sdim, &dwork[iwrk2], &m, &u1[ib1 * u1_dim1 + 1], ldu1, 4L);
                }
                if (lupdu)
                {
                    /*                 Update U2. */
                    dgemm_("No Transpose", "No Transpose", &m, &sdim, &sdim, &c_b14, &u2[ib1 * u2_dim1 + 1], ldu2, &dwork[iq2], &sdim, &c_b35, &dwork[iwrk2], &m, 12L, 12L);
...
L40:
    ...
    for (k = r__; k >= i__1; --k)
    {
        ...
        dlacpy_("Upper", &dim1, &dim1, &f[ir + ir * f_dim1], ldb, &dwork[ihuple + dim1 * sdim], &sdim, 5L);
        if (dim1 == 2)
        {
            dwork[izuple + 1] = 0.;
            dwork[izlori + sdim] = 0.;
        ...
        i__2 = *ldwork - iwrk5 + 1;
        mb03gd_(&sdim, &dwork[izuple], &sdim, &dwork[ihuple], &sdim, par, &dwork[iquple], &sdim, &dwork[iuuple], &sdim, &dwork[iwrk5], &i__2, info);
        if (*info > 0)
        {
            *info = 3;
            return 0;
        ...
        if (dim1 == 2)
        {
            ...
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &dwork[iwrk3], &m, &dwork[iquple], &sdim, &c_b35, &a[ir * a_dim1 + 1], lda, 12L, 12L);
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &d__[ir * d_dim1 + 1], ldd, &dwork[iqlole], &sdim, &c_b14, &a[ir * a_dim1 + 1], lda, 12L, 12L);
            /*           Update D by transformations from the right. */
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &dwork[iwrk3], &m, &dwork[iqupri], &sdim, &c_b35, &dwork[itmp1], &m, 12L, 12L);
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &d__[ir * d_dim1 + 1], ldd, &dwork[iqlori], &sdim, &c_b14, &dwork[itmp1], &m, 12L, 12L);
            dlacpy_("Full", &m, &dim1, &dwork[itmp1], &m, &d__[ir * d_dim1 + 1], ldd, 4L);
            /*           Compute intermediate product Cf*Q21, with */
            /*           Cf = C(M-1:M,M-1:M). */
            dgemm_("No Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &c__[ir + ir * c_dim1], ldc, &dwork[iqlole], &sdim, &c_b35, &dwork[itmp1], &dim1, 12L, 12L);
            /*           Update C by transformations from the right. */
            dgemm_("No Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &c__[ir + ir * c_dim1], ldc, &dwork[iqlori], &sdim, &c_b35, &dwork[iwrk3], &dim1, 12L, 12L);
            dlacpy_("Full", &dim1, &dim1, &dwork[iwrk3], &dim1, &c__[ir + ir * c_dim1], ldc, 4L);
            /*           Update A by transformations from the left. */
            dgemm_("Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &dwork[iuuple], &sdim, &a[ir + ir * a_dim1], lda, &c_b35, &dwork[iwrk3], &dim1, 9L, 12L);
            dgemm_("Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b274, &dwork[iuupri], &sdim, &dwork[itmp1], &dim1, &c_b14, &dwork[iwrk3], &dim1, 9L, 12L);
            ...
            dgemm_("Transpose", "No Transpose", &dim1, &m, &dim1, &c_b14, &dwork[iuuple], &sdim, &dwork[iwrk3], &dim1, &c_b35, &d__[ir + d_dim1], ldd, 9L, 12L);
            dgemm_("Transpose", "No Transpose", &dim1, &m, &dim1, &c_b274, &dwork[iuupri], &sdim, &c__[ir + c_dim1], ldc, &c_b14, &d__[ir + d_dim1], ldd, 9L, 12L);
            /*           Update C by transformations from the left. */
            dgemm_("Transpose", "No Transpose", &dim1, &m, &dim1, &c_b14, &dwork[iuupri], &sdim, &dwork[iwrk3], &dim1, &c_b35, &dwork[itmp1], &dim1, 9L, 12L);
            dgemm_("Transpose", "No Transpose", &dim1, &m, &dim1, &c_b14, &dwork[iuuple], &sdim, &c__[ir + c_dim1], ldc, &c_b14, &dwork[itmp1], &dim1, 9L, 12L);
            ...
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &dwork[iwrk3], &m, &dwork[iquple], &sdim, &c_b35, &b[ir * b_dim1 + 1], ldb, 12L, 12L);
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &f[ir * f_dim1 + 1], ldf, &dwork[iqlole], &sdim, &c_b14, &b[ir * b_dim1 + 1], ldb, 12L, 12L);
            /*           Update F by transformations from the right. */
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &dwork[iwrk3], &m, &dwork[iqupri], &sdim, &c_b35, &dwork[itmp1], &m, 12L, 12L);
            dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &f[ir * f_dim1 + 1], ldf, &dwork[iqlori], &sdim, &c_b14, &dwork[itmp1], &m, 12L, 12L);
            dlacpy_("Full", &m, &dim1, &dwork[itmp1], &m, &f[ir * f_dim1 + 1], ldf, 4L);
            /*           Compute intermediate products Bf'*Q21 and Bf'*Q22, with */
            /*           Bf = B(M-1:M,M-1:M). */
            dgemm_("Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &dwork[iwrk3 + m - dim1], &m, &dwork[iqlole], &sdim, &c_b35, &dwork[itmp1], &dim1, 9L, 12L);
            dgemm_("Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &dwork[iwrk3 + m - dim1], &m, &dwork[iqlori], &sdim, &c_b35, &dwork[itmp2], &dim1, 9L, 12L);
            /*           Update B by transformations from the left. */
            dlacpy_("Full", &dim1, &dim1, &b[ir + ir * b_dim1], ldb, &dwork[itmp3], &dim1, 4L);
            dgemm_("Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &dwork[iqupri], &sdim, &dwork[itmp1], &dim1, &c_b35, &b[ir + ir * b_dim1], ldb, 9L, 12L);
            dgemm_("Transpose", "No Transpose", &dim1, &dim1, &dim1, &c_b14, &dwork[iqlori], &sdim, &dwork[itmp3], &dim1, &c_b14, &b[ir + ir * b_dim1], ldb, 9L, 12L);
            /*           Update F by transformations from the left. */
            mb01rx_("Left", "Upper", "Transpose", &dim1, &dim1, &c_b35, &c_b14, &dwork[itmp1], &dim1, &dwork[iqlori], &sdim, &f[ir + ir * f_dim1], ldf, info, 4L, 5L, 9L);
            mb01rx_("Left", "Upper", "Transpose", &dim1, &dim1, &c_b14, &c_b14, &dwork[itmp1], &dim1, &dwork[iqupri], &sdim, &dwork[itmp2], &dim1, info, 4L, 5L, 9L);
            ...
            if (lcmpq)
            {
                ...
                dgemm_("No Transpose", "No Transpose", n, &dim1, &dim1, &c_b14, &dwork[iwrk4], n, &dwork[iquple], &sdim, &c_b35, &q[ir * q_dim1 + 1], ldq, 12L, 12L);
                dgemm_("No Transpose", "No Transpose", n, &dim1, &dim1, &c_b14, &q[(m + ir) * q_dim1 + 1], ldq, &dwork[iqlole], &sdim, &c_b14, &q[ir * q_dim1 + 1], ldq, 12L, 12L);
                dgemm_("No Transpose", "No Transpose", n, &dim1, &dim1, &c_b14, &dwork[iwrk4], n, &dwork[iqupri], &sdim, &c_b35, &dwork[iwrk3], n, 12L, 12L);
                dgemm_("No Transpose", "No Transpose", n, &dim1, &dim1, &c_b14, &q[(m + ir) * q_dim1 + 1], ldq, &dwork[iqlori], &sdim, &c_b14, &dwork[iwrk3], n, 12L, 12L);
            ...
            if (lcmpu)
            {
                ...
                dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &dwork[itmp1], &m, &dwork[iuuple], &sdim, &c_b35, &u1[ir * u1_dim1 + 1], ldu1, 12L, 12L);
                dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b274, &u2[ir * u2_dim1 + 1], ldu2, &dwork[iuupri], &sdim, &c_b14, &u1[ir * u1_dim1 + 1], ldu1, 12L, 12L);
                dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &dwork[itmp1], &m, &dwork[iuupri], &sdim, &c_b35, &dwork[iwrk3], &m, 12L, 12L);
                dgemm_("No Transpose", "No Transpose", &m, &dim1, &dim1, &c_b14, &u2[ir * u2_dim1 + 1], ldu2, &dwork[iuuple], &sdim, &c_b14, &dwork[iwrk3], &m, 12L, 12L);
        ...
        for (j = r__ - 1; j >= i__2; --j)
        {
            ...
            /*           Perform eigenvalue/matrix block exchange. */
            i__3 = *ldwork - iwrk1 + 1;
            mb03cd_("Upper", &dim1, &dim2, &prec, &dwork[ic], &sdim, &dwork[ia], &sdim, &dwork[ib], &sdim, &dwork[iq1], &sdim, &dwork[iq2], &sdim, &dwork[iq3], &sdim, &dwork[iwrk1], &i__3, info, 5L);
            if (*info > 0)
            {
                *info = 2;
                return 0;
            ...
            dlacpy_("Full", &sdim, &ncols, &dwork[iwrk2], &sdim, &f[ib1 + ib3 * f_dim1], ldf, 4L);
            i__3 = *ldwork - iwrk2 + 1;
            mb01ru_("Upper", "Transpose", &sdim, &sdim, &c_b35, &c_b14, &f[ib1 + ib1 * f_dim1], ldf, &dwork[iq3], &sdim, &f[ib1 + ib1 * f_dim1], ldf, &dwork[iwrk2], &i__3, info, 5L, 9L);
            i__3 = *ldf + 1;
            dscal_(&sdim, &c_b118, &f[ib1 + ib1 * f_dim1], &i__3);
            if (lcmpq)
            {
                /*              Update Q. */
                dgemm_("No Transpose", "No Transpose", n, &sdim, &sdim, &c_b14, &q[ib1 * q_dim1 + 1], ldq, &dwork[iq1], &sdim, &c_b35, &dwork[iwrk2], n, 12L, 12L);
                dlacpy_("Full", n, &sdim, &dwork[iwrk2], n, &q[ib1 * q_dim1 + 1], ldq, 4L);
                dgemm_("No Transpose", "No Transpose", n, &sdim, &sdim, &c_b14, &q[(m + ib1) * q_dim1 + 1], ldq, &dwork[iq3], &sdim, &c_b35, &dwork[iwrk2], n, 12L, 12L);

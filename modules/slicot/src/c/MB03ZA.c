...
ftnlen which_len;
{
    ...
    --dwork;
    /* Function Body */
    wantc = lsame_(compc, "U", 1L, 1L);
    wantu = lsame_(compu, "U", 1L, 1L);
    wantv = lsame_(compv, "U", 1L, 1L);
    initw = lsame_(compw, "I", 1L, 1L);
    wantw = initw || lsame_(compw, "V", 1L, 1L);
...
L50:
    ...
    for (k = 1; k <= i__1; ++k) {
        if (pair) {
            ...
            pwdl = pwd + (k - 1 << 1);
            i__2 = *m - k + 1;
            dlaset_("All", &nb, &i__2, &c_b20, &c_b20, &dwork[pwck], &c__2, 3L);
            i__2 = *m - k + 1;
            dlacpy_("All", &nb, &i__2, &a[k + k * a_dim1], lda, &dwork[pwdl], &c__2, 3L);
            i__2 = *m - k + 1;
        ...
        L60:
            ...
            if (k != l) {
                ...
                i__2 = nb + nbl;
                dlacpy_("All", &nb, &i__2, &q[nbl], &c__4, z__, &c__4, 3L);
                i__2 = nb + nbl;
                i__3 = nb + nbl;
                dlacpy_("All", &i__2, &i__3, z__, &c__4, q, &c__4, 3L);
            }

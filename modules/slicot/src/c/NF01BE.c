...
EXPORTSYMBOL /* Subroutine */ int nf01be_(iflag, nsmp, n, ipar, lipar, z__, ldz, y, ldy, x, nfevl, e, j, ldj, dwork, ldwork, info)
...
integer *ldwork, *info;
{
    ...
    *info = 0;
    ...
    else if (*iflag == 2)
    {
        ...
        /*        Workspace: need:    0. */
        i__1 = *lipar - 2;
        nf01by_("N", nsmp, &ipar[2], &c__1, &ipar[3], &i__1, &x[1], n, &z__[z_offset], ldz, &e[1], &j[j_offset], ldj, &dwork[1], &dwork[1], ldwork, info, 1L);
        *nfevl = 0;
        dwork[1] = 0.;
    }
    else if (*iflag == 3)

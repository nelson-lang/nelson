...
doublereal ab13dx_(dico, jobe, jobd, n, m, p, omega, a, lda, e, lde, b, ldb, c__, ldc, d__, ldd, iwork, dwork, ldwork, cwork, lcwork, info, dico_len, jobe_len, jobd_len)
char *dico, *jobe, *jobd;
integer *n, *m, *p;
doublereal *omega, *a;
integer *lda;
...
ftnlen jobd_len;
{
    ...
    integer a_dim1, a_offset, b_dim1, b_offset, c_dim1, c_offset, d_dim1, d_offset, e_dim1, e_offset, i__1, i__2, i__3;
    ...
    extern /* Subroutine */ EXPORTSYMBOL int mb02rd_(), mb02sd_(), dgemm_();
    extern logical lsame_();
    static logical discr, specl, fulle;
    extern /* Subroutine */ EXPORTSYMBOL int mb02rz_();
    static doublereal bnorm, cnorm;
    static logical withd;
    static integer minpm;
    extern /* Subroutine */ EXPORTSYMBOL int mb02sz_(), zgemm_();
    static logical nodyn;
    extern /* Subroutine */ EXPORTSYMBOL int zlacp2_();
    static integer id;
    static doublereal lambdi;
    extern doublereal dlange_();
    static integer is;
    static doublereal lambdr;
    extern /* Subroutine */ EXPORTSYMBOL int dgesvd_(), xerbla_();
    ...
    *info = 0;
    ...
    if (nodyn)
    {
        ...
        if (withd)
        {
            ...
            /*                      prefer larger. */
            i__1 = *ldwork - iwrk + 1;
            dgesvd_("No Vectors", "No Vectors", p, m, &d__[d_offset], ldd, &dwork[is], &dwork[1], p, &dwork[1], m, &dwork[iwrk], &i__1, &ierr, 10L, 10L);
            if (ierr > 0)
            {
                *info = *n + 1;
                return ret_val;
    ...
    if (specl)
    {
        ...
        if (withd)
        {
            dgemm_("No Transpose", "No Transpose", p, m, n, &c_b17, &c__[c_offset], ldc, &b[b_offset], ldb, &c_b18, &d__[d_offset], ldd, 12L, 12L);
            i__1 = *ldwork - iwrk + 1;
            dgesvd_("No Vectors", "No Vectors", p, m, &d__[d_offset], ldd, &dwork[is], &dwork[1], p, &dwork[1], m, &dwork[iwrk], &i__1, &ierr, 10L, 10L);
        }
        else
        {
            ...
            id = iwrk;
            iwrk = id + *p **m;
            dgemm_("No Transpose", "No Transpose", p, m, n, &c_b17, &c__[c_offset], ldc, &b[b_offset], ldb, &c_b24, &dwork[id], p, 12L, 12L);
            i__1 = *ldwork - iwrk + 1;
            dgesvd_("No Vectors", "No Vectors", p, m, &dwork[id], p, &dwork[is], &dwork[1], p, &dwork[1], m, &dwork[iwrk], &i__1, &ierr, 10L, 10L);
        }
        if (ierr > 0)
        {
            *info = *n + 1;
    ...
    z__1.r = upd, z__1.i = 0.;
    zgemm_("No Transpose", "No Transpose", p, m, n, &c_b1, &cwork[icc], p, &cwork[icb], n, &z__1, &cwork[icd], p, 12L, 12L);
    /*     Additional workspace, complex: need   2*MIN(P,M) + MAX(P,M); */
    /*                                    prefer larger; */
    /*                           real:    need   5*MIN(P,M). */
    i__1 = *lcwork - icwk + 1;
    zgesvd_("No Vectors", "No Vectors", p, m, &cwork[icd], p, &dwork[is], &cwork[1], p, &cwork[1], m, &cwork[icwk], &i__1, &dwork[iwrk], &ierr, 10L, 10L);
    if (ierr > 0)
    {
        *info = *n + 1;
        return ret_val;

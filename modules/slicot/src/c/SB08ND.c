...
/* Subroutine */ EXPORTSYMBOL int sb08nd_(acona, da, a, res, e, dwork, ldwork, info, acona_len)
char *acona;
integer *da;
...
ftnlen acona_len;
{
    ...
    extern /* Subroutine */ EXPORTSYMBOL int sb08ny_(), daxpy_();
    static integer lambda, nc;
    static logical lacona;
    static integer lq, lalpha;
...
L60:
        if (k <= *da && hurwtz)
        {
            ...
            if (hurwtz)
            {
                ...
                i__1 = nck - 1;
                d__1 = -w;
                daxpy_(&i__1, &d__1, &dwork[lalpha + 1], &c_n1, &dwork[leta + 1], &c__1);
                ++k;
            }
            goto L60;
        }
            ...
            *res = (d__1 = dwork[idamax_(&nc, &dwork[lq], &c__1) + lq - 1], abs(d__1));
            ...
            if (! conv)
            {
                ...
                for (k = *da; k >= 1; --k)
                {
                    ...
                    w = dwork[lambda + k - 1];
                    d__1 = -w;
                    daxpy_(&nck, &d__1, &dwork[lro], &c__1, &dwork[leta], &c__1);
                    /* L80: */
                }
                s = 0.;
                i__1 = *da;

...
EXPORTSYMBOL /* Subroutine */ int tb01iz_(job, n, m, p, maxred, a, lda, b, ldb, c__, ldc, scale, info, job_len)
char *job;
integer *n, *m, *p;
doublereal *maxred;
doublecomplex *a;
...
L30:
    ...
    for (i__ = 1; i__ <= i__1; ++i__)
    {
        ...
        for (j = 1; j <= i__2; ++j)
        {
            ...
            i__3 = j + i__ * a_dim1;
            co += (d__1 = a[i__3].r, abs(d__1)) + (d__2 = d_imag(&a[j + i__ * a_dim1]), abs(d__2));
            i__3 = i__ + j * a_dim1;
            ro += (d__1 = a[i__3].r, abs(d__1)) + (d__2 = d_imag(&a[i__ + j * a_dim1]), abs(d__2));
L40:
            ;
        }

...
EXPORTSYMBOL /* Subroutine */ int sb10md_(nc, mp, lendat, f, ord, mnb, nblock, itype, qutol, a, lda, b, ldb, c__, ldc, d__, ldd, omega, totord, ad, ldad, bd, ldbd, cd, ldcd, dd, lddd, mju, iwork, liwork, dwork, ldwork, zwork, lzwork, info)
...
integer *lzwork, *info;
{
    ...
    integer a_dim1, a_offset, ad_dim1, ad_offset, b_dim1, b_offset, bd_dim1, bd_offset, c_dim1, c_offset, cd_dim1, cd_offset, d_dim1, d_offset, dd_dim1, dd_offset, i__1, i__2, i__3, i__4, i__5, i__6;
    doublereal d__1;
...
/* Computing MAX */
	i__3 = *nc, i__4 = *mp - 1;
	i__1 = *nc + max(i__3,i__4), i__2 = (*mp << 1) * *mp * *mnb - *mp * *mp + *mnb * 9 * *mnb + *mp * *mnb + *mp * 11 + *mnb * 33 - 11;
	dlwmax = lwa + max(i__1,i__2);

/* Computing MAX */
	i__1 = icwrk - 1 + *nc * *nc + (*nc << 1), i__2 = *mp * 6 * *mp * *mnb + *mp * 13 * *mp + *mnb * 6 + *mp * 6 - 3;
...
/* Computing MAX */
/* Computing MAX */
	    i__3 = max(2,lw1), i__3 = max(i__3,lw2), i__3 = max(i__3,lw3), i__3 = max(i__3,lw4), i__4 = *ord << 1;
	    i__1 = dlwmax, i__2 = lwb + max(i__3,i__4);
	    dlwmax = max(i__1,i__2);
/* Computing MAX */
	    i__1 = clwmax, i__2 = *lendat * ((*ord << 1) + 3), i__1 = max(i__1,i__2), i__2 = *ord * (*ord + 3) + 1;
	    clwmax = max(i__1,i__2);
	}
	if (*liwork < ii) {
	    *info = -30;
...
/*        Complex workspace: need   MP*MP + NC*MP + NC*NC + 2*NC. */
	...
	tb05ad_(baleig, inita, nc, mp, mp, &freq, &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset], ldc, &rcnd, &zwork[1], mp, &dwork[1], &dwork[1], &zwork[iwb], nc, &iwork[1], &dwork[idwrk], &ldsize, &zwork[icwrk], &lcsize, &info2, 1L, 1L);
...
/*                                  6*MP - 3. */
	...
	i__2 = *lzwork - iwb + 1;
	ab13md_("N", mp, &zwork[1], mp, mnb, &nblock[1], &itype[1], &dwork[iwx], &mju[w], &dwork[(w - 1) * *mp + 1], &dwork[iwgjom], &iwork[1], &dwork[idwrk], &ldsize, &zwork[iwb], &i__2, &info2, 1L);
...
/*           Complex workspace: need   LENDAT*(2*N+3). */
	...
	sb10yd_(&c__0, &c__1, lendat, &dwork[iwrfrd], &dwork[iwifrd], &omega[1], &lord, &dwork[iwad], ord, &dwork[iwbd], &dwork[iwcd], &dwork[iwdd], &tol, &iwork[1], &dwork[idwrk], &ldsize, &zwork[1], lzwork, &info2);
...
/*              Complex workspace: need   1 + ORD + LORD*LORD + 2*LORD. */
	    ...
	    tb05ad_(baleig, inita, &lord, &c__1, &c__1, &freq, &dwork[iwad], ord, &dwork[iwbd], ord, &dwork[iwcd], &c__1, &rcnd, &zwork[1], &c__1, &dwork[idwrk], &dwork[idwrk], &zwork[2], ord, &iwork[1], &dwork[idwrk], &ldsize, &zwork[icwrk], &lcsize, &info2, 1L, 1L);
...
/*        Copy ad(ii), bd(ii) and cd(ii) to AD, BD and CD, respectively. */
	...
	dlacpy_("Full", &lord, &lord, &dwork[iwad], ord, &ad[*totord - lord + 1 + (*totord - lord + 1) * ad_dim1], ldad, 4L);

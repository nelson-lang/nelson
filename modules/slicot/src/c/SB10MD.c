...
EXPORTSYMBOL /* Subroutine */ int sb10md_(nc, mp, lendat, f, ord, mnb, nblock, itype, qutol, a, lda,
    b, ldb, c__, ldc, d__, ldd, omega, totord, ad, ldad, bd, ldbd, cd, ldcd, dd, lddd, mju, iwork,
    liwork, dwork, ldwork, zwork, lzwork, info) integer *nc,
    *mp, *lendat, *f, *ord, *mnb, *nblock, *itype;
doublereal *qutol, *a;
integer* lda;
doublereal* b;
integer* ldb;
...
integer *lzwork, *info;
{
    ...
    integer a_dim1, a_offset, ad_dim1, ad_offset, b_dim1, b_offset, bd_dim1, bd_offset, c_dim1,
        c_offset, cd_dim1, cd_offset, d_dim1, d_offset, dd_dim1, dd_offset, i__1, i__2, i__3, i__4,
        i__5, i__6;
    doublereal d__1;
    ...
    static integer iwifrd, lcsize, ldsize, clwmax, dlwmax, iwgjom, iwrfrd, maxcwr, maxwrk, lw1, lw2,
        lw3, lw4, iwb, lwa, lwb;
    static doublereal rqe, tol;
    static integer iwx;
    static doublereal mod1, mod2;
    /*     SLICOT RELEASE 5.0. */
    ...
    *info = 0;
    if (*nc < 0) {
        ...
        *info = -27;
        ...
        lw3 = (*lendat << 1) * ((*ord << 1) + 1) + max(i__1, i__2) + max(i__3, i__4);
        /* Computing MAX */
        ...
        /* Computing MAX */
        i__3 = *nc, i__4 = *mp - 1;
        i__1 = *nc + max(i__3, i__4),
        i__2 = (*mp << 1) * *mp * *mnb - *mp * *mp + *mnb * 9 * *mnb + *mp * *mnb + *mp * 11
            + *mnb * 33 - 11;
        dlwmax = lwa + max(i__1, i__2);
        /* Computing MAX */
        i__1 = icwrk - 1 + *nc * *nc + (*nc << 1),
        i__2 = *mp * 6 * *mp * *mnb + *mp * 13 * *mp + *mnb * 6 + *mp * 6 - 3;
        ...
        if (*qutol >= 0.) {
            ...
            /* Computing MAX */
            /* Computing MAX */
            i__3 = max(2, lw1), i__3 = max(i__3, lw2), i__3 = max(i__3, lw3), i__3 = max(i__3, lw4),
            i__4 = *ord << 1;
            i__1 = dlwmax, i__2 = lwb + max(i__3, i__4);
            dlwmax = max(i__1, i__2);
            /* Computing MAX */
            i__1 = clwmax, i__2 = *lendat * ((*ord << 1) + 3), i__1 = max(i__1, i__2),
            i__2 = *ord * (*ord + 3) + 1;
            clwmax = max(i__1, i__2);
        }
        if (*liwork < ii) {
    ...
    *(unsigned char*)baleig = 'C';
    ...
    for (w = 1; w <= i__1; ++w) {
        ...
        tb05ad_(baleig, inita, nc, mp, mp, &freq, &a[a_offset], lda, &b[b_offset], ldb,
            &c__[c_offset], ldc, &rcnd, &zwork[1], mp, &dwork[1], &dwork[1], &zwork[iwb], nc,
            &iwork[1], &dwork[idwrk], &ldsize, &zwork[icwrk], &lcsize, &info2, 1L, 1L);
        if (info2 > 0) {
            *info = 1;
            return 0;
        }
        ...
        for (k = 1; k <= i__2; ++k) {
            ...
            for (i__ = 1; i__ <= i__3; ++i__) {
                ...
                z__1.r = zwork[i__5].r + z__2.r, z__1.i = zwork[i__5].i + z__2.i;
        ...
        /*                                  6*MP - 3. */
        i__2 = *lzwork - iwb + 1;
        ab13md_("N", mp, &zwork[1], mp, mnb, &nblock[1], &itype[1], &dwork[iwx], &mju[w],
            &dwork[(w - 1) * *mp + 1], &dwork[iwgjom], &iwork[1], &dwork[idwrk], &ldsize,
            &zwork[iwb], &i__2, &info2, 1L);
        if (info2 != 0) {
            *info = info2 + 1;
            return 0;
        }
    ...
    *(unsigned char*)inita = 'H';
    ...
    i__1 = *mp * *ord;
    i__2 = *mp * *ord;
    dlaset_("Full", &i__1, &i__2, &c_b15, &c_b15, &ad[ad_offset], ldad, 4L);
    i__1 = *mp * *ord;
    i__2 = *mp + *f;
    dlaset_("Full", &i__1, &i__2, &c_b15, &c_b15, &bd[bd_offset], ldbd, 4L);
    i__1 = *mp + *f;
    i__2 = *mp * *ord;
    dlaset_("Full", &i__1, &i__2, &c_b15, &c_b15, &cd[cd_offset], ldcd, 4L);
    i__1 = *mp + *f;
    i__2 = *mp + *f;
    dlaset_("Full", &i__1, &i__2, &c_b15, &c_b25, &dd[dd_offset], lddd, 4L);
    ...
    L50:
        ...
        sb10yd_(&c__0, &c__1, lendat, &dwork[iwrfrd], &dwork[iwifrd], &omega[1], &lord,
            &dwork[iwad], ord, &dwork[iwbd], &dwork[iwcd], &dwork[iwdd], &tol, &iwork[1],
            &dwork[idwrk], &ldsize, &zwork[1], lzwork, &info2);
        ...
        for (w = 1; w <= i__2; ++w) {
            ...
            tb05ad_(baleig, inita, &lord, &c__1, &c__1, &freq, &dwork[iwad], ord, &dwork[iwbd], ord,
                &dwork[iwcd], &c__1, &rcnd, &zwork[1], &c__1, &dwork[idwrk], &dwork[idwrk],
                &zwork[2], ord, &iwork[1], &dwork[idwrk], &ldsize, &zwork[icwrk], &lcsize, &info2,
                1L, 1L);
            if (info2 > 0) {
                *info = 1;
                return 0;
    ...
    L70:
        *totord += lord;
        ...
        dlacpy_("Full", &lord, &lord, &dwork[iwad], ord,
            &ad[*totord - lord + 1 + (*totord - lord + 1) * ad_dim1], ldad, 4L);
        dcopy_(&lord, &dwork[iwbd], &c__1, &bd[*totord - lord + 1 + ii * bd_dim1], &c__1);
        dcopy_(&lord, &dwork[iwcd], &c__1, &cd[ii + (*totord - lord + 1) * cd_dim1], ldcd);
        /*        Copy dd(ii) to DD. */

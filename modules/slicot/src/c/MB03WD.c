...
/* Subroutine */ EXPORTSYMBOL int mb03wd_(job, compz, n, p, ilo, ihi, iloz, ihiz, h__, ldh1, ldh2, z__, ldz1, ldz2, wr, wi, dwork, ldwork, info, job_len, compz_len)
char *job, *compz;
...
ftnlen compz_len;
{
    ...
    integer h_dim1, h_dim2, h_offset, z_dim1, z_dim2, z_offset, i__1, i__2, i__3, i__4;
    doublereal d__1, d__2;
    /* Builtin functions */
    double sqrt(), d_sign();
    /* Local variables */
    static doublereal h43h34, disc;
    static integer jmin, jmax;
    static doublereal unfl, ovfl;
    extern /* Subroutine */ EXPORTSYMBOL int drot_();
    static integer nrow, i__, j, k, l, m;
    static doublereal s, v[3];
    extern logical lsame_();
    extern /* Subroutine */ EXPORTSYMBOL int mb04py_(), dcopy_();
    static integer i1, i2;
    static logical initz, wantt, wantz;
    static doublereal v1, v2, v3;
    extern /* Subroutine */ EXPORTSYMBOL int dlanv2_(), dlabad_();
    static doublereal h11, h12, h21, h22, h33, h44;
    static integer nh;
    ...
    extern /* Subroutine */ EXPORTSYMBOL int dlaset_(), dlartg_(), xerbla_(), dlarfx_();
    ...
    h_dim1 = *ldh1;
    h_dim2 = *ldh2;
    h_offset = h_dim1 * (h_dim2 + 1) + 1;
    h__ -= h_offset;
    z_dim1 = *ldz1;
    z_dim2 = *ldz2;
    z_offset = z_dim1 * (z_dim2 + 1) + 1;
    z__ -= z_offset;
    --wr;
    --wi;
    --dwork;
        ...
        if (wantt)
        {
            ...
            if (jmin != 0)
            {
                ...
                for (j = 1; j <= i__1; ++j)
                {
                    ...
                    dlarfg_(&c__2, &h__[i__ + (i__ + j * h_dim2) * h_dim1], v, &c__1, &tau);
            ...
            else
            {
                ...
                for (j = *p; j >= i__1; --j)
                {
                    ...
                    /*                 element of the matrix H_j. */
                    v[0] = h__[i__ + (i__ - 1 + j * h_dim2) * h_dim1];
                    dlarfg_(&c__2, &h__[i__ - 1 + (i__ - 1 + j * h_dim2) * h_dim1], v, &c__1, &tau);
                    h__[i__ + (i__ - 1 + j * h_dim2) * h_dim1] = 0.;
                    /*                 Apply G1 from the left to transform the rows of the */

...
EXPORTSYMBOL /* Subroutine */ int ab05qd_(over, n1, m1, p1, n2, m2, p2, a1, lda1, b1, ldb1, c1,
    ldc1, d1, ldd1, a2, lda2, b2, ldb2, c2, ldc2, d2, ldd2, n, m, p, a, lda, b, ldb, c__, ldc, d__,
    ldd, info, over_len) char* over;
...
ftnlen over_len;
{
    ...
    *info = 0;
    ...
    if (*n2 > 0) {
        dlaset_("F", n1, n2, &c_b9, &c_b9, &a[(*n1 + 1) * a_dim1 + 1], lda, 1L);
        dlaset_("F", n2, n1, &c_b9, &c_b9, &a[*n1 + 1 + a_dim1], lda, 1L);
    ...
    if (*m2 > 0) {
        dlaset_("F", n1, m2, &c_b9, &c_b9, &b[(*m1 + 1) * b_dim1 + 1], ldb, 1L);
    }
    if (*n2 > 0) {
        dlaset_("F", n2, m1, &c_b9, &c_b9, &b[*n1 + 1 + b_dim1], ldb, 1L);
        if (*m2 > 0) {
            dlacpy_("F", n2, m2, &b2[b2_offset], ldb2, &b[*n1 + 1 + (*m1 + 1) * b_dim1], ldb, 1L);
        }
    }
    /*                         ( C1   0  ) */
    /*     Construct      C =  (         ) . */
    ...
    if (*n2 > 0) {
        dlaset_("F", p1, n2, &c_b9, &c_b9, &c__[(*n1 + 1) * c_dim1 + 1], ldc, 1L);
    }
    if (*p2 > 0) {
        if (*n1 > 0) {
            dlaset_("F", p2, n1, &c_b9, &c_b9, &c__[*p1 + 1 + c_dim1], ldc, 1L);
        }
        if (*n2 > 0) {
            dlacpy_("F", p2, n2, &c2[c2_offset], ldc2, &c__[*p1 + 1 + (*n1 + 1) * c_dim1], ldc, 1L);
        }
    }
    /*                          ( D1  0  ) */
    /*     Construct       D =  (        ) . */
    ...
    if (*m2 > 0) {
        dlaset_("F", p1, m2, &c_b9, &c_b9, &d__[(*m1 + 1) * d_dim1 + 1], ldd, 1L);
    }
    if (*p2 > 0) {
        dlaset_("F", p2, m1, &c_b9, &c_b9, &d__[*p1 + 1 + d_dim1], ldd, 1L);
        if (*m2 > 0) {
            dlacpy_("F", p2, m2, &d2[d2_offset], ldd2, &d__[*p1 + 1 + (*m1 + 1) * d_dim1], ldd, 1L);
        }
    }
    return 0;
    /* *** Last line of AB05QD *** */

...
EXPORTSYMBOL /* Subroutine */ int ab09gd_(dico, jobcf, fact, jobmr, equil, ordsel, n, m, p, nr,
    alpha, a, lda, b, ldb, c__, ldc, d__, ldd, nq, hsv, tol1, tol2, tol3, iwork, dwork, ldwork,
    iwarn, info, dico_len, jobcf_len, fact_len, jobmr_len, equil_len, ordsel_len) char *dico,
...
ftnlen ordsel_len;
{
    ...
    static logical left;
    static integer ierr;
    extern /* Subroutine */ int sb08cd_(), ab09bx_(), sb08dd_(), sb08ed_(), sb08fd_(), sb08gd_(),
        sb08hd_(), tb01id_();
    ...
    if (left) {
        ...
        if (stabd) {
            ...
            sb08ed_(dico, n, m, p, alpha, &a[a_offset], lda, &dwork[kb], n, &dwork[kc], &maxmp,
                &dwork[kd], &maxmp, nq, &ndr, &dwork[kbr], n, &dwork[kdr], &maxmp, tol3, &dwork[kw],
                &lwr, iwarn, info, 1L);
        } else {
            ...
            sb08cd_(dico, n, m, p, &a[a_offset], lda, &dwork[kb], n, &dwork[kc], &maxmp, &dwork[kd],
                &maxmp, nq, &ndr, &dwork[kbr], n, &dwork[kdr], &maxmp, tol3, &dwork[kw], &lwr,
                iwarn, info, 1L);
        }
        *iwarn *= 10;
        if (*info != 0) {
        ...
        kw = kti + *nq * *nq;
        i__1 = *ldwork - kw + 1;
        ab09bx_(dico, jobmr, ordsel, nq, &mp, p, nr, &a[a_offset], lda, &dwork[kb], n, &dwork[kc],
            &maxmp, &dwork[kd], &maxmp, &hsv[1], &dwork[kt], n, &dwork[kti], n, tol1, tol2,
            &iwork[1], &dwork[kw], &i__1, &iwarnk, &ierr, 1L, 1L, 1L);
        *iwarn += iwarnk;
        if (ierr != 0) {
            *info = 4;
            return 0;
        ...
        if (stabd) {
            ...
            sb08fd_(dico, n, m, p, alpha, &a[a_offset], lda, &b[b_offset], ldb, &dwork[kc], &pm,
                &dwork[kd], &pm, nq, &ndr, &dwork[kcr], &pm, &dwork[kdr], &pm, tol3, &dwork[kw],
                &lwr, iwarn, info, 1L);
        } else {
        ...
        *iwarn *= 10;
        ...
        kw = kti + *nq * *nq;
        i__1 = *ldwork - kw + 1;
        ab09bx_(dico, jobmr, ordsel, nq, m, &pm, nr, &a[a_offset], lda, &b[b_offset], ldb,
            &dwork[kc], &pm, &dwork[kd], &pm, &hsv[1], &dwork[kt], n, &dwork[kti], n, tol1, tol2,
            &iwork[1], &dwork[kw], &i__1, &iwarnk, &ierr, 1L, 1L, 1L);
        *iwarn += iwarnk;
        if (ierr != 0) {
            *info = 4;

...
EXPORTSYMBOL /* Subroutine */ int sb04qd_(
...
integer *ldwork, *info;
{
    ...
    *info = 0;
    ...
    {
        /* Computing MAX */
        i__1 = 1, i__2 = (*n << 1) * *n + *n * 9, i__1 = max(i__1, i__2), i__2 = *m * 5,
    ...
    if (blas3) {
        ...
        for (i__ = 1; i__2 < 0 ? i__ >= i__1 : i__ <= i__1; i__ += i__2) {
            ...
            i__3 = *n - i__ + 1;
            bl = min(i__3, chunk);
            dgemm_("NoTranspose", "NoTranspose", &bl, m, m, &c_b11, &c__[i__ + c_dim1], ldc,
                &z__[z_offset], ldz, &c_b12, &dwork[jwork], &bl, 11L, 11L);
...
L80:
    if (ind > 1) {
        if (b[ind + (ind - 1) * b_dim1] == 0.) {
            ...
            sb04qy_(m, n, &ind, &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset], ldc,
            ...
            /*           Workspace:  2*N*N + 9*N; */
            sb04qu_(m, n, &ind, &a[a_offset], lda, &b[b_offset], ldb, &c__[c_offset], ldc,
                &dwork[jwork], &iwork[1], info);
            if (*info != 0) {
                *info += *m;
                return 0;
    ...
    dormhr_("Left", "No transpose", n, m, &ilo, &ihi, &a[a_offset], lda, &dwork[itau],
        &c__[c_offset], ldc, &dwork[jwork], &i__2, &ifail, 4L, 12L);
    /* Computing MAX */
    ...
    if (blas3) {
        ...
        for (i__ = 1; i__1 < 0 ? i__ >= i__2 : i__ <= i__2; i__ += i__1) {
            ...
            i__3 = *n - i__ + 1;
            bl = min(i__3, chunk);
            dgemm_("NoTranspose", "Transpose", &bl, m, m, &c_b11, &c__[i__ + c_dim1], ldc,
                &z__[z_offset], ldz, &c_b12, &dwork[jwork], &bl, 11L, 9L);
            dlacpy_("Full", &bl, m, &dwork[jwork], &bl, &c__[i__ + c_dim1], ldc, 4L);
            /* L100: */
        }
    } else {

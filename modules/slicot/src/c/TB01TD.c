...
integer* info;
{
    ...
    *info = 0;
    ...
    dgebal_("Both", n, &a[a_offset], lda, low, igh, &scstat[1], info, 4L);
    ...
    for (k = 1; k <= i__1; ++k) {
        ...
        if (*low > kold || kold > *igh) {
            ...
            if (knew != kold) {
                ...
                dswap_(p, &c__[kold * c_dim1 + 1], &c__1, &c__[knew * c_dim1 + 1], &c__1);
            }
        }
        /* L10: */
    }
    ...
    acnorm = dlange_("1-norm", n, n, &a[a_offset], lda, &dwork[1], 6L);
    arnorm = dlange_("I-norm", n, n, &a[a_offset], lda, &dwork[1], 6L);

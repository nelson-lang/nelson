...
L50:
    ...
    if (*nr > 0) {
        ...
        dlacpy_("Full", &nminr, &nu, &a[nkr1 * a_dim1 + 1], lda, &a[nr1 * a_dim1 + 1], lda, 4L);
        dlacpy_("Full", &nu, &na, &a[nkr1 + a_dim1], lda, &a[nr1 + a_dim1], lda, 4L);
        dlacpy_("Full", &nu, m, &b[nkr1 + b_dim1], ldb, &b[nr1 + b_dim1], ldb, 4L);
        dlacpy_("Full", p, &nu, &c__[nkr1 * c_dim1 + 1], ldc, &c__[nr1 * c_dim1 + 1], ldc, 4L);
        /*        Save B1 and C1. */
        dlacpy_("Full", &na, m, &b[b_offset], ldb, &dwork[kb1], &ldb1, 4L);
        dlacpy_("Full", p, &na, &c__[c_offset], ldc, &dwork[kc1], &ldc1, 4L);
    }

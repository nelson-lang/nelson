...
EXPORTSYMBOL /* Subroutine */ int sb10id_(n, m, np, a, lda, b, ldb, c__, ldc, d__, ldd, factor, nk,
    ak, ldak, bk, ldbk, ck, ldck, dk, lddk, rcond, iwork, dwork, ldwork, bwork, info) integer *n,
    *m, *np;
doublereal* a;
integer* lda;
doublereal* b;
...
integer* info;
{
    ...
    *info = 0;
    ...
    minwrk = (*n << 2) * *n + *m * *m + *np * *np + (*m << 1) * *n + *n * *np + (*n << 2)
        + max(i__1, i__2);
    if (*ldwork < minwrk) {
        *info = -25;
    }
    ...
    iwrk = i13 + (*n << 2) * *n;
    /*     Compute D'*C . */
    dgemm_("T", "N", m, n, np, &c_b5, &d__[d_offset], ldd, &c__[c_offset], ldc, &c_b6,
        &dwork[i2 + 1], m, 1L, 1L);
    /*     Compute S = Im + D'*D . */
    ...
    dtrsm_(
        "L", "U", "T", "N", np, n, &c_b5, &dwork[i5 + 1], np, &dwork[i6 + 1], np, 1L, 1L, 1L, 1L);
    /*                         -1 */
    /*     Compute Ar = A - B*S  D'*C . */
    dlacpy_("F", n, n, &a[a_offset], lda, &dwork[i7 + 1], n, 1L);
    dgemm_("N", "N", n, n, m, &c_b39, &b[b_offset], ldb, &dwork[i2 + 1], m, &c_b5, &dwork[i7 + 1],
        n, 1L, 1L);
    /*                                            -1 */
    ...
    n2 = *n << 1;
    i__1 = *ldwork - iwrk;
    sb02rd_("A", "C", hinv, "N", "U", "G", "S", "N", "O", n, &dwork[i7 + 1], n, &dwork[i10 + 1], n,
        &ak[ak_offset], ldak, &dwork[i9 + 1], n, &dwork[i8 + 1], n, &dwork[1], n, &sep, &rcond[1],
        &ferr, &dwork[i11 + 1], &dwork[i12 + 1], &dwork[i13 + 1], &n2, &iwork[1], &dwork[iwrk + 1],
        &i__1, &bwork[1], &info2, 1L, 1L, 1L, 1L, 1L, 1L, 1L, 1L, 1L);
    if (info2 != 0) {
        *info = 1;
        return 0;
    }
    ...
    i__1 = *ldwork - iwrk;
    sb02rd_("A", "C", hinv, "T", "U", "G", "S", "N", "O", n, &dwork[i7 + 1], n, &dwork[i10 + 1], n,
        &ak[ak_offset], ldak, &dwork[i8 + 1], n, &dwork[i9 + 1], n, &dwork[i1 + 1], n, &sep,
        &rcond[2], &ferr, &dwork[i11 + 1], &dwork[i12 + 1], &dwork[i13 + 1], &n2, &iwork[1],
        &dwork[iwrk + 1], &i__1, &bwork[1], &info2, 1L, 1L, 1L, 1L, 1L, 1L, 1L, 1L, 1L);
    if (info2 != 0) {
        *info = 2;
        return 0;
    }
    ...
    dgemm_("T", "N", m, n, n, &c_b39, &dwork[i3 + 1], n, &dwork[1], n, &c_b39, &dwork[i2 + 1], m,
        1L, 1L);
    /*     Compute gamma . */
    dgemm_("N", "N", n, n, n, &c_b5, &dwork[1], n, &dwork[i1 + 1], n, &c_b6, &dwork[i7 + 1], n, 1L,
        1L);
    i__1 = *ldwork - iwrk;
    dgees_("N", "N", select_, n, &dwork[i7 + 1], n, &sdim, &dwork[i11 + 1], &dwork[i12 + 1],
    ...
    dgemm_("N", "N", n, n, m, &c_b5, &b[b_offset], ldb, &dwork[i2 + 1], m, &c_b5, &dwork[i4 + 1], n,
        1L, 1L);
    /*     Compute W1' = (1-gamma^2)*In + Z*X . */
    d__1 = 1. - gamma * gamma;
    dlaset_("F", n, n, &c_b6, &d__1, &dwork[i3 + 1], n, 1L);
    dgemm_("N", "N", n, n, n, &c_b5, &dwork[i1 + 1], n, &dwork[1], n, &c_b5, &dwork[i3 + 1], n, 1L,
        1L);
    /*     Compute Bcp = gamma^2*Z*C' . */
    d__1 = gamma * gamma;
    dgemm_("N", "T", n, np, n, &d__1, &dwork[i1 + 1], n, &c__[c_offset], ldc, &c_b6, &bk[bk_offset],
        ldbk, 1L, 1L);
    /*     Compute C + D*F1 . */
    dlacpy_("F", np, n, &c__[c_offset], ldc, &dwork[i5 + 1], np, 1L);
    dgemm_("N", "N", np, n, m, &c_b5, &d__[d_offset], ldd, &dwork[i2 + 1], m, &c_b5, &dwork[i5 + 1],
        np, 1L, 1L);
    /*     Compute Acp = W1'*Ac + gamma^2*Z*C'*(C+D*F1) . */
    dgemm_("N", "N", n, n, n, &c_b5, &dwork[i3 + 1], n, &dwork[i4 + 1], n, &c_b6, &ak[ak_offset],
        ldak, 1L, 1L);
    dgemm_("N", "N", n, n, np, &c_b5, &bk[bk_offset], ldbk, &dwork[i5 + 1], np, &c_b5,
        &ak[ak_offset], ldak, 1L, 1L);
    /*     Compute Ccp = B'*X . */
    dgemm_("T", "N", m, n, n, &c_b5, &b[b_offset], ldb, &dwork[1], n, &c_b6, &ck[ck_offset], ldck,
        1L, 1L);
    /*     Set Dcp = -D' . */
    i__1 = *m;
    for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    /*                            prefer larger. */
    i__1 = *ldwork - iwrk;
    sb10jd_(n, np, m, &ak[ak_offset], ldak, &bk[bk_offset], ldbk, &ck[ck_offset], ldck,
        &dk[dk_offset], lddk, &dwork[i3 + 1], n, nk, &dwork[iwrk + 1], &i__1, &info2);
    if (info2 != 0) {
        *info = 3;
        return 0;
    }
    ...
    dgemm_("N", "N", np, np, m, &c_b39, &d__[d_offset], ldd, &dk[dk_offset], lddk, &c_b5, &dwork[1],
        np, 1L, 1L);
    /*                         -1 */
    /*     Compute Bk*(Ip-D*Dk)  . */
    ...
    dgemm_("N", "N", m, m, np, &c_b39, &dk[dk_offset], lddk, &d__[d_offset], ldd, &c_b5,
        &dwork[i3 + 1], m, 1L, 1L);
    /*                        -1 */
    /*     Compute B*(Im-Dk*D)  . */
    ...
    /*     Compute D*Ck . */
    dgemm_("N", "N", np, nk, m, &c_b5, &d__[d_offset], ldd, &ck[ck_offset], ldck, &c_b6,
        &dwork[i5 + 1], np, 1L, 1L);
    /*     Compute Dk*C . */
    dgemm_("N", "N", m, n, np, &c_b5, &dk[dk_offset], lddk, &c__[c_offset], ldc, &c_b6,
        &dwork[i6 + 1], m, 1L, 1L);
    /*     Compute the closed-loop state matrix. */
    i__1 = *n + *nk;
    dlacpy_("F", n, n, &a[a_offset], lda, &dwork[i7 + 1], &i__1, 1L);
    i__1 = *n + *nk;
    dgemm_("N", "N", n, n, m, &c_b5, &dwork[i4 + 1], n, &dwork[i6 + 1], m, &c_b5, &dwork[i7 + 1],
        &i__1, 1L, 1L);
    i__1 = *n + *nk;
    dgemm_("N", "N", nk, n, np, &c_b5, &dwork[i2 + 1], nk, &c__[c_offset], ldc, &c_b6,
        &dwork[i7 + *n + 1], &i__1, 1L, 1L);
    i__1 = *n + *nk;
    dgemm_("N", "N", n, nk, m, &c_b5, &dwork[i4 + 1], n, &ck[ck_offset], ldck, &c_b6,
        &dwork[i7 + (*n + *nk) * *n + 1], &i__1, 1L, 1L);
    i__1 = *n + *nk;
    dlacpy_("F", nk, nk, &ak[ak_offset], ldak, &dwork[i7 + (*n + *nk) * *n + *n + 1], &i__1, 1L);
    i__1 = *n + *nk;
    dgemm_("N", "N", nk, nk, np, &c_b5, &dwork[i2 + 1], nk, &dwork[i5 + 1], np, &c_b5,
        &dwork[i7 + (*n + *nk) * *n + *n + 1], &i__1, 1L, 1L);
    ...
    i__2 = *n + *nk;
    i__3 = *ldwork - iwrk;
    dgees_("N", "N", select_, &i__1, &dwork[i7 + 1], &i__2, &sdim, &dwork[1], &dwork[*n + *nk + 1],
        &dwork[iwrk + 1], n, &dwork[iwrk + 1], &i__3, &bwork[1], &info2, 1L, 1L);
    if (info2 != 0) {
        *info = 3;
        return 0;
    }

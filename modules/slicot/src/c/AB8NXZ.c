...
EXPORTSYMBOL /* Subroutine */ int ab8nxz_(n, m, p, ro, sigma, svlmax, abcd, ldabcd, ninfz, infz,
    kronl, mu, nu, nkrol, tol, iwork, dwork, zwork, lzwork, info) integer *n,
    *m, *p, *ro, *sigma;
doublereal* svlmax;
doublecomplex* abcd;
...
L20:
    ...
    if (*m > 0) {
        if (*sigma != 0) {
            ...
            for (i1 = 1; i1 <= i__1; ++i1) {
                i__2 = *ro + 1;
                zlarfg_(&i__2, &abcd[irow + i1 * abcd_dim1], &abcd[irow + 1 + i1 * abcd_dim1],
                    &c__1, &tc);
                i__2 = *ro + 1;
                i__3 = mnu - i1;
                d_cnjg(&z__1, &tc);
                zlatzm_("L", &i__2, &i__3, &abcd[irow + 1 + i1 * abcd_dim1], &c__1, &z__1,
        ...
        if (*sigma < *m) {
            ...
            irow = *nu + i1;
            i__1 = *m - *sigma;
            mb3oyz_(&ro1, &i__1, &abcd[irow + i1 * abcd_dim1], ldabcd, tol, svlmax, &rank, sval,
                &iwork[1], &zwork[itau], &dwork[1], &zwork[jwork], info);
            /* Computing MAX */
            ...
            if (rank > 0) {
                ...
                i__1 = *lzwork - jwork + 1;
                zunmqr_("Left", "Conjugate", &ro1, nu, &rank, &abcd[irow + i1 * abcd_dim1], ldabcd,
                    &zwork[itau], &abcd[irow + mm1 * abcd_dim1], ldabcd, &zwork[jwork], &i__1, info,
                    4L, 9L);
                /* Computing MAX */
                i__3 = jwork;
                ...
                if (ro1 > 1) {
                    ...
                    i__3 = ro1 - 1;
                    i__2 = min(i__3, rank);
                    zlaset_("Lower", &i__1, &i__2, &c_b1, &c_b1, &abcd[irow + 1 + i1 * abcd_dim1],
                        ldabcd, 5L);
                }
                ro1 -= rank;
            }
    ...
    *sigma = *mu - tau;
    ...
    if (*nu <= 0) {
        ...
        *ro = 0;
        ...
        mb3pyz_(&tau, nu, &abcd[i1 + mm1 * abcd_dim1], ldabcd, tol, svlmax, &rank, sval, &iwork[1],
            &zwork[itau], &dwork[1], &zwork[jwork], info);
        /* Computing MAX */
        ...
        if (rank > 0) {
            ...
            i__1 = i1 - 1;
            i__2 = *lzwork - jwork + 1;
            zunmrq_("Right", "ConjTranspose", &i__1, nu, &rank, &abcd[irow + mm1 * abcd_dim1],
                ldabcd, &zwork[mntau - rank + 1], &abcd[mm1 * abcd_dim1 + 1], ldabcd, &zwork[jwork],
                &i__2, info, 5L, 13L);
            /* Computing MAX */
            i__3 = jwork;
            i__1 = wrkopt, i__2 = (integer)zwork[i__3].r + jwork - 1;
            ...
            i__1 = *lzwork - jwork + 1;
            zunmrq_("Left", "NoTranspose", nu, &mnu, &rank, &abcd[irow + mm1 * abcd_dim1], ldabcd,
                &zwork[mntau - rank + 1], &abcd[abcd_offset], ldabcd, &zwork[jwork], &i__1, info,
                4L, 11L);
            /* Computing MAX */
            i__3 = jwork;
            i__1 = wrkopt, i__2 = (integer)zwork[i__3].r + jwork - 1;
            ...
            if (rank > 1) {
                i__1 = rank - 1;
                i__2 = rank - 1;
                zlaset_("Lower", &i__1, &i__2, &c_b1, &c_b1,

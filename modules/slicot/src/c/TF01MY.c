...
EXPORTSYMBOL /* Subroutine */ int tf01my_(n, m, p, ny, a, lda, b, ldb, c__, ldc, d__, ldd, u, ldu, x, y, ldy, dwork, ldwork, info)
integer *n, *m, *p, *ny;
doublereal *a;
integer *lda;
doublereal *b;
...
/*        Non-dynamic system: compute the output vectors. */
	...
	if (*m == 0) {
	    ...
	    dgemm_("No transpose", "Transpose", ny, p, m, &c_b8, &u[u_offset], ldu, &d__[d_offset], ldd, &c_b4, &y[y_offset], ldy, 12L, 9L);
	}
	return 0;
    }
...
/*        One row of the array Y is computed for each loop index value. */
	...
	i__1 = *ny;
	for (ik = 1; ik <= i__1; ++ik) {
	    dgemv_("No transpose", p, n, &c_b8, &c__[c_offset], ldc, &x[1], &c__1, &c_b4, &y[ik + y_dim1], ldy, 12L);

	    dgemv_("No transpose", n, n, &c_b8, &a[a_offset], lda, &x[1], &c__1, &c_b4, &dwork[1], &c__1, 12L);
	    dgemv_("No transpose", n, m, &c_b8, &b[b_offset], ldb, &u[ik + u_dim1], ldu, &c_b8, &dwork[1], &c__1, 12L);
...
/*           Compute the current NS-1 state vectors in the workspace. */
	    ...
	    i__3 = ns - 1;
	    dgemm_("No transpose", "Transpose", n, &i__3, m, &c_b8, &b[b_offset], ldb, &u[ik + u_dim1], ldu, &c_b4, &dwork[*n + 1], &maxn, 12L, 9L);

	    i__3 = ns - 1;
	    for (is = 1; is <= i__3; ++is) {
		dgemv_("No transpose", n, n, &c_b8, &a[a_offset], lda, &dwork[(is - 1) * *n + 1], &c__1, &upd, &dwork[is * *n + 1], &c__1, 12L);
/* L20: */
	    }

/*           Initialize the current NS output vectors. */

	    dgemm_("Transpose", "Transpose", &ns, p, n, &c_b8, &dwork[1], &maxn, &c__[c_offset], ldc, &c_b4, &y[ik + y_dim1], ldy, 9L, 9L);

/*           Prepare the next iteration. */

	    dgemv_("No transpose", n, m, &c_b8, &b[b_offset], ldb, &u[ik + ns - 1 + u_dim1], ldu, &c_b4, &dwork[1], &c__1, 12L);
	    dgemv_("No transpose", n, n, &c_b8, &a[a_offset], lda, &dwork[(ns - 1) * *n + 1], &c__1, &upd, &dwork[1], &c__1, 12L);
/* L30: */
	}
...
/*           First, compute the current IREM-1 state vectors. */
	    ...
	    ik = iyl + 1;
	    i__2 = irem - 1;
	    dgemm_("No transpose", "Transpose", n, &i__2, m, &c_b8, &b[b_offset], ldb, &u[ik + u_dim1], ldu, &c_b4, &dwork[*n + 1], &maxn, 12L, 9L);
...
/*           Initialize the last IREM output vectors. */
	    ...
	    dgemm_("Transpose", "Transpose", &irem, p, n, &c_b8, &dwork[1], &maxn, &c__[c_offset], ldc, &c_b4, &y[ik + y_dim1], ldy, 9L, 9L);

/*           Prepare the final state vector. */

	    dgemv_("No transpose", n, m, &c_b8, &b[b_offset], ldb, &u[ik + irem - 1 + u_dim1], ldu, &c_b4, &dwork[1], &c__1, 12L);
	    dgemv_("No transpose", n, n, &c_b8, &a[a_offset], lda, &dwork[(irem - 1) * *n + 1], &c__1, &upd, &dwork[1], &c__1, 12L);

	} else if (irem == 1) {

/*           Compute the last 1 output vectors. */

	    dgemv_("No transpose", p, n, &c_b8, &c__[c_offset], ldc, &dwork[1], &c__1, &c_b4, &y[ik + y_dim1], ldy, 12L);

/*           Prepare the final state vector. */

	    dcopy_(n, &dwork[1], &c__1, &dwork[*n + 1], &c__1);
	    dgemv_("No transpose", n, m, &c_b8, &b[b_offset], ldb, &u[ik + u_dim1], ldu, &c_b4, &dwork[1], &c__1, 12L);
	    dgemv_("No transpose", n, n, &c_b8, &a[a_offset], lda, &dwork[*n + 1], &c__1, &upd, &dwork[1], &c__1, 12L);
	}
...
/*     Add the direct contribution of the input to the output vectors. */
    ...
    dgemm_("No transpose", "Transpose", ny, p, m, &c_b8, &u[u_offset], ldu, &d__[d_offset], ldd, &c_b8, &y[y_offset], ldy, 12L, 9L);

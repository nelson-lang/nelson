...
ftnlen compz_len;
{
    ...
    --dwork;
    /* Function Body */
    upper = lsame_(jobe, "U", 1L, 1L);
    inq = lsame_(compq, "I", 1L, 1L);
    ilq = lsame_(compq, "V", 1L, 1L) || inq;
    inz = lsame_(compz, "I", 1L, 1L);
    ilz = lsame_(compz, "V", 1L, 1L) || inz;
    withb = *m > 0;
    withc = *p > 0;
    *info = 0;
    ...
    for (jcol = *ilo; jcol <= i__1; ++jcol) {
        ...
        for (jrow = *ihi; jrow >= i__2; --jrow) {
            ...
            dlartg_(&temp, &e[jrow + (jrow - 1) * e_dim1], &cs, &s, &e[jrow + jrow * e_dim1]);

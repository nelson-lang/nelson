...
/* Subroutine */ EXPORTSYMBOL int sb03pd_(job, fact, trana, n, a, lda, u, ldu, c__, ldc, scale, sepd, ferr, wr, wi, iwork, dwork, ldwork, info, job_len, fact_len, trana_len)
char *job, *fact, *trana;
integer *n;
doublereal *a;
integer *lda;
...
doublereal *c__;
integer *ldc;
doublereal *scale, *sepd, *ferr, *wr, *wi;
integer *iwork;
doublereal *dwork;
integer *ldwork, *info;
ftnlen job_len;
...
ftnlen trana_len;
{
    ...
    extern /* Subroutine */ EXPORTSYMBOL int sb03mx_(), dcopy_();
    static char notra[1];
    ...
    --dwork;
    /* Function Body */
    wantx = lsame_(job, "X", 1L, 1L);
    wantsp = lsame_(job, "S", 1L, 1L);
    ...
    *info = 0;
    ...
    if (! wantsp)
    {
        ...
        *(unsigned char *)uplo = 'U';
        mb01rd_(uplo, "Transpose", n, n, &c_b15, &c_b16, &c__[c_offset], ldc, &u[u_offset], ldu, &c__[c_offset], ldc, &dwork[1], ldwork, info, 1L, 9L);
        i__1 = *n;
        ...
        mb01rd_(uplo, "No transpose", n, n, &c_b15, &c_b16, &c__[c_offset], ldc, &u[u_offset], ldu, &c__[c_offset], ldc, &dwork[1], ldwork, info, 1L, 12L);
        i__1 = *n;
        for (i__ = 2; i__ <= i__1; ++i__)
        {
            i__2 = i__ - 1;
...
L30:
        ...
        if (kase != 0)
        {
            if (kase == 1)
            {
                sb03mx_(trana, n, &a[a_offset], lda, &dwork[1], n, &scalef, &dwork[(*n << 1) **n + 1], &ierr, 1L);
            }
            else
            {
                sb03mx_(notra, n, &a[a_offset], lda, &dwork[1], n, &scalef, &dwork[(*n << 1) **n + 1], &ierr, 1L);
            }
            goto L30;
        }
        /*        UNTIL KASE = 0 */
        *sepd = scalef / est;
        if (wantbh)
        {
            ...
            *ferr = dlamch_("Precision", 9L) * (d__1 * d__1) / *sepd;
        }
    }

...
ArrayOfVector Nelson::SlicotGateway::slicot_ab04mdBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    ArrayOf DWORK = ArrayOf::doubleMatrix2dConstructor(1 , std::max(1, (int)A.getDimensions().getRows()));
    ...
    Dimensions dimsC_expected(std::max(1, (int)C.getDimensions().getRows()), (int)A.getDimensions().getRows());
    if (!dimsC.equals(dimsC_expected))
    {
        Error(eval, _("Input argument #6: wrong size.") + " " + dimsC_expected.toString() + " " + "expected" + ".");
    }
    Dimensions dimsD_expected(std::max(1, (int)C.getDimensions().getRows()), (int)B.getDimensions().getColumns());
    if (!dimsD.equals(dimsD_expected))
    {
        Error(eval, _("Input argument #7: wrong size.") + " " + dimsD_expected.toString() + " " + "expected" + ".");
    }
    // CALL EXTERN FUNCTION
    try
    {
        ab04md_ ( TYPE_ptr, N_ptr, M_ptr, P_ptr, ALPHA_ptr, BETA_ptr, A_output_ptr, LDA_ptr, B_output_ptr, LDB_ptr, C_output_ptr, LDC_ptr, D_output_ptr, LDD_ptr, IWORK_ptr, DWORK_ptr, LDWORK_ptr, INFO_output_ptr);
    }

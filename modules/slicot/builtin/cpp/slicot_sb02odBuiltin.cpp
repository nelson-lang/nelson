...
{
#endif
extern int sb02od_ ( const char *DICO, const char *JOBB, const char *FACT, const char *UPLO, const char *JOBL, const char *SORT, int *N, int *M, int *P,const double *A, int *LDA,const double *B, int *LDB,const double *Q, int *LDQ,const double *R, int *LDR,const double *L, int *LDL, double *RCOND, double *X, int *LDX, double *ALFAR, double *ALFAI, double *BETA, double *S, int *LDS, double *T, int *LDT, double *U, int *LDU, double *TOL, int *IWORK, double *DWORK, int *LDWORK, int *BWORK, int *INFO);
#ifdef __cplusplus
}
#endif
//=============================================================================
...
//=============================================================================
ArrayOfVector Nelson::SlicotGateway::slicot_sb02odBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    ArrayOf X_output = ArrayOf::doubleMatrix2dConstructor((indexType)std::max(1, (int)B.getDimensions().getRows()), (indexType)(int)B.getDimensions().getRows());
    double *X_output_ptr = (double*)X_output.getDataPointer();
    ArrayOf ALFAR_output = ArrayOf::doubleMatrix2dConstructor((indexType)1, (indexType)2 * (int)B.getDimensions().getColumns());
    double *ALFAR_output_ptr = (double*)ALFAR_output.getDataPointer();
    ArrayOf ALFAI_output = ArrayOf::doubleMatrix2dConstructor((indexType)1, (indexType)2 * (int)B.getDimensions().getColumns());
    double *ALFAI_output_ptr = (double*)ALFAI_output.getDataPointer();
    ArrayOf BETA_output = ArrayOf::doubleMatrix2dConstructor((indexType)1, (indexType)2 * (int)B.getDimensions().getColumns());
    double *BETA_output_ptr = (double*)BETA_output.getDataPointer();
    ArrayOf S_output = ArrayOf::doubleMatrix2dConstructor((indexType)(JOBB.getContentAsCString().compare("B") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()) : std::max(1, 2 * (int)B.getDimensions().getRows()), (indexType)(JOBB.getContentAsCString().compare("B") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()) : std::max(1, 2 * (int)B.getDimensions().getRows()));
    double *S_output_ptr = (double*)S_output.getDataPointer();
    ArrayOf T_output = ArrayOf::doubleMatrix2dConstructor((indexType)(JOBB.getContentAsCString().compare("B") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()) : (DICO.getContentAsCString().compare("D") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows()) : 1, (indexType)2 * (int)B.getDimensions().getRows());
    double *T_output_ptr = (double*)T_output.getDataPointer();
    ArrayOf U_output = ArrayOf::doubleMatrix2dConstructor((indexType)std::max(1, 2 * (int)B.getDimensions().getRows()), (indexType)2 * (int)B.getDimensions().getRows());
    ...
    N_local_ptr[0] = (int)B.getDimensions().getRows();
    ...
    LDA_local_ptr[0] = std::max(1, (int)B.getDimensions().getRows());
    ...
    LDQ_local_ptr[0] = std::max(1, std::max((int)P.getContentAsInteger32Scalar(), (int)B.getDimensions().getRows()));
    ...
    LDL_local_ptr[0] = std::max(1, (int)B.getDimensions().getRows());
    ArrayOf LDX_local = ArrayOf::int32VectorConstructor(1);
    int* LDX_local_ptr = (int*)LDX_local.getDataPointer();
    LDX_local_ptr[0] = std::max(1, (int)B.getDimensions().getRows());
    ArrayOf LDS_local = ArrayOf::int32VectorConstructor(1);
    int* LDS_local_ptr = (int*)LDS_local.getDataPointer();
    LDS_local_ptr[0] = (JOBB.getContentAsCString().compare("B") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()) : std::max(1, 2 * (int)B.getDimensions().getRows());
    ArrayOf LDT_local = ArrayOf::int32VectorConstructor(1);
    int* LDT_local_ptr = (int*)LDT_local.getDataPointer();
    LDT_local_ptr[0] = (JOBB.getContentAsCString().compare("B") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()) : (DICO.getContentAsCString().compare("D") == 0) ? std::max(1, 2 * (int)B.getDimensions().getRows()) : 1;
    ArrayOf LDU_local = ArrayOf::int32VectorConstructor(1);
    int* LDU_local_ptr = (int*)LDU_local.getDataPointer();
    LDU_local_ptr[0] = std::max(1, 2 * (int)B.getDimensions().getRows());
    ArrayOf IWORK_local = ArrayOf::int32VectorConstructor(1);
    int* IWORK_local_ptr = (int*)IWORK_local.getDataPointer();
    ArrayOf DWORK_local = ArrayOf::doubleMatrix2dConstructor(1 , std::max(std::max(std::max(7 * (2 * (int)B.getDimensions().getRows() + 1) + 16, 16 * (int)B.getDimensions().getRows()),2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()), 3 * (int)B.getDimensions().getColumns()));
    ...
    LDWORK_local_ptr[0] = std::max(std::max(std::max(7 * (2 * (int)B.getDimensions().getRows() + 1) + 16, 16 * (int)B.getDimensions().getRows()),2 * (int)B.getDimensions().getRows() + (int)B.getDimensions().getColumns()), 3 * (int)B.getDimensions().getColumns());
    ArrayOf BWORK_local = ArrayOf::int32Matrix2dConstructor(1 , 2 * (int)B.getDimensions().getRows());

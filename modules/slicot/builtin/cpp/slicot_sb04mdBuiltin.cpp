...
//=============================================================================
ArrayOfVector Nelson::SlicotGateway::slicot_sb04mdBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    ArrayOf DWORK = ArrayOf::doubleMatrix2dConstructor(std::max(std::max(std::max(1, 2 * (int)A.getDimensions().getColumns() * (int)A.getDimensions().getColumns() + 8 * (int)A.getDimensions().getColumns()), 5 * (int)B.getDimensions().getRows()), (int)A.getDimensions().getColumns() + (int)B.getDimensions().getRows()) , 1);
    double * DWORK_ptr = (double*)DWORK.getDataPointer();
    ArrayOf LDWORK = ArrayOf::int32VectorConstructor(1);
    int* LDWORK_ptr = (int*)LDWORK.getDataPointer();
    LDWORK_ptr[0] = std::max(std::max(std::max(1, 2 * (int)A.getDimensions().getColumns() * (int)A.getDimensions().getColumns() + 8 * (int)A.getDimensions().getColumns()), 5 * (int)B.getDimensions().getRows()), (int)A.getDimensions().getColumns() + (int)B.getDimensions().getRows());
    // OUTPUT VARIABLES
    ...
    Dimensions dimsC_expected(std::max(1, (int)A.getDimensions().getColumns()), (int)B.getDimensions().getRows());
    ...
    try
    {
        sb04md_ ( N_ptr, M_ptr, A_output_ptr, LDA_ptr, B_output_ptr, LDB_ptr, C_output_ptr, LDC_ptr, Z_output_ptr, LDZ_ptr, IWORK_ptr, DWORK_ptr, LDWORK_ptr, INFO_output_ptr);
    }

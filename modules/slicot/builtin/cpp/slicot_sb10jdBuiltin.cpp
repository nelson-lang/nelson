...
//=============================================================================
ArrayOfVector Nelson::SlicotGateway::slicot_sb10jdBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    ArrayOf DWORK = ArrayOf::doubleMatrix2dConstructor(std::max( 1, 2 * (int)A.getDimensions().getColumns() * (int)A.getDimensions().getColumns() + 2 * (int)A.getDimensions().getColumns() + (int)A.getDimensions().getColumns() * std::max( 5, (int)A.getDimensions().getColumns() + (int)B.getDimensions().getRows() + (int)C.getDimensions().getColumns() ) ) , 1);
    double * DWORK_ptr = (double*)DWORK.getDataPointer();
    ArrayOf LDWORK = ArrayOf::int32VectorConstructor(1);
    int* LDWORK_ptr = (int*)LDWORK.getDataPointer();
    LDWORK_ptr[0] = std::max( 1, 2 * (int)A.getDimensions().getColumns() * (int)A.getDimensions().getColumns() + 2 * (int)A.getDimensions().getColumns() + (int)A.getDimensions().getColumns() * std::max( 5, (int)A.getDimensions().getColumns() + (int)B.getDimensions().getRows() + (int)C.getDimensions().getColumns() ) );
    // OUTPUT VARIABLES
    ...
    Dimensions dimsB_expected(std::max(1, (int)A.getDimensions().getColumns()), (int)B.getDimensions().getRows());
    if (!dimsB.equals(dimsB_expected))
    {
        Error(eval, _("Input argument #2: wrong size.") + " " + dimsB_expected.toString() + " " + "expected" + ".");
    }
    Dimensions dimsC_expected(std::max(1, (int)A.getDimensions().getColumns()), (int)A.getDimensions().getColumns());
    if (!dimsC.equals(dimsC_expected))
    {
        Error(eval, _("Input argument #3: wrong size.") + " " + dimsC_expected.toString() + " " + "expected" + ".");
    }
    Dimensions dimsD_expected(std::max(1, (int)C.getDimensions().getColumns()), (int)B.getDimensions().getRows());
    if (!dimsD.equals(dimsD_expected))
    {
        Error(eval, _("Input argument #4: wrong size.") + " " + dimsD_expected.toString() + " " + "expected" + ".");
    }
    Dimensions dimsE_expected(std::max(1, (int)A.getDimensions().getColumns()), (int)A.getDimensions().getColumns());
    ...
    try
    {
        sb10jd_ ( N_ptr, M_ptr, NP_ptr, A_output_ptr, LDA_ptr, B_output_ptr, LDB_ptr, C_output_ptr, LDC_ptr, D_output_ptr, LDD_ptr, E_output_ptr, LDE_ptr, NSYS_output_ptr, DWORK_ptr, LDWORK_ptr, INFO_output_ptr);
    }
    catch (std::runtime_error &e)
    {
        e.what();
        Error(eval, "sb10jd function fails.");
    }

...
{
#endif
extern int mb02md_ ( const char *JOB, int *M, int *N, int *L, int *RANK, double *C, int *LDC, double *S, double *X, int *LDX, double *TOL, int *IWORK, double *DWORK, int *LDWORK, int *IWARN, int *INFO);
#ifdef __cplusplus
}
#endif
//=============================================================================
...
//=============================================================================
ArrayOfVector Nelson::SlicotGateway::slicot_mb02mdBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    LDC_ptr[0] = std::max(std::max(1, (int)M.getContentAsInteger32Scalar()), (int)N.getContentAsInteger32Scalar() + (int)L.getContentAsInteger32Scalar());
    ...
    Dimensions dimsC_expected(std::max(std::max(1, (int)M.getContentAsInteger32Scalar()), (int)N.getContentAsInteger32Scalar() + (int)L.getContentAsInteger32Scalar()), (int)N.getContentAsInteger32Scalar() + (int)L.getContentAsInteger32Scalar());
    if (!dimsC.equals(dimsC_expected))
    {
        Error(eval, _("Input argument #6: wrong size.") + " " + dimsC_expected.toString() + " " + "expected" + ".");
    }
    if (!dimsTOL.isScalar())
    {
    ...
    try
    {
        mb02md_ ( JOB_ptr, M_ptr, N_ptr, L_ptr, RANK_output_ptr, C_output_ptr, LDC_ptr, S_output_ptr, X_output_ptr, LDX_ptr, TOL_ptr, IWORK_ptr, DWORK_ptr, LDWORK_ptr, IWARN_output_ptr, INFO_output_ptr);
    }
    catch (std::runtime_error &e)
    {
        e.what();
        Error(eval, "mb02md function fails.");
    }

...
ArrayOfVector Nelson::SlicotGateway::slicot_sb03odBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    LDB_ptr[0] = TRANS.getContentAsCString().compare("N") == 0 ? std::max(1, std::max((int)A.getDimensions().getRows(), (int)B.getDimensions().getRows())) : std::max(1, (int)A.getDimensions().getRows());
    ArrayOf DWORK = ArrayOf::doubleMatrix2dConstructor(1 , std::max(1, 4 * (int)A.getDimensions().getRows() + std::min((int)B.getDimensions().getRows(), (int)A.getDimensions().getRows())));
    double * DWORK_ptr = (double*)DWORK.getDataPointer();
    ArrayOf LDWORK = ArrayOf::int32VectorConstructor(1);
    int* LDWORK_ptr = (int*)LDWORK.getDataPointer();
    LDWORK_ptr[0] = std::max(1, 4 * (int)A.getDimensions().getRows() + std::min((int)B.getDimensions().getRows(), (int)A.getDimensions().getRows()));
    ...
    try
    {
        sb03od_ ( DICO_ptr, FACT_ptr, TRANS_ptr, N_ptr, M_ptr, A_ptr, LDA_ptr, Q_output_ptr, LDQ_ptr, B_output_ptr, LDB_ptr, SCALE_output_ptr, WR_output_ptr, WI_output_ptr, DWORK_ptr, LDWORK_ptr, INFO_output_ptr);
    }

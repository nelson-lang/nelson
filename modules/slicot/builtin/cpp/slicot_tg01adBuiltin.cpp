...
{
#endif
extern int tg01ad_ ( const char *JOB, int *L, int *N, int *M, int *P, double *THRESH, double *A, int *LDA, double *E, int *LDE, double *B, int *LDB, double *C, int *LDC, double *LSCALE, double *RSCALE, double *DWORK, int *INFO);
#ifdef __cplusplus
}
#endif
//=============================================================================
...
//=============================================================================
ArrayOfVector Nelson::SlicotGateway::slicot_tg01adBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    ArrayOf DWORK = ArrayOf::doubleMatrix2dConstructor(1 , 3 * ((int)L.getContentAsInteger32Scalar() + (int)N.getContentAsInteger32Scalar()));
    double * DWORK_ptr = (double*)DWORK.getDataPointer();
    // OUTPUT VARIABLES
    ArrayOf LSCALE_output = ArrayOf::doubleMatrix2dConstructor((indexType)1, (indexType)(int)L.getContentAsInteger32Scalar());
    double *LSCALE_output_ptr = (double*)LSCALE_output.getDataPointer();
    ArrayOf RSCALE_output = ArrayOf::doubleMatrix2dConstructor((indexType)1, (indexType)(int)N.getContentAsInteger32Scalar());
    ...
    {
        Error(eval, _("Input argument #4: wrong size.") + " " + dimsE_expected.toString() + " " + "expected" + ".");
    }
    Dimensions dimsB_expected(std::max(1, (int)L.getContentAsInteger32Scalar()), (int)M.getContentAsInteger32Scalar());
    ...
    try
    {
        tg01ad_ ( JOB_ptr, L_ptr, N_ptr, M_ptr, P_ptr, THRESH_ptr, A_output_ptr, LDA_ptr, E_output_ptr, LDE_ptr, B_output_ptr, LDB_ptr, C_output_ptr, LDC_ptr, LSCALE_output_ptr, RSCALE_output_ptr, DWORK_ptr, INFO_output_ptr);
    }
    catch (std::runtime_error &e)
    {
        e.what();
        Error(eval, "tg01ad function fails.");
    }

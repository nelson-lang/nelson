...
//=============================================================================
ArrayOfVector Nelson::SlicotGateway::slicot_sb04qdBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    ArrayOf DWORK = ArrayOf::doubleMatrix2dConstructor(1 , std::max(std::max(std::max(1, 2 * (int)A.getDimensions().getColumns() * (int)A.getDimensions().getColumns() + 9 * (int)A.getDimensions().getColumns()), 5 * (int)B.getDimensions().getRows()), (int)A.getDimensions().getColumns() + (int)B.getDimensions().getRows()));
    double * DWORK_ptr = (double*)DWORK.getDataPointer();
    ArrayOf LDWORK = ArrayOf::int32VectorConstructor(1);
    int* LDWORK_ptr = (int*)LDWORK.getDataPointer();
    LDWORK_ptr[0] = std::max(std::max(std::max(1, 2 * (int)A.getDimensions().getColumns() * (int)A.getDimensions().getColumns() + 9 * (int)A.getDimensions().getColumns()), 5 * (int)B.getDimensions().getRows()), (int)A.getDimensions().getColumns() + (int)B.getDimensions().getRows());
    // OUTPUT VARIABLES
    ArrayOf Z_output = ArrayOf::doubleMatrix2dConstructor((indexType)std::max(1, (int)B.getDimensions().getRows()), (indexType)(int)B.getDimensions().getRows());
    ...
    Dimensions dimsB_expected(std::max(1, (int)B.getDimensions().getRows()), (int)B.getDimensions().getRows());
    if (!dimsB.equals(dimsB_expected))
    {
        Error(eval, _("Input argument #2: wrong size.") + " " + dimsB_expected.toString() + " " + "expected" + ".");
    }
    Dimensions dimsC_expected(std::max(1, (int)B.getDimensions().getRows()), (int)B.getDimensions().getRows());
    ...
    try
    {
        sb04qd_ ( N_ptr, M_ptr, A_output_ptr, LDA_ptr, B_output_ptr, LDB_ptr, C_output_ptr, LDC_ptr, Z_output_ptr, LDZ_ptr, IWORK_ptr, DWORK_ptr, LDWORK_ptr, INFO_output_ptr);
    }
    catch (std::runtime_error &e)
    {
        e.what();
        Error(eval, "sb04qd function fails.");
    }

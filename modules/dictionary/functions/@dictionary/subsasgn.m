...
function obj = subsasgn(obj, s, val)
  ...
  switch (s(1).type)
    ...
    case '()'
      ...
      if ischar(val)
        nbValues = numel(string(val));
      else
        nbValues = numel(val);
      end
      if (numel(key) ~= nbValues && nbValues ~= 1 && nbValues ~= 0)
    ...
    case '{}'
      if (iscell(s(1).subs) && (numel(s(1).subs) > 1))
        error('Nelson:dictionary:TooManyInputsToBracketAssign', _('Too many input arguments. To insert multiple entries, utilize an array of keys instead.'));
      end
      key = s(1).subs{1};
      if ischar(key)
        key = string (key);
    ...
    otherwise
      error(_("Only '()' and '{}' indexing is supported for assigning values."));
      end
    end
%=============================================================================

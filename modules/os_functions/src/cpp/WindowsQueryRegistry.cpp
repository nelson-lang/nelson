...
#define MAX_VALUE_NAME 16383
    ...
    static void initializeHkeyMap();
    static bool isValidHkeyFromString(const std::wstring &hkeystring);
    static HKEY getHkeyFromString(const std::wstring &hkeystring);
    static bool openRegistryKey(HKEY rootKey, std::wstring subKey, HKEY *ouputHkey);
    static bool getRegistryKeyInfo(HKEY rootKey, std::wstring subKey,
                                   size_t &keysNumber, size_t &valuesNumber);
#endif
    //=============================================================================
    ArrayOf windowsQueryRegistryAllValuesNames(const std::wstring &rootkey,
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (size_t i = 0; i < nbKeys; i++)
                    {
                        ...
                        DWORD retCode = RegEnumValue(desiredHkey,(DWORD) i,
                                                     achKey,
                                                     &cbName,
                                                     NULL,
                                                     NULL,
            ...
            std::wstring &errorMessage)
    {
        ...
        else
        {
            ...
            DWORD ouputType = 0;
            if (RegQueryValueEx(desiredHkey, valname.c_str(), NULL, &ouputType, NULL, NULL) == ERROR_SUCCESS)
            {
                if ((ouputType == REG_EXPAND_SZ) || (ouputType == REG_SZ))
                {
                    DWORD Length = MAX_VALUE_NAME;
                    wchar_t Line[MAX_VALUE_NAME];
                    if (RegQueryValueEx(desiredHkey, valname.c_str(), NULL, &ouputType, (LPBYTE)&Line, &Length) == ERROR_SUCCESS)
                    {
                ...
                else
                {
                    DWORD size = 4;
                    int Num = 0;
                    if (RegQueryValueEx(desiredHkey, valname.c_str(), NULL, &ouputType, (LPBYTE)&Num, &size) == ERROR_SUCCESS)
                    {
    ...
    //=============================================================================
    bool isValidHkeyFromString(const std::wstring &hkeystring)
    {
        return (hkeyMap.count(hkeystring) != 0);
    }
    //=============================================================================
    HKEY getHkeyFromString(const std::wstring &hkeystring)
    {
        return hkeyMap[hkeystring];
    }
    //=============================================================================
    bool openRegistryKey(HKEY rootKey, std::wstring subKey, HKEY *ouputHkey)
    {
    ...
    //=============================================================================
    bool getRegistryKeyInfo(HKEY rootKey, std::wstring subKey,
                            size_t &keysNumber, size_t &valuesNumber)
    {
        keysNumber = 0;
        valuesNumber = 0;

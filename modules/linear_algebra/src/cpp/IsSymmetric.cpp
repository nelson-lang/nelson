...
namespace Nelson {
	//=============================================================================
	template <class T>
	bool isSymmetricComplex(T* data, indexType N, bool skew)
	{
		indexType i, j;
		for (i = 0; i < N; i++)
		{
	...
	//=============================================================================
	template <class T>
	bool isSymmetricNoSkew(T* data, indexType N)
	{
		indexType i, j;
		for (i = 0; i < N; i++)
		{
	...
	//=============================================================================
	template <class T>
	bool isSymmetric(T* data, indexType N, bool skew)
	{
		indexType i, j;
		for (i = 0; i < N; i++)
		{
	...
	//=============================================================================
	template <class T>
	bool isSymmetric(T* data, indexType N, double tol)
	{
		indexType i, j;
		for (i = 0; i < N; i++)
		{
			for (j = 1; j < N; j++)
			{
				T realA = data[i + j * N];
				T realB = data[j + i * N];
				if (double(realA - realB) > tol) return false;
			}
		}
		return true;
	}
	//=============================================================================
	template <class T>
	bool isSymmetricComplex(T* data, indexType N, double tol)
	{
		indexType i, j;
		for (i = 0; i < N; i++)
		{
			for (j = 1; j < N; j++)
			{
				...
				if (realA - realB > (T)tol) return false;
				if (imagA - imagB > (T)tol) return false;
			}
		}
		return true;
	}
	//=============================================================================
	bool IsSymmetric(ArrayOf A, bool skew)
	{
		bool res = false;
		if (!A.is2D())
		{
		...
		switch (A.getDataClass())
		{
			...
			case NLS_SCOMPLEX:
				return isSymmetricComplex<single>((single*)A.getDataPointer(), A.getDimensions().getRows(), skew);
			case NLS_DCOMPLEX:
				return isSymmetricComplex<double>((double*)A.getDataPointer(), A.getDimensions().getRows(), skew);
			case NLS_INT8:
    ...
    //=============================================================================
	bool IsSymmetric(ArrayOf A, double tol)
	{
		bool res = false;
		if (!A.is2D())
		{
		...
		case NLS_SCOMPLEX:
			return isSymmetricComplex<single>((single*)A.getDataPointer(), A.getDimensions().getRows(), tol);
		case NLS_DCOMPLEX:
			return isSymmetricComplex<double>((double*)A.getDataPointer(), A.getDimensions().getRows(), tol);
		case NLS_INT8:

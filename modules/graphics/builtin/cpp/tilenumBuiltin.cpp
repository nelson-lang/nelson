...
static GOTiledChartLayout*
getParentLayout(int64 objHandle);
static ArrayOf
rowVectorFromDoubles(const std::vector<double>& values);
//=============================================================================
ArrayOfVector
tilenumBuiltin(int nLhs, const ArrayOfVector& argIn)
{
    ...
    if (argIn.size() == 3) {
        ...
        std::vector<double> rows = argIn[1].getContentAsDoubleVector();
        std::vector<double> cols = argIn[2].getContentAsDoubleVector();
        if (rows.size() != cols.size()) {
            Error(_W("Row and column arguments must have the same number of elements."));
        }

        std::vector<double> nums(rows.size());
        std::vector<double> gs = layout->findVectorDoubleProperty(GO_GRID_SIZE_PROPERTY_NAME_STR);
        int gridM = (gs.size() >= 1) ? std::max(1, (int)gs[0]) : 1;
        int gridN = (gs.size() >= 2) ? std::max(1, (int)gs[1]) : 1;
        for (size_t i = 0; i < rows.size(); ++i) {
            int r = (int)rows[i];
            int c = (int)cols[i];
            if (r < 1 || r > gridM || c < 1 || c > gridN) {
                nums[i] = NaN;
            } else {
                int idx = layout->rowColToTileIndex(r, c);
                nums[i] = (idx < 1) ? NaN : (double)idx;
            }
        }
        retval << rowVectorFromDoubles(nums);
        return retval;
    }
    ...
    if (argIn.size() == 1) {
        ...
        if (!objArg.isGraphicsObject()) {
            Error(
                _W("Argument must be a graphics object handle or TiledChartLayout with row/col."));
        }
        ...
        std::vector<double> nums(handles.size());
        for (size_t i = 0; i < handles.size(); ++i) {
            ...
            if (getTileFromLayoutOptions(objH, tileFromLayout, isEdgeTile)) {
                ...
                if (!currentLayout) {
                    currentLayout = findAnyTopLevelLayoutOnFigure(fig);
                }

                if (currentLayout) {
                    auto [r, c] = currentLayout->tileIndexToRowCol((int)tileFromLayout);
                    if (r >= 1 && c >= 1) {
                        nums[i] = tileFromLayout;
                        continue;
                    }
                }
            }

            GOTiledChartLayout* layout = getParentLayout(objH);
            if (!layout || layout->isEdgeTileHandle(objH)) {
                nums[i] = NaN;
                continue;
            }
            GOTiledChartLayout::TileSpanInfo info = layout->getTileSpanInfo(objH);
            if (info.tile == 0 && tileFromLayout > 0.0) {
                nums[i] = tileFromLayout;
            } else {
                nums[i] = (info.tile == 0) ? NaN : (double)info.tile;
            }
        }
        retval << rowVectorFromDoubles(nums);
        return retval;

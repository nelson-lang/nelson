...
void
client_impl::connect(
    const string& uri, const map<string, string>& query, const map<string, string>& headers)
{
    if (m_reconn_timer) {
...
socket::ptr const&
client_impl::socket(string const& nsp)
{
...
/*************************protected:*************************/
void
client_impl::send(packet& p)
{
    m_packet_mgr.encode(p);
}

void
client_impl::remove_socket(string const& nsp)
{
    lock_guard<mutex> guard(m_socket_mutex);
...
asio::io_service&
client_impl::get_io_service()
{
    return m_client.get_io_service();
}

void
client_impl::on_socket_closed(string const& nsp)
{
...
void
client_impl::on_socket_opened(string const& nsp)
{
...
void
client_impl::connect_impl(const string& uri, const string& queryString)
{
    do {
...
void
client_impl::close_impl(close::status::value const& code, string const& reason)
{
...
void
client_impl::send_impl(shared_ptr<const string> const& payload_ptr, frame::opcode::value opcode)
{
...
void
client_impl::ping(const asio::error_code& ec)
{
    if (ec || m_con.expired()) {
...
void
client_impl::timeout_pong(const asio::error_code& ec)
{
    if (ec) {
        return;
    }
...
void
client_impl::timeout_reconnect(asio::error_code const& ec)
{
    if (ec) {
        return;
    }
...
unsigned
client_impl::next_delay() const
{
    // no jitter, fixed power root.
    unsigned reconn_made = min<unsigned>(m_reconn_made, 32); // protect the pow result to be too
                                                             // big.
...
socket::ptr
client_impl::get_socket_locked(string const& nsp)
{
    lock_guard<mutex> guard(m_socket_mutex);
    auto it = m_sockets.find(nsp);
    if (it != m_sockets.end()) {
...
void
client_impl::sockets_invoke_void(void (sio::socket::*fn)(void))
{
...
void
client_impl::on_fail(connection_hdl)
{
...
void
client_impl::on_open(connection_hdl con)
{
    if (m_con_state == con_closing) {
...
void
client_impl::on_close(connection_hdl con)
{
...
void
client_impl::on_message(connection_hdl, client_type::message_ptr msg)
{
    if (m_ping_timeout_timer) {
        asio::error_code ec;
...
void
client_impl::on_handshake(message::ptr const& message)
{
...
failed:
    // just close it.
    m_client.get_io_service().dispatch(std::bind(
        &client_impl::close_impl, this, close::status::policy_violation, "Handshake error"));
}
...
void
client_impl::on_decode(packet const& p)
{
    switch (p.get_frame()) {
...
void
client_impl::on_encode(bool isBinary, shared_ptr<const string> const& payload)
{
    LOG("encoded payload length:" << payload->length() << endl);
    m_client.get_io_service().dispatch(std::bind(&client_impl::send_impl, this, payload,
        isBinary ? frame::opcode::binary : frame::opcode::text));
}

void
client_impl::clear_timers()
{
...
#if SIO_TLS
client_impl::context_ptr
client_impl::on_tls_init(connection_hdl conn)
{
...
std::string
client_impl::encode_query_string(const std::string& query)
{
    ostringstream ss;
    ss << std::hex;

...
public:
    static void
    adapt_func(socket::event_listener_aux const& func, event& event)
    {
        func(event.get_name(), event.get_message(), event.need_ack(), event.get_ack_message_impl());
    }

    static inline socket::event_listener
    do_adapt(socket::event_listener_aux const& func)
    {
    ...
    static inline event
    create_event(
        std::string const& nsp, std::string const& name, message::list&& message, bool need_ack)
    {
...
const message::list&
event::get_messages() const
{
    return m_messages;
}

bool
event::need_ack() const
{
    return m_need_ack;
}

void
event::put_ack_message(message::list const& ack_message)
{
    if (m_need_ack)
...
inline event::event(
    std::string const& nsp, std::string const& name, message::list&& messages, bool need_ack)
    : m_nsp(nsp), m_name(name), m_messages(std::move(messages)), m_need_ack(need_ack)
{ }

inline event::event(
    std::string const& nsp, std::string const& name, message::list const& messages, bool need_ack)
    : m_nsp(nsp), m_name(name), m_messages(messages), m_need_ack(need_ack)
{ }

message::list const&
event::get_ack_message() const
{
    return m_ack_message;
}

inline message::list&
event::get_ack_message_impl()
{
    return m_ack_message;
}
...
public:
    ...
    void
    on(std::string const& event_name, event_listener_aux const& func);

    void
    on(std::string const& event_name, event_listener const& func);

    void
    off(std::string const& event_name);
...
#undef SYNTHESIS_SETTER
    ...
    void
    on_error(error_listener const& l);
    ...
    void
    emit(std::string const& name, message::list const& msglist,
...
protected:
    ...
    void
    on_message_packet(packet const& packet);
...
private:
    ...
    void
    on_socketio_event(
        const std::string& nsp, int msgId, const std::string& name, message::list&& message);
    void
    on_socketio_ack(int msgId, message::list const& message);
    void
    on_socketio_error(message::ptr const& err_message);

    event_listener
    get_bind_listener_locked(string const& event);

    void
    ack(int msgId, string const& name, message::list const& ack_message);

    void
    timeout_connection(const asio::error_code& ec);
...
void
socket::impl::on(std::string const& event_name, event_listener_aux const& func)
{
    this->on(event_name, event_adapter::do_adapt(func));
}

void
socket::impl::on(std::string const& event_name, event_listener const& func)
{
...
void
socket::impl::off(std::string const& event_name)
{
    std::lock_guard<std::mutex> guard(m_event_mutex);
...
void
socket::impl::on_error(error_listener const& l)
{
    m_error_listener = l;
}

void
socket::impl::off_error()
{
    m_error_listener = nullptr;
}

socket::impl::impl(client_impl* client, std::string const& nsp)
    : m_client(client), m_connected(false), m_nsp(nsp)
{
    NULL_GUARD(client);
    if (m_client->opened()) {
        send_connect();
...
void
socket::impl::emit(std::string const& name, message::list const& msglist,
    std::function<void(message::list const&)> const& ack)
{
    NULL_GUARD(m_client);
...
void
socket::impl::on_message_packet(packet const& p)
{
    NULL_GUARD(m_client);
    if (p.get_nsp() == m_nsp) {
        switch (p.get_type()) {
        ...
        case packet::type_binary_event: {
            ...
            if (ptr->get_flag() == message::flag_array) {
                ...
                if (array_ptr->get_vector().size() >= 1
                    ...
                    this->on_socketio_event(
                        p.get_nsp(), p.get_pack_id(), name_ptr->get_string(), std::move(mlist));
                }
            }
...
void
socket::impl::ack(int msgId, const string&, const message::list& ack_message)
{
...
void
socket::impl::on_socketio_ack(int msgId, message::list const& message)
{
...
void
socket::impl::on_socketio_error(message::ptr const& err_message)
{
    if (m_error_listener)
...
void
socket::impl::timeout_connection(const asio::error_code& ec)
{
    NULL_GUARD(m_client);
    if (ec) {
        return;
...
void
socket::impl::send_packet(sio::packet& p)
{
    NULL_GUARD(m_client);
    if (m_connected) {
        while (true) {
...
socket::event_listener
socket::impl::get_bind_listener_locked(const string& event)
{
    std::lock_guard<std::mutex> guard(m_event_mutex);
...
void
socket::on(std::string const& event_name, event_listener const& func)
{
    m_impl->on(event_name, func);
}

void
socket::on(std::string const& event_name, event_listener_aux const& func)
{
    m_impl->on(event_name, func);
}

void
socket::off(std::string const& event_name)
{
...
void
socket::on_error(error_listener const& l)
{
    m_impl->on_error(l);
}
...
void
socket::emit(std::string const& name, message::list const& msglist,
    std::function<void(message::list const&)> const& ack)
{
...
void
socket::on_message_packet(packet const& p)
{

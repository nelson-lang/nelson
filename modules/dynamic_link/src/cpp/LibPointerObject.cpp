...
namespace Nelson {
    ...
    LibPointerObject::LibPointerObject(void *pointer,
                                       ...
                                       Class currentType) : HandleGenericObject(std::wstring(LIBPOINTER_CATEGORY_STR), this, false)
    {
        initializeCommon();
        _voidPointer = pointer;
        _DataType = DataType;
    ...
    //=============================================================================
    LibPointerObject * LibPointerObject::plus(indexType offset)
    {
    ...
    bool LibPointerObject::get(std::wstring propertyName, ArrayOf &res)
    {
        ...
        {
            if (_dimX == -1 || _dimY == -1 || _currentType == NLS_NOT_TYPED || _voidPointer == nullptr)
            {
                throw Exception(_W("The datatype and size of the value must be defined."));
            }

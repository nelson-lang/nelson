...
namespace Nelson {
    //=============================================================================
    ArrayOf StringReplace(ArrayOf STR, ArrayOf OLD, ArrayOf NEW, bool doOverlaps)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for (size_t i = 0; i < nbOutput; i++)
                {
                    ...
                    elements[i] = ArrayOf::stringConstructor(stringReplace(wstr[idx_str], wold[idx_old], wnew[idx_new], doOverlaps));
                }
    ...
    //=============================================================================
    std::wstring stringReplace(std::wstring originStr,
                               std::wstring subStr,
                               std::wstring replaceStr, bool doOverlaps)
    {
    ...
    //=============================================================================
    ArrayOf Replace(ArrayOf STR, ArrayOf OLD, ArrayOf NEW)
    {
        ArrayOf res;
        wstringVector wstr = STR.getContentAsWideStringVector(false);
        wstringVector wold = OLD.getContentAsWideStringVector(false);
        wstringVector wnew = NEW.getContentAsWideStringVector(false);
        if ((wstr.size() == 1) && OLD.isCell() && NEW.isCell() && OLD.getDimensions().equals(NEW.getDimensions()))
        {
                ...
                else
                {
                    ...
                    else
                    {
                        for (size_t i = 0; i < nbOutput; i++)
                        {
                            ...
                            size_t idx_old = (i)* old_incr;
                            size_t idx_new = (i)* new_incr;
                            elements[i] = ArrayOf::stringConstructor(Replace(wstr[idx_str], wold[idx_old], wnew[idx_new]));
                        }
                    }
                    res = ArrayOf(NLS_CELL_ARRAY, outputDims, elements);
                }
    ...
    //=============================================================================
    std::wstring Replace(std::wstring originStr,
                         std::wstring subStr,

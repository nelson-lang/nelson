...
#define PlusChar L'+'
    ...
    static int ParseNumber(std::wstring tx)
    {
        if (ToUpper(tx).compare(L"NAN*I") == 0 ||
                ...
                ToUpper(tx).compare(L"INFI") == 0 ||
                ToUpper(tx).compare(L"NANI") == 0)
        {
            return 3;
        }
        std::locale loc;
        ...
        if ((tx[lookahead] == L'E') || (tx[lookahead] == L'e') || (tx[lookahead] == L'D') || (tx[lookahead] == L'd'))
        {
            lookahead++;
            ...
            else
            {
                ...
                if (STR.length() > 0)
                {
                    ...
                    else
                    {
                        double realPart;
                        double imagPart;
                        if (boost::algorithm::contains(STR, L"i") || boost::algorithm::contains(STR, L"j"))
                        {
                            boost::replace_all(STR, L" ", L"");

...
namespace Nelson {
    //=============================================================================
    std::wstring MatrixToString(ArrayOf A, indexType precision, bool withClass)
    {
        std::wstring res;
        if (!A.is2D())
        {
        ...
        else
        {
            ...
            if (A.isEmpty())
            {
                ...
                {
                    res = res + L"zeros(" + std::to_wstring(dims.getRows()) + L"," + std::to_wstring(dims.getColumns()) + L")";
                }
            }
            else
            {
                    ...
                    case NLS_SCOMPLEX:
                    {
                        ...
                        for (size_t x = 0; x < dims.getRows(); x++)
                        {
                            for (size_t y = 0; y < dims.getColumns(); y++)
                            {
                                ...
                                if (imagPartStr.compare(L"NaN") == 0 || imagPartStr.compare(L"Inf") == 0 || imagPartStr.compare(L"-Inf") == 0)
                                {

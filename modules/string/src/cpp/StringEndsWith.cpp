...
namespace Nelson {
    //=============================================================================
    static bool endsWithString(const std::wstring &A, const std::wstring &pattern, bool bCaseSensitive)
    {
        bool res = false;
        if (bCaseSensitive)
        {
    ...
    //=============================================================================
    ArrayOf StringEndsWith(ArrayOf A, ArrayOf Pattern, bool bCaseSensitive)
    {
        ArrayOf res;
        if (A.isString() && Pattern.isString())
        {
            res = ArrayOf::logicalConstructor(endsWithString(A.getContentAsWideString(), Pattern.getContentAsWideString(), bCaseSensitive));
        }
        else
        {
            ...
            else if (IsCellOfString(A) && Pattern.isString())
            {
                ...
                {
                    result[k] = endsWithString(cellA[k].getContentAsWideString(), pattern, bCaseSensitive);
                }
            ...
            else if (IsCellOfString(A) && IsCellOfString(Pattern))
            {
                ...
                for (size_t k = 0; k < nbA; k++)
                {
                    ...
                    {
                        bool val = endsWithString(cellA[k].getContentAsWideString(), cellPattern[l].getContentAsWideString(), bCaseSensitive);
                        if (val)
                        {
                            result[k] = val;
                            break;

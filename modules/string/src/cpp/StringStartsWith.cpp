...
namespace Nelson {
    //=============================================================================
    static bool startsWithString(const std::wstring &A, const std::wstring &pattern, bool bCaseSensitive)
    {
        bool res = false;
        if (bCaseSensitive)
        {
    ...
    //=============================================================================
    ArrayOf StringStartsWith(ArrayOf A, ArrayOf Pattern, bool bCaseSensitive)
    {
        ArrayOf res;
        if (A.isString() && Pattern.isString())
        {
            res = ArrayOf::logicalConstructor(startsWithString(A.getContentAsWideString(), Pattern.getContentAsWideString(), bCaseSensitive));
        }
        else
        {
            ...
            else if (IsCellOfString(A) && Pattern.isString())
            {
                ...
                {
                    result[k] = startsWithString(cellA[k].getContentAsWideString(), pattern, bCaseSensitive);
                }
            ...
            else if (IsCellOfString(A) && IsCellOfString(Pattern))
            {
                ...
                for (size_t k = 0; k < nbA; k++)
                {
                    ...
                    {
                        bool val = startsWithString(cellA[k].getContentAsWideString(), cellPattern[l].getContentAsWideString(), bCaseSensitive);
                        if (val)
                        {
                            result[k] = val;
                            break;

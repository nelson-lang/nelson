...
namespace Nelson {
    ...
    std::wstring utf8_to_wstring(const std::string &str)
    {
        ...
        {
#ifdef _MSC_VER
            int size = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, str.c_str(), (int)str.length(), nullptr, 0);
            if (size > 0)
            {
                std::wstring utf16_str(size, '\0');
                int res = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, str.c_str(), (int)str.length(), &utf16_str[0], size);
                if (res > 0)
                {
                    return utf16_str;
                }
        ...
        buffer.resize(len);
        u_strToWCS(&result[0], (int32_t)result.size(), &len, &buffer[0], (int32_t)buffer.size(), &status);
        if (U_FAILURE(status))
        {
            return std::wstring(L"!!!ERROR ICU!!!");
        }
    ...
    std::string wstring_to_utf8(const std::wstring &str)
    {
        ...
        buffer.resize(len);
        u_strToUTF8(&result[0], (int32_t)result.size(), &len, &buffer[0], (int32_t)buffer.size(), &status);
        if (U_FAILURE(status))
        {
            return std::string("!!!ERROR ICU!!!");
        }

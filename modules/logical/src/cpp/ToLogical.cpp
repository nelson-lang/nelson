    ...
    ArrayOf ToLogical(ArrayOf A)
    {
        switch (A.getDataClass())
        {
            ...
            case NLS_STRUCT_ARRAY:
            {
                ...
                {
                    throw Exception(_("Undefined function 'logical' for input arguments of type '") + A.getStructType() + "'.");
                }
                else
                {
            ...
            case NLS_DOUBLE:
            {
                if (A.isSparse())
                {
                    void* pLogical = TypeConvertSparseDynamicFunction(NLS_LOGICAL, A.getDimensions().getRows(), A.getDimensions().getColumns(), A.getSparseDataPointer(), NLS_DOUBLE);

...
//=============================================================================
namespace Nelson {
    ArrayOf AnyLogical(ArrayOf A, indexType dim)
    {
        ArrayOf res;
        if (A.isEmpty())
        {
            if (dim == 0)
            {
                ...
                else
                {
                    ...
                    {
                        logical *logicalarray = (logical*)ArrayOf::allocateArrayOf(NLS_LOGICAL, A.getDimensions().getColumns());
                        res = ArrayOf(NLS_LOGICAL, Dimensions(1, A.getDimensions().getColumns()), logicalarray);
                    }
                }
            }
            else
            {
                ...
                else
                {
                    if (A.getDimensions().getRows() > A.getDimensions().getColumns())
                    {
                        ...
                        {
                            logical *logicalarray = (logical*)ArrayOf::allocateArrayOf(NLS_LOGICAL, A.getDimensions().getRows());
                            res = ArrayOf(NLS_LOGICAL, Dimensions(A.getDimensions().getRows(), 1), logicalarray);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            res = ArrayOf::emptyConstructor(Dimensions(0, A.getDimensions().getColumns()));
                            res.promoteType(NLS_LOGICAL);
                        }
                    }
                }
        ...
        else if (!A.isEmpty() && !A.isVector())
        {
            ...
            else if (dim == 2)
            {
                ...
                Eigen::Map<Eigen::Matrix<logical, Eigen::Dynamic, Eigen::Dynamic>> matC(logicalarray, mA, 1);

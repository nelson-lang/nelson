...
typedef size_t (*char_trigger)(
    hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t offset, size_t size);
...
static char_trigger markdown_char_ptrs[] = { NULL, &char_emphasis, &char_codespan, &char_linebreak,
    &char_link, &char_image, &char_langle_tag, &char_escape, &char_entity, &char_autolink_url,
    &char_autolink_email, &char_autolink_www, &char_superscript, &char_quote, &char_math };
...
add_link_ref(struct link_ref** references, const uint8_t* name, size_t name_size)
{
...
parse_inline(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size)
{
    ...
    if (doc->work_bufs[BUFFER_SPAN].size + doc->work_bufs[BUFFER_BLOCK].size > doc->max_nesting) {
        return;
    }
    while (i < size) {
...
static size_t
parse_math(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t offset, size_t size,
    const char* end, size_t delimsz, int displaymode)
{
    ...
    while (1) {
        ...
        if (!is_escaped(data, i) && !(i + delimsz > size) && memcmp(data + i, end, delimsz) == 0) {
            break;
        }
        i++;
    }
...
char_escape(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t offset, size_t size)
{
    ...
    size_t w;
    if (size > 1) {
        if (data[1] == '\\' && (doc->ext_flags & HOEDOWN_EXT_MATH) && size > 2
            && (data[2] == '(' || data[2] == '[')) {
    ...
    hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t offset, size_t size)
{
    ...
    if ((link_len = hoedown_autolink__www(
             &rewind, link, data, offset, size, HOEDOWN_AUTOLINK_SHORT_DOMAINS))
        > 0) {
...
static size_t
parse_htmlblock(
    hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size, int do_render);
...
parse_paragraph(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size)
{
    ...
    while (i < size) {
        ...
        if (is_atxheader(doc, data + i, size - i) || is_hrule(data + i, size - i)
...
static size_t
parse_listitem(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size,
...
static size_t
parse_list(
    hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size, hoedown_list_flags flags)
{
    hoedown_buffer* work = 0;
    size_t i = 0, j;
    work = newbuf(doc, BUFFER_BLOCK);
...
static void
parse_footnote_def(
    hoedown_buffer* ob, hoedown_document* doc, unsigned int num, uint8_t* data, size_t size)
{
    hoedown_buffer* work = 0;
    work = newbuf(doc, BUFFER_SPAN);
...
static size_t
htmlblock_is_end(const char* tag, size_t tag_len, hoedown_document* doc, uint8_t* data, size_t size)
{
    size_t i = tag_len + 3, w;
    /* try to match the end tag */
    /* note: we're not considering tags like "</tag >" which are still valid */
    if (i > size || data[1] != '/' || strncasecmp((char*)data + 2, tag, tag_len) != 0
        || data[tag_len + 2] != '>') {
        return 0;
    }
...
static size_t
htmlblock_find_end(
    const char* tag, size_t tag_len, hoedown_document* doc, uint8_t* data, size_t size)
{
    size_t i = 0, w;
    while (1) {
        while (i < size && data[i] != '<') {
...
static void
parse_table_row(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size,
    size_t columns, hoedown_table_flags* col_data, hoedown_table_flags header_flag)
{
    size_t i = 0, col, len;
...
static size_t
parse_table_header(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size,
    size_t* columns, hoedown_table_flags** column_data)
{
    int pipes;
...
static size_t
parse_table(hoedown_buffer* ob, hoedown_document* doc, uint8_t* data, size_t size)
{
    size_t i;
    hoedown_buffer* work = 0;
    ...
    if (i > 0) {
        while (i < size) {
            ...
            parse_table_row(body_work, doc, data + row_start, i - row_start, columns, col_data, 0);
            i++;
        }
...
static void
expand_tabs(hoedown_buffer* ob, const uint8_t* line, size_t size)
{
...
hoedown_document_new(
    const hoedown_renderer* renderer, hoedown_extensions extensions, size_t max_nesting)
{
...
hoedown_document_render_inline(
    hoedown_document* doc, hoedown_buffer* ob, const uint8_t* data, size_t size)
{
    size_t i = 0, mark;

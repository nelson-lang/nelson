...
static size_t char_link(hoedown_buffer *ob, hoedown_document *doc,
                        uint8_t *data, size_t offset, size_t size);
static size_t char_image(hoedown_buffer *ob, hoedown_document *doc,
                         uint8_t *data, size_t offset, size_t size);
...
static size_t parse_math(hoedown_buffer *ob, hoedown_document *doc,
  ...
  if (delimsz == 2 && !(doc->ext_flags & HOEDOWN_EXT_MATH_EXPLICIT)) {
    displaymode =
        is_empty_all(data - offset, offset) && is_empty_all(data + i, size - i);
  }
  /* call callback */
...
static size_t char_autolink_www(hoedown_buffer *ob, hoedown_document *doc,
  ...
  if ((link_len = hoedown_autolink__www(&rewind, link, data, offset, size,
                                        HOEDOWN_AUTOLINK_SHORT_DOMAINS)) > 0) {
...
static size_t char_autolink_email(hoedown_buffer *ob, hoedown_document *doc,
  ...
  link = newbuf(doc, BUFFER_SPAN);
  if ((link_len =
           hoedown_autolink__email(&rewind, link, data, offset, size, 0)) > 0) {
    if (ob->size > rewind) {
      ob->size -= rewind;
    } else {
      ob->size = 0;
...
static size_t char_autolink_url(hoedown_buffer *ob, hoedown_document *doc,
  ...
  link = newbuf(doc, BUFFER_SPAN);
  if ((link_len = hoedown_autolink__url(&rewind, link, data, offset, size, 0)) >
      0) {
    if (ob->size > rewind) {
      ob->size -= rewind;
    } else {
    ...
    while (i < size) {
      if (data[i] == '\\') {
        ...
        i++;
                 ...
                 (data[i] == '\'' ||
                  data[i] == '"')) // lgtm [cpp/constant-comparison]
      {
        break;
      } else {
        i++;
...
static size_t parse_fencedcode(hoedown_buffer *ob, hoedown_document *doc,
  ...
  if (doc->md.blockcode) {
    doc->md.blockcode(ob, text.size ? &text : NULL, lang.size ? &lang : NULL,
                      &doc->data);
  }
  return i;
}
  ...
  while (beg < size) {
    ...
    if ((has_next_uli && !is_hrule(data + beg + i, end - beg - i)) ||
      ...
      if (pre <= orgpre) {
        ...
        if (in_empty && (((*flags & HOEDOWN_LIST_ORDERED) && has_next_uli) ||
...
static size_t parse_table(hoedown_buffer *ob, hoedown_document *doc,
  ...
  if (i > 0) {
    while (i < size) {
      ...
      parse_table_row(body_work, doc, data + row_start, i - row_start, columns,
                      col_data, 0);
      i++;
    }

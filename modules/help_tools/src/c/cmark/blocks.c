...
static bufsize_t
parse_list_marker(cmark_mem* mem, cmark_chunk* input, bufsize_t pos, bool interrupts_paragraph,
    cmark_list** dataptr)
{
    unsigned char c;
    bufsize_t startpos;
...
parse_code_block_prefix(
    cmark_parser* parser, cmark_chunk* input, cmark_node* container, bool* should_continue)
{
    bool res = false;
...
static void
add_text_to_container(cmark_parser* parser, cmark_node* container,
    cmark_node* last_matched_container, cmark_chunk* input)
{
    ...
    const bool last_line_blank = (parser->blank && ctype != CMARK_NODE_BLOCK_QUOTE

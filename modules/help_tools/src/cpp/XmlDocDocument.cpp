...
namespace Nelson {
    ...
    //=============================================================================
    static bool isDir(std::wstring dirname)
    {
    ...
    //=============================================================================
    void XmlDocDocument::clearItems()
    {
    ...
    //=============================================================================
    wstringVector XmlDocDocument::getError()
    {
        return this->errorMessage;
    }
    //=============================================================================
    wstringVector XmlDocDocument::getWarning()
    {
        return this->warningMessage;
    }
    //=============================================================================
    std::wstring XmlDocDocument::getFilename()
    {
        return this->xmlfilename;
    }
    //=============================================================================
    size_t XmlDocDocument::count(std::string tag)
    {
        size_t nbref = 0;
        for (size_t k = 0; k < this->items.size(); k++)
        {
    ...
    //=============================================================================
    boost::container::vector<XmlDocGenericItem *> XmlDocDocument::getXmlDocGenericItems()
    {
        return items;
    }
    //=============================================================================
    bool XmlDocDocument::readFile()
    {
        if (this->bReadOk)
        {
            return true;
        ...
        while (currentNode != nullptr)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(currentNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
        ...
        {
            xmlFreeDoc(doc);
            this->errorMessage.push_back(_W("line ") + std::to_wstring(currentNode->line) + _W(": ") + utf8_to_wstring(XMLDOC_TAG) + L" " + _W("missing."));
            this->bReadOk = false;
            return false;
        }
        if (nbXmlDocTag > 1)
        {
            xmlFreeDoc(doc);
            this->errorMessage.push_back(_W("line ") + std::to_wstring(currentNode->line) + _W(": ") + utf8_to_wstring(XMLDOC_TAG) + L" " + _W("duplicated."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::isKeywordDocument()
    {
        if (this->bReadOk)
        {
    ...
    //=============================================================================
    bool XmlDocDocument::isChapterDocument()
    {
        if (this->bReadOk)
        {
    ...
    //=============================================================================
    bool XmlDocDocument::isTitleDocument()
    {
        if (this->bReadOk)
        {
    ...
    //=============================================================================
    XmlDocGenericItem * XmlDocDocument::findfirst(std::string tag)
    {
        if (count(tag) > 0)
        {
            for (size_t k = 0; k < this->items.size(); k++)
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getPageTitle()
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getPageDescription()
    {
        return this->getShortDescription();
    }
    //=============================================================================
    std::wstring XmlDocDocument::getShortDescription()
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getLanguage()
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getTitle()
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getChapter()
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getKeyword()
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getCopyright()
    {
    ...
    //=============================================================================
    bool XmlDocDocument::haveExample()
    {
        return (count(EXAMPLES_TAG) > 0);
    }
    //=============================================================================
    enum XMLDOC_ITEMS_ID XmlDocDocument::stringTagToId(std::string tag)
    {
    ...
    //=============================================================================
    void XmlDocDocument::writeNavigatorBarAsHtml(std::string &utf8stream)
    {
        if (!this->indexLinkName.empty() || !this->previousLinkName.empty() || !this->nextLinkName.empty())
        {
            ...
            if (!this->previousLinkName.empty())
            {
                ...
                utf8stream = utf8stream + "<span class=\"top\"><a href=\"" + wstring_to_utf8(this->previousLinkUrl) + "\">" + "&lt;&lt; " + wstring_to_utf8(this->previousLinkName) + "</a></span>" + "\n";
            ...
            if (!this->indexLinkName.empty())
            {
                ...
                utf8stream = utf8stream + "<span class=\"top\"><a href=\"" + wstring_to_utf8(this->indexLinkUrl) + "\">" + wstring_to_utf8(this->indexLinkName) + "</a></span>" + "\n";
                utf8stream = utf8stream + "</td>" + "\n";
                utf8stream = utf8stream + "\n";
            }
            if (!this->nextLinkName.empty())
            {
                ...
                utf8stream = utf8stream + "<span class = \"next\"><a href = \"" + wstring_to_utf8(this->nextLinkUrl) + "\">" + wstring_to_utf8(this->nextLinkName) + " &gt;&gt;</a></span>" + "\n";
                utf8stream = utf8stream + "</td>" + "\n";
                utf8stream = utf8stream + "\n";
            }
    ...
    bool XmlDocDocument::writeAsHtml(std::string &utf8stream)
    {
        ...
        // default style
        utf8stream = utf8stream + "<link rel=\"stylesheet\" type=\"text/css\" href=\"style.css\" />" + "\n";
        // header for examples
        if (haveExample())
        {
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseUsedFunction(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(USED_FUNCTION_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        if (node->children == nullptr)
        {
            warningMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(USED_FUNCTION_TAG) + L" " + _W("is empty."));
        }
        else
        {
            ...
            {
                xmlFreeDoc(doc);
                errorMessage.push_back(_W("line ") + std::to_wstring(usedFunctionNode->line) + _W(": ") + utf8_to_wstring(USED_FUNCTION_TAG) + L" " + _W("has child."));
                this->bReadOk = false;
                return false;
            }
            std::string str;
            ...
            if (str.empty())
            {
                warningMessage.push_back(_W("line ") + std::to_wstring(usedFunctionNode->line) + _W(": ") + utf8_to_wstring(USED_FUNCTION_TAG) + L" " + _W("is empty."));
            }
            else
            {
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseSyntax(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocSyntax *syntaxItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(SYNTAX_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (syntaxItemNode != nullptr)
        {
            ...
            else if (currentNodeName == SYNTAX_ITEM_TAG)
            {
                if (syntaxItemNode->content)
                {
                    ...
                    xmlFreeDoc(doc);
                    errorMessage.push_back(_W("line ") + std::to_wstring(syntaxItemNode->line) + _W(": ") + utf8_to_wstring(SYNTAX_ITEM_TAG) + L" " + _W("invalid."));
                    this->bReadOk = false;
                    return false;
                }
                ...
                if (itemNode->children)
                {
                    ...
                    xmlFreeDoc(doc);
                    errorMessage.push_back(_W("line ") + std::to_wstring(itemNode->line) + _W(": ") + utf8_to_wstring(SYNTAX_ITEM_TAG) + L" " + _W("has child."));
                    this->bReadOk = false;
                    return false;
                }
                std::string str;
                ...
                if (str.empty())
                {
                    ...
                    xmlFreeDoc(doc);
                    errorMessage.push_back(_W("line ") + std::to_wstring(itemNode->line) + _W(": ") + utf8_to_wstring(SYNTAX_ITEM_TAG) + L" " + _W("is empty."));
                    this->bReadOk = false;
                    return false;
                }
                else
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(syntaxItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseLanguage(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(LANGUAGE_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(languageNode->line) + _W(": ") + utf8_to_wstring(LANGUAGE_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(languageNode->line) + _W(": ") + utf8_to_wstring(LANGUAGE_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(languageNode->line) + _W(": ") + utf8_to_wstring(LANGUAGE_TAG) + L" " + wlang + _W("not supported."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseTitle(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(TITLE_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(titleNode->line) + _W(": ") + utf8_to_wstring(TITLE_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(titleNode->line) + _W(": ") + utf8_to_wstring(TITLE_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseKeyword(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(KEYWORD_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(keywordNode->line) + _W(": ") + utf8_to_wstring(KEYWORD_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(keywordNode->line) + _W(": ") + utf8_to_wstring(KEYWORD_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseCopyright(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(COPYRIGHT_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(copyrightNode->line) + _W(": ") + utf8_to_wstring(COPYRIGHT_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        if (str.empty())
        {
            warningMessage.push_back(_W("line ") + std::to_wstring(copyrightNode->line) + _W(": ") + utf8_to_wstring(COPYRIGHT_TAG) + L" " + _W("is empty."));
        }
        else
        {
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseDescription(xmlDocPtr doc, xmlNodePtr node)
    {
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(DESCRIPTION_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseShortDescription(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(SHORT_DESCRIPTION_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(shortDescriptionNode->line) + _W(": ") + utf8_to_wstring(SHORT_DESCRIPTION_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(shortDescriptionNode->line) + _W(": ") + utf8_to_wstring(SHORT_DESCRIPTION_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseBibliography(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(BIBLIOGRAPHY_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        if (node->children == nullptr)
        {
            warningMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(BIBLIOGRAPHY_TAG) + L" " + _W("is empty."));
        }
        else
        {
            ...
            {
                xmlFreeDoc(doc);
                errorMessage.push_back(_W("line ") + std::to_wstring(bibliogragphyNode->line) + _W(": ") + utf8_to_wstring(BIBLIOGRAPHY_TAG) + L" " + _W("has child."));
                this->bReadOk = false;
                return false;
            }
            std::string str;
            ...
            if (str.empty())
            {
                warningMessage.push_back(_W("line ") + std::to_wstring(bibliogragphyNode->line) + _W(": ") + utf8_to_wstring(BIBLIOGRAPHY_TAG) + L" " + _W("is empty."));
            }
            else
            {
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseAuthors(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocAuthors *authorsItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(AUTHORS_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (authorItemNode != nullptr)
        {
            ...
            else if (currentNodeName == AUTHOR_ITEM_TAG)
            {
                if (authorItemNode->content)
                {
                    ...
                    xmlFreeDoc(doc);
                    errorMessage.push_back(_W("line ") + std::to_wstring(authorItemNode->line) + _W(": ") + utf8_to_wstring(AUTHOR_ITEM_TAG) + L" " + _W("invalid."));
                    this->bReadOk = false;
                    return false;
                }
                ...
                if (itemNode->children)
                {
                    ...
                    xmlFreeDoc(doc);
                    errorMessage.push_back(_W("line ") + std::to_wstring(itemNode->line) + _W(": ") + utf8_to_wstring(AUTHOR_ITEM_TAG) + L" " + _W("has child."));
                    this->bReadOk = false;
                    return false;
                }
                std::string str;
                ...
                if (str.empty())
                {
                    ...
                    xmlFreeDoc(doc);
                    errorMessage.push_back(_W("line ") + std::to_wstring(itemNode->line) + _W(": ") + utf8_to_wstring(AUTHOR_ITEM_TAG) + L" " + _W("is empty."));
                    this->bReadOk = false;
                    return false;
                }
                else
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(authorItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseHistory(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocHistory *historyItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(HISTORY_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (historyItemNode != nullptr)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(historyItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
            else
            {
                ...
                while (currentItemNode != nullptr)
                {
                    ...
                    else if (currentItemNodeName == HISTORY_ITEM_VERSION_TAG)
                    {
                        ...
                        if (versionNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(versionNode->line) + _W(": ") + utf8_to_wstring(HISTORY_ITEM_VERSION_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(versionNode->line) + _W(": ") + utf8_to_wstring(HISTORY_ITEM_VERSION_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
                    ...
                    else if (currentItemNodeName == HISTORY_ITEM_DESCRIPTION_TAG)
                    {
                        ...
                        if (descriptionNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(HISTORY_ITEM_DESCRIPTION_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(HISTORY_ITEM_VERSION_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
                    ...
                    {
                        xmlFreeDoc(doc);
                        this->errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(currentItemNodeName) + L" " + _W("not managed."));
                        this->bReadOk = false;
                        return false;
                    }
                ...
                if (nbdescriptiontag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(HISTORY_ITEM_DESCRIPTION_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
                if (nbversiontag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(HISTORY_ITEM_VERSION_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseParamInput(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocParamInput *paramInputItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(PARAM_INPUT_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (paramInputItemNode != nullptr)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(paramInputItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
            else
            {
                    ...
                    else if (currentItemNodeName == PARAM_NAME_TAG)
                    {
                        ...
                        if (nameNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(nameNode->line) + _W(": ") + utf8_to_wstring(PARAM_NAME_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(nameNode->line) + _W(": ") + utf8_to_wstring(PARAM_NAME_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
                    ...
                    else if (currentItemNodeName == PARAM_DESCRIPTION_TAG)
                    {
                        ...
                        if (descriptionNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(PARAM_DESCRIPTION_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(PARAM_DESCRIPTION_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
                ...
                if (nbdescriptiontag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(paramInputItemNode->line) + _W(": ") + utf8_to_wstring(PARAM_DESCRIPTION_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
                if (nbnametag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(paramInputItemNode->line) + _W(": ") + utf8_to_wstring(PARAM_NAME_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseParamOutput(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocParamOutput *paramOutputItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(PARAM_OUTPUT_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (paramOutputItemNode != nullptr)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(paramOutputItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
            else
            {
                ...
                if (nbdescriptiontag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(paramOutputItemNode->line) + _W(": ") + utf8_to_wstring(PARAM_DESCRIPTION_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
                if (nbnametag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(paramOutputItemNode->line) + _W(": ") + utf8_to_wstring(PARAM_NAME_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseExamples(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocExamples *examplesItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(EXAMPLES_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (exampleItemNode != nullptr)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(exampleItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
            else
            {
                ...
                while (currentItemNode != nullptr)
                {
                    ...
                    else if (currentItemNodeName == EXAMPLE_ITEM_TYPE_TAG)
                    {
                        ...
                        if (nameNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(nameNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_TYPE_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(nameNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_TYPE_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
                    ...
                    else if (currentItemNodeName == EXAMPLE_ITEM_DESCRIPTION_TAG)
                    {
                        ...
                        if (descriptionNode)
                        {
                            ...
                            if (str.empty())
                            {
                                warningMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DESCRIPTION_TAG) + L" " + _W("is empty."));
                            }
                            description = utf8_to_wstring(str);
                            if (nbdescriptiontag == 0)
                            {
                            ...
                            else
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DESCRIPTION_TAG) + L" " + _W("duplicated."));
                                this->bReadOk = false;
                                return false;
                            }
                        }
                        ...
                        {
                            warningMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DESCRIPTION_TAG) + L" " + _W("is empty."));
                            description = L"";
                            if (nbdescriptiontag == 0)
                            {
                                nbdescriptiontag++;
                            ...
                            else
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DESCRIPTION_TAG) + L" " + _W("duplicated."));
                                this->bReadOk = false;
                                return false;
                            }
                        }
                    ...
                    else if (currentItemNodeName == EXAMPLE_ITEM_IMG_TAG)
                    {
                        ...
                        if (!isValidImageTag(imagetag))
                        {
                            ...
                            xmlFreeDoc(doc);
                            errorMessage.push_back(_W("line ") + std::to_wstring(dataNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_IMG_TAG) + L" " + _W("malformed."));
                            this->bReadOk = false;
                            return false;
                        }
                        std::wstring newPath;
                        ...
                        if (!parseImageTag(imagetag, this->xmlDirectory, oldPath, newPath))
                        {
                            ...
                            xmlFreeDoc(doc);
                            errorMessage.push_back(_W("line ") + std::to_wstring(dataNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_IMG_TAG) + L" " + _W("file does not exist."));
                            this->bReadOk = false;
                            return false;
                        }
                        if (nbimgtag == 0)
                        ...
                        else
                        {
                            ...
                            xmlFreeDoc(doc);
                            errorMessage.push_back(_W("line ") + std::to_wstring(dataNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_IMG_TAG) + L" " + _W("duplicated."));
                            this->bReadOk = false;
                            return false;
                        }
                    }
                    else if (currentItemNodeName == EXAMPLE_ITEM_DATA_TAG)
                    {
                        ...
                        if (dataNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(dataNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DATA_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(dataNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DATA_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
                ...
                if (nbtypetag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_TYPE_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
                if (nbdescriptiontag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DESCRIPTION_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
                if (nbdatatag == 0)
                {
                    ...
                    xmlFreeDoc(doc);
                    this->errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(EXAMPLE_ITEM_DATA_TAG) + L" " + _W("missing."));
                    this->bReadOk = false;
                    return false;
                }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseSeeAlso(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocSeeAlso *seeAlsoItems = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(SEE_ALSO_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (seeAlsoItemNode != nullptr)
        {
            ...
            else if (currentNodeName != SEE_ALSO_ITEM_TAG)
            {
                ...
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(seeAlsoItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
            else
            {
                ...
                while (currentItemNode != nullptr)
                {
                    ...
                    if (nbLinksItems > 0)
                    {
                        ...
                        xmlFreeDoc(doc);
                        errorMessage.push_back(_W("line ") + std::to_wstring(currentItemNode->line) + _W(": ") + utf8_to_wstring(XML_LINK_TAG) + L" " + _W("duplicated."));
                        this->bReadOk = false;
                        return false;
                    }
                    if (currentItemNodeName == XML_COMMENT_TAG)
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseChapter(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(CHAPTER_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(chapterNode->line) + _W(": ") + utf8_to_wstring(CHAPTER_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(chapterNode->line) + _W(": ") + utf8_to_wstring(CHAPTER_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseChapterDescription(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(CHAPTER_DESCRIPTION_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(chapterDescriptionNode->line) + _W(": ") + utf8_to_wstring(CHAPTER_DESCRIPTION_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        if (str.empty())
        {
            warningMessage.push_back(_W("line ") + std::to_wstring(chapterDescriptionNode->line) + _W(": ") + utf8_to_wstring(CHAPTER_DESCRIPTION_TAG) + L" " + _W("is empty."));
        }
        else
        {
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseChapterIndex(xmlDocPtr doc, xmlNodePtr node)
    {
        XmlDocChapterIndexItem *chapterIndexItem = nullptr;
        if (node->children == nullptr)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(CHAPTER_INDEX_TAG) + L" " + _W("has no child."));
            this->bReadOk = false;
            return false;
        }
        try
        ...
        while (chapterIndexItemNode != nullptr)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(chapterIndexItemNode->line) + _W(": ") + utf8_to_wstring(currentNodeName) + L" " + _W("not managed."));
                this->bReadOk = false;
                return false;
            }
            else
                    ...
                    else if (currentItemNodeName == CHAPTER_REF_DESCRIPTION_TAG)
                    {
                        ...
                        if (descriptionNode)
                        {
                            ...
                            if (str.empty())
                            {
                                ...
                                xmlFreeDoc(doc);
                                errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(CHAPTER_REF_DESCRIPTION_TAG) + L" " + _W("is empty."));
                                this->bReadOk = false;
                                return false;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    xmlFreeDoc(doc);
                                    errorMessage.push_back(_W("line ") + std::to_wstring(descriptionNode->line) + _W(": ") + utf8_to_wstring(CHAPTER_REF_DESCRIPTION_TAG) + L" " + _W("duplicated."));
                                    this->bReadOk = false;
                                    return false;
                                }
                            }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseLink(xmlDocPtr doc, xmlNodePtr node, std::wstring &name, std::wstring &url)
    {
        name = L"";
        url = L"";
        ...
        if (node->properties)
        {
            ...
            {
                xmlFreeDoc(doc);
                this->errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(XML_LINK_TAG) + L" " + _W("has no property."));
                this->bReadOk = false;
                return false;
            }
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseImage(xmlDocPtr doc, xmlNodePtr node)
    {
        xmlBufferPtr buffer = xmlBufferCreate();
    ...
    //=============================================================================
    bool XmlDocDocument::readFileCaseModuleName(xmlDocPtr doc, xmlNodePtr node)
    {
        if (node->content)
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(node->line) + _W(": ") + utf8_to_wstring(MODULE_NAME_TAG) + L" " + _W("invalid."));
            this->bReadOk = false;
            return false;
        }
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(moduleNameNode->line) + _W(": ") + utf8_to_wstring(MODULE_NAME_TAG) + L" " + _W("has child."));
            this->bReadOk = false;
            return false;
        }
        std::string str;
        ...
        {
            xmlFreeDoc(doc);
            errorMessage.push_back(_W("line ") + std::to_wstring(moduleNameNode->line) + _W(": ") + utf8_to_wstring(MODULE_NAME_TAG) + L" " + _W("is empty."));
            this->bReadOk = false;
            return false;
        }
    ...
    //=============================================================================
    void XmlDocDocument::setDestinationFile(std::wstring _filenameDestination)
    {
    ...
    //=============================================================================
    std::wstring XmlDocDocument::getDestinationFile()
    {
        return this->filenameDestination;
    }
    //=============================================================================
    bool XmlDocDocument::copyHtmlDependencies()
    {
        ...
        if (!helpToolsPath.empty())
        {
            ...
            for (size_t k = 0; k < files.size(); k++)
            {
                ...
                if (!boost::filesystem::exists(dstFile))
                {
                    ...
                    bool bIsFile = boost::filesystem::exists(srcFile) && !boost::filesystem::is_directory(srcFile);
                    if (bIsFile)
                    {
    ...
    //=============================================================================
    bool XmlDocDocument::needToUpdate()
    {
    ...
    //=============================================================================
    void XmlDocDocument::setPreviousPageLink(std::wstring linkname, std::wstring linkurl)
    {
        this->previousLinkName = linkname;
        this->previousLinkUrl = linkurl;
    }
    //=============================================================================
    void XmlDocDocument::setNextPageLink(std::wstring linkname, std::wstring linkurl)
    {
        this->nextLinkName = linkname;
        this->nextLinkUrl = linkurl;
    }
    //=============================================================================
    void XmlDocDocument::setIndexPageLink(std::wstring linkname, std::wstring linkurl)
    {
        this->indexLinkName = linkname;
        this->indexLinkUrl = linkurl;
    }
    //=============================================================================
    std::wstring XmlDocDocument::getChapterDescription()
    {
        XmlDocGenericItem *pItem = findfirst(CHAPTER_DESCRIPTION_TAG);
        if (pItem)
        {
            XmlDocChapterDescriptionItem *pChapterDescriptionItem = (XmlDocChapterDescriptionItem *)pItem;
            return pChapterDescriptionItem->getValue();
        }
        return L"";
    }
    //=============================================================================
    std::wstring XmlDocDocument::getModuleName()
    {

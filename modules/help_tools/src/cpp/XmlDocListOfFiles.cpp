...
namespace Nelson {
    ...
    //=============================================================================
    void XmlDocListOfFiles::clearItems()
    {
    ...
    bool XmlDocListOfFiles::read()
    {
            ...
            if (xmlDoc)
            {
                if (xmlDoc->readFile())
                {
                    if (xmlDoc->isChapterDocument())
                    {
                        ...
                        boost::container::vector<XmlDocGenericItem *> items = xmlDoc->getXmlDocGenericItems();
                        XmlDocChapterIndexItem *indexItem = new XmlDocChapterIndexItem();
                        for (size_t k = 0; k < xmlItems.size(); k++)
                        {
                            bool bSuccess;
                            indexItem->append(xmlItems[k]->getPageTitle(), RelativePath(dstDirectory, xmlItems[k]->getDestinationFile(), bSuccess), xmlItems[k]->getPageDescription());
                        }
        ...
        for (size_t k = 0; k < xmlItems.size(); k++)
        {
            if (k > 0)
            {
                ...
                bool bRes;
                std::wstring linkUrl = RelativePath(dstDirectory, xmlItems[k - 1]->getDestinationFile(), bRes);
    ...
    //=============================================================================
    bool XmlDocListOfFiles::writeAsHtml()
    {
        for (size_t k = 0; k < this->xmlItems.size(); k++)
        {
            if (this->xmlItems[k])
    ...
    //=============================================================================
    std::wstring XmlDocListOfFiles::getLastError()
    {
        return this->lastError;
    }
    //=============================================================================
    std::wstring XmlDocListOfFiles::getGeneratedChapterFilename()
    {
        return this->chapterResultFilename;
    }
    //=============================================================================
    std::wstring XmlDocListOfFiles::getChapterTitle()
    {
        return this->chapterTitle;
    }
    //=============================================================================
    std::wstring XmlDocListOfFiles::getModuleName()
    {
        return this->moduleName;
    }
    //=============================================================================
    void XmlDocListOfFiles::getIndex(wstringVector &names, wstringVector &urls, wstringVector &descriptions)
    {
        bool bSuccess = false;
        for (size_t k = 0; k < xmlItems.size(); k++)
        {
    ...
    //=============================================================================
    void XmlDocListOfFiles::setUpSection(std::wstring sectionName, std::wstring sectionUrl)
    {

...
//=============================================================================
ArrayOfVector Nelson::HelpToolsGateway::xmldoccheckerBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    if (arg1.isSingleString())
    {
        ...
        try
        {
            IsFileIn = boost::filesystem::exists(pathIn) && !boost::filesystem::is_directory(pathIn);
        }
        ...
        {
            Error(eval, _W("Wrong value for argument #1: An existing .xml documentation file expected."));
        }
    }
    else
    {

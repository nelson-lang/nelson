...
namespace Nelson {
	...
	//=============================================================================
	static bool convertToJsonVariable(const jsmntok_t& token, bool convertInfNan, JsonVariable &jsVar)
	{
		bool res = false;
	...
	//=============================================================================
	static ArrayOf jsonVariableToNelson(JsonVariable jsVar);
	//=============================================================================
	static ArrayOf jsonVariableToNelsonStringType(JsonVariable jsVar)
	{
	...
	//=============================================================================
	static ArrayOf jsonVariableToNelsonLogicalType(JsonVariable jsVar)
	{
		switch (jsVar.dims.size())
		{
		case 0:
	...
	//=============================================================================
	static ArrayOf jsonVariableToNelsonDoubleType(JsonVariable jsVar)
	{
		switch (jsVar.dims.size())
		{
		case 0:
		...
		{
			Dimensions dims(jsVar.dims[0], 1);
			double *dptr = (double *)ArrayOf::allocateArrayOf(NLS_DOUBLE, dims.getElementCount(), stringVector(), false);
	...
	//=============================================================================
	static ArrayOf jsonVariableToNelsonStructType(JsonVariable jsVar, Dimensions dims)
	{
		stringVector fieldnames = jsVar.fieldnames;
		ArrayOf* ptrStruct = (ArrayOf*)ArrayOf::allocateArrayOf(NLS_STRUCT_ARRAY, dims.getElementCount(), fieldnames);
		size_t  offset = 0;
	...
	//=============================================================================
	static ArrayOf jsonVariableToNelsonStructType(JsonVariable jsVar)
	{
		switch (jsVar.dims.size())
		{
			case 0:
	...
	//=============================================================================
	static ArrayOf jsonVariableToNelsonCellType(JsonVariable jsVar)
	{
	...
	//=============================================================================
	static JSON_TO_NELSON_Type findCommonJsonVariableType(JsonVariable jsVar)
	{
	...
	static bool transformStringArray(JsonVariable &jsVar, unsigned int totaldims)
	{
		...
		case 1:
		{
			...
			std::transform(jsVar.vectorJsonVariable.begin(), jsVar.vectorJsonVariable.end(), jsVar.vectorString.begin(), [](JsonVariable val) { return val.scalarString; });
		}
		break;
		case 2:
		{
	...
	static bool transformLogicalArray(JsonVariable &jsVar, unsigned int totaldims)
	{
		...
		case 1:
		{
			...
			std::transform(jsVar.vectorJsonVariable.begin(), jsVar.vectorJsonVariable.end(), jsVar.vectorLogical.begin(), [](JsonVariable val) { return val.scalarLogical; });
		}
		break;
		case 2:
		{
	...
	static bool transformDoubleArray(JsonVariable &jsVar, unsigned int totaldims)
	{
		...
		case 1:
		{
			...
			std::transform(jsVar.vectorJsonVariable.begin(), jsVar.vectorJsonVariable.end(), jsVar.vectorDouble.begin(), [](JsonVariable val) { return val.scalarDouble; });
		}
		break;
		case 2:
		{
	...
	static bool transformStructArray(JsonVariable &jsVar, unsigned int totaldims)
	{
		...
		for (int i = 1; i < jsVar.vectorJsonVariable.size(); ++i)
		{
			...
			{
				if (std::find(fieldnamesRef.begin(), fieldnamesRef.end(), element) == fieldnamesRef.end())
				{
					jsVar.reduced = true;
					jsVar.jsonVariableType = JSON_TO_NELSON_CELL;
					return true;
			...
			default:
			{
				for (auto name : fieldnamesRef)
				{
					for (auto var : jsVar.vectorJsonVariable)
					{
						if (var.dims.size() != 0)
						{
							jsVar.map[name].insert(jsVar.map[name].end(), var.map[name].begin(), var.map[name].end());
						}
						else
						{
	...
	//=============================================================================
	static bool importTokens(const jsmntok_t* tokens, bool convertInfNan, JsonVariable &jsVar)
	{
		bool res = false;
		...
		case JSMN_ARRAY:
		{
			...
			for (int i = 0; i < size; ++i)
			{
				...
				bool ok = importTokens(tokens, convertInfNan, jsElement);
				if (!ok)
				{
					return ok;
				}
		...
		case JSMN_OBJECT:
		{
			...
			{
				JsonVariable jsKey;
				bool ok = importTokens(tokens, convertInfNan, jsKey);
				if (!ok)
				{
					return ok;
				}
				...
				ok = importTokens(tokens, convertInfNan, jsValue);
				if (!ok)
				{
					return ok;
				}
				...
				else
				{
					...
					do
					{
						...
						auto it = std::find(jsVar.fieldnames.begin(), jsVar.fieldnames.end(), modifiedKey);
		...
		case JSMN_PRIMITIVE:
		{
			return convertToJsonVariable(tokens[tokens_offset++], convertInfNan, jsVar);
		}
		break;
		}
		return res;
	...
	//=============================================================================
	static ArrayOf jsonDecodeInternal(std::wstring stringToDecode, bool convertInfNan, std::wstring &errorMessage)
    {
		...
		else if (nbTokensOrError > 0)
        {
            ...
            int nbTokensUsed = jsmn_parse(&parserJson, jsonString.c_str(), jsonString.size(), tokens, nbTokensOrError + 1);
			if (nbTokensUsed == 0)
			{
				delete[] tokens;
				errorMessage = _W("valid JSON Object expected.");
			...
			else
			{
				tokens_offset = 0;
				JsonVariable jsVar;
				bool converted = importTokens(tokens, convertInfNan, jsVar);
    ...
    //=============================================================================
	ArrayOf jsonDecode(std::wstring stringToDecode, bool convertInfNan, std::wstring &errorMessage)
	{
		tokens_offset = 0;
		jsonString = "";
		ArrayOf res = jsonDecodeInternal(stringToDecode, convertInfNan, errorMessage);
		tokens_offset = 0;
		jsonString = "";
		return res;
	}

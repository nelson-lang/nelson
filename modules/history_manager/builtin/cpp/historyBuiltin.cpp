...
//=============================================================================
static ArrayOfVector historyBuiltin_size_one_rhs(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    if (arg1.isSingleString())
    {
        ...
        if (str == L"size")
        {
            retval.push_back(ArrayOf::doubleConstructor((double)ptrHistoryManager->getLastNCommandsSize()));
        }
        ...
        {
            retval.push_back(ArrayOf::logicalConstructor(ptrHistoryManager->getAllowDuplicatedLines()));
        }
        else if (str == L"saveafter")
        {
            retval.push_back(ArrayOf::doubleConstructor((double)ptrHistoryManager->getSaveAfterNCommands()));
        }
...
//=============================================================================
static ArrayOfVector historyBuiltin_no_rhs(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ArrayOfVector retval;
    HistoryManager *ptrHistoryManager = (HistoryManager *)eval->HistoryManager;
    wstringVector res = ptrHistoryManager->get();
...
//=============================================================================
static ArrayOfVector historyBuiltin_two_rhs(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ArrayOfVector retval;
    ArrayOf arg1 = argIn[0];
    ...
    if (arg1.isSingleString())
    {
        ...
        if (str == L"size")
        {
            ...
            if (arg2.isScalar())
            {
                if (arg2.getDataClass() == NLS_DOUBLE)
                {
                    ...
                    if (dvalue < 0)
                    {
                        Error(eval, ERROR_WRONG_ARGUMENT_2_POSITIVE_VALUE_EXPECTED);
                    }
            ...
            if (arg2.getDataClass() == NLS_DOUBLE)
            {
                ...
                else
                {
                    ...
                    if (sze.equals(supported))
                    {
                        ...
                        if (ivalue2 <= ivalue1)
                        {
                            Error(eval, ERROR_WRONG_ARGUMENT_2_A_MUST_BE_HIGHER_THAN_B);
                        }
                        if ((ivalue1 >= ptrHistoryManager->getCurrentSize()) || (ivalue2 >= ptrHistoryManager->getCurrentSize()))
                        {
                            Error(eval, ERROR_WRONG_ARGUMENT_2_A_MUST_BE_HIGHER_THAN_B);
                        }
                        else
                    ...
                    {
                        Error(eval, ERROR_WRONG_ARGUMENT_2_SIZE_A_B_VECTOR_EXPECTED);
                    }
                }
            }
            else
        ...
        else if (str == L"append")
        {
            ...
            {
                Error(eval, ERROR_WRONG_ARGUMENT_2_TYPE_STRING_OR_CELL_EXPECTED);
            }
        }
        else if (str == L"filename")
        {
...
//=============================================================================
ArrayOfVector Nelson::HistoryManagerGateway::historyBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ArrayOfVector retval;
    if (eval->HistoryManager == nullptr)
    {
        Error(eval, _W("History manager not enabled."));
    }
    if (nLhs > 1)
    {
        Error(eval, ERROR_WRONG_NUMBERS_OUTPUT_ARGS);

...
namespace Nelson {
    //=============================================================================
    static bool ComVariantToScalarDouble(VARIANT variant, double &value, std::wstring &errorMessage)
    {
        errorMessage = L"";
    ...
    //=============================================================================
    bool ComVariantToNelson(VARIANT *variant, ArrayOf &res, std::wstring &errorMessage)
    {
        errorMessage = L"";
        switch (variant->vt)
        {
            ...
            case VT_DATE:
            {
                VARIANT variantConverted;
                VariantInit(&variantConverted);
                if (SUCCEEDED(VariantChangeType(&variantConverted, variant, VARIANT_NOUSEROVERRIDE, VT_BSTR)))
                {
            ...
            case VT_INT:
            {
                VARIANT variantConverted;
                VariantInit(&variantConverted);
                if (SUCCEEDED(VariantChangeType(&variantConverted, variant, VARIANT_NOUSEROVERRIDE, VT_I4)))
                {
            ...
            case VT_UINT:
            {
                VARIANT variantConverted;
                VariantInit(&variantConverted);
                if (SUCCEEDED(VariantChangeType(&variantConverted, variant, VARIANT_NOUSEROVERRIDE, VT_UI4)))
                {
                ...
                if (variant->vt & VT_ARRAY)
                {
                    ...
                    switch (subtype)
                    {
                        case VT_VARIANT:
                        {
                            VARIANT *pvar;
                            HRESULT hr;
                            ArrayOf* pCell = (ArrayOf*)ArrayOf::allocateArrayOf(NLS_CELL_ARRAY, dims.getElementCount());
                        ...
                        case VT_R8:
                        {
                            ...
                            double* pDouble= (double*)ArrayOf::allocateArrayOf(NLS_DOUBLE, dims.getElementCount(), Nelson::stringVector(), true);
                            hr = SafeArrayAccessData(arr, (void **)&pvar);
                            if (FAILED(hr))
                            {
                                errorMessage = L"Failed accessing array data.";
                        ...
                        case VT_UI1:
                        {
                            ...
                            uint8* pUint8 = (uint8*)ArrayOf::allocateArrayOf(NLS_UINT8, dims.getElementCount(), Nelson::stringVector(), true);
                            hr = SafeArrayAccessData(arr, (void **)&pvar);
                            if (FAILED(hr))
                            {
                                errorMessage = L"Failed accessing array data.";
                        ...
                        case VT_I2:
                        {
                            ...
                            int16* pInt16 = (int16*)ArrayOf::allocateArrayOf(NLS_INT16, dims.getElementCount(), Nelson::stringVector(), true);
                            hr = SafeArrayAccessData(arr, (void **)&pvar);
                            if (FAILED(hr))
                            {
                                errorMessage = L"Failed accessing array data.";
    ...
    //=============================================================================
    static SAFEARRAY* makeSafeArrayFromDimensions(Dimensions dims, VARTYPE vt)
    {
        SAFEARRAYBOUND *bounds = (SAFEARRAYBOUND*)LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT, sizeof(SAFEARRAYBOUND)*dims.getLength());
        if (bounds)
        {
    ...
    //=============================================================================
    bool NelsonToComVariant(ArrayOf A, VARIANT* variant, std::wstring &errorMessage)
    {
        errorMessage = L"";

...
//=============================================================================
ArrayOfVector Nelson::ConstructorsGateway::epsBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    else
    {
        ...
        else if (argIn[0].getDataClass() == NLS_SINGLE || argIn[0].getDataClass() == NLS_SCOMPLEX)
        {
            if (!argIn[0].isScalar())
            {
                Error(eval, ERROR_WRONG_ARGUMENT_1_SIZE_SCALAR_EXPECTED);

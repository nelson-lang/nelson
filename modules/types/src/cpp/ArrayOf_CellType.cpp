...
namespace Nelson {
                ...
                */
                ...
                else
                {
                    ...
                    */
                    ...
                    {
                        throw Exception(_W("Cell definition must have same number of elements in each row"));
                    }
                }
                ++i;
            }
    ...
    */
    ArrayOf ArrayOf::getVectorContents(ArrayOf& indexing)
    {
        if (this->isCell())
        {
    ...
    */
    ArrayOf ArrayOf::getNDimContents(ArrayOfVector& indexing)
    {
        if (!this->isCell())
        {
            throw Exception(_W("Attempt to apply contents-indexing to non-cell array object."));
    ...
    */
    ArrayOfVector ArrayOf::getVectorContentsAsList(ArrayOf& index)
    {
        ArrayOfVector m;
        if (!this->isCell())
        {
    ...
    */
    ArrayOfVector ArrayOf::getNDimContentsAsList(ArrayOfVector& index)
    {
        if (!this->isCell())
        {
            throw Exception(_W("Attempt to apply contents-indexing to non cell-array object."));
    ...
    */
    void ArrayOf::setVectorContents(ArrayOf& index, ArrayOf& data)
    {
        ...
        {
            throw Exception(_W("In expression A{I} = B, I must reference a single element of cell-array A."));
        }
        constIndexPtr index_p = (constIndexPtr)index.dp->getData();
        if (*index_p == 0)
        {
    ...
    */
    void ArrayOf::setNDimContents(ArrayOfVector& index, ArrayOf& data)
    {
        promoteType(NLS_CELL_ARRAY, data.dp->fieldNames);
        if (isSparse())
        {
        ...
        for (i = 0; i<L; i++)
        {
            ...
            if (!index[i].isScalar())
            {
                throw Exception(_W("In expression A{I1,I2,...,IN} = B, (I1,...,IN) must reference a single element of cell-array A."));
            }
    ...
    *   2. Deletions do not occur.
    */
    void ArrayOf::setVectorContentsAsList(ArrayOf& index, ArrayOfVector& data)
    {
        if (isSparse())
        {
        ...
        {
            throw Exception(_W("Not enough right hand side values to satisy left hand side expression."));
        }
        // Get the maximum index
        indexType max_index = index.getMaxAsIndex();
    ...
    * This is for content-based indexing (curly brackets).
    */
    void ArrayOf::setNDimContentsAsList(ArrayOfVector& index, ArrayOfVector& data)
    {
        if (isSparse())
        {
        ...
        try
        {
            ...
            {
                throw Exception(_W("Not enough right hand side values to satisfy left hand side expression"));
            }
            // Resize us as necessary
            resize(a);

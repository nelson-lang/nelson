...
namespace Nelson {
    //=============================================================================
    boost::container::vector<FileInfo> ListFilesWithWildcard(std::wstring mask, bool bSubdirectories)
    {
        ...
        else
        {
            ...
            if (boost::filesystem::is_directory(branch))
            {
                ...
                for (const std::pair<boost::wregex, const wchar_t*>* r = repl; r < repl + sizeof(repl) / sizeof(*repl); ++r)
                {
    ...
    boost::container::vector<FileInfo> ListFiles(const std::wstring &directory, bool bSubdirectories)
    {
        ...
        bool bWithWildCard = (foundstar != std::string::npos) || (foundInterrogationMark != std::string::npos);
        ...
        else
        {
            ...
            else
            {
                ...
                if (boost::algorithm::ends_with(directory, L"/") || boost::algorithm::ends_with(directory, L"\\"))
                {
                ...
                if (bSubdirectories)
                {
                    if (IsDirectory(branch.wstring()))
                    {
                        try
                        {
                            for (boost::filesystem::recursive_directory_iterator dir_iter(branch), end; dir_iter != end; ++dir_iter)
                            {
                ...
                else
                {
                    ...
                    if (IsDirectory(directory))
                    {
                        ...
                        try
                        {
                            for (boost::filesystem::directory_iterator dir_iter(directory), end; dir_iter != end; ++dir_iter)
                            {
                                boost::filesystem::path current = dir_iter->path();
                                res.push_back(FileInfo(current.wstring(), bSubdirectories));
                            }

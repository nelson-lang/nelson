...
//=============================================================================
ArrayOfVector Nelson::FilesFoldersGateway::dirBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    if (nLhs == 0)
    {
        ...
        if (res.empty())
        {
            std::wstring msg = std::wstring(L"\'") + wpath + std::wstring(L"\' ") + _W("Not a file or a directory.");
            io->outputMessage(msg);
        }
        else
        {
            for (boost::container::vector<FileInfo>::iterator it = res.begin(); it != res.end(); ++it)
            {
    ...
    else
    {
        ...
        {
            ArrayOf *elements = (ArrayOf*)ArrayOf::allocateArrayOf(NLS_STRUCT_ARRAY, dims.getElementCount(),fieldnames);

...
namespace Nelson {
    ...
    void InitGuiObjectsDynamic(void)
    {
        ...
        {
            InitGuiObjectsPtr = reinterpret_cast<PROC_InitGuiObjects>(get_function(nlsGuiHandleDynamicLibrary, "InitGuiObjects"));
            if (!InitGuiObjectsPtr)
            {
    ...
    void * CreateGuiEvaluatorDynamic(void * vcontext, NELSON_ENGINE_MODE _mode)
    {
        ...
        {
            CreateGuiEvaluatorPtr = reinterpret_cast<PROC_CreateGuiEvaluator>(get_function(nlsGuiHandleDynamicLibrary, "CreateGuiEvaluator"));
            if (!CreateGuiEvaluatorPtr)
            {
                std::string msg = _("Gui Function not loaded.") + std::string("\n");
                fprintf(stderr, "%s",  msg.c_str());
    ...
    void DestroyMainGuiObjectDynamic(void *term)
    {
        ...
        {
            DestroyMainGuiObjectPtr = reinterpret_cast<PROC_DestroyMainGuiObject>(get_function(nlsGuiHandleDynamicLibrary, "DestroyMainGuiObject"));
            if (!DestroyMainGuiObjectPtr)
            {
                std::string msg = _("Gui Function not loaded.") + std::string("\n");
                fprintf(stderr, "%s", msg.c_str());
    ...
    void * GetMainGuiObjectDynamic(void)
    {
        ...
        {
            GetMainGuiObjectPtr = reinterpret_cast<PROC_GetMainGuiObject>(get_function(nlsGuiHandleDynamicLibrary, "GetMainGuiObject"));
            if (!GetMainGuiObjectPtr)
            {
                std::string msg = _("Gui Function not loaded.") + std::string("\n");
                fprintf(stderr, "%s", msg.c_str());

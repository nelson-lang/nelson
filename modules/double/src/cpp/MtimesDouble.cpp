...
namespace Nelson {
    //=============================================================================
    static ArrayOf double_mtimes(ArrayOf a, ArrayOf  b)
    {
        Dimensions Cdim;
        ...
        if (a.isScalar())
        {
            ...
            matC = a.getContentsAsDoubleScalar() * matB.array();
        }
        else if (b.isScalar())
        {
            Eigen::Map<Eigen::MatrixXd> matA((double*)a.getDataPointer(), mA, nA);
            matC = matA.array() * b.getContentsAsDoubleScalar();
        }
        else
        {
            Eigen::Map<Eigen::MatrixXd> matA((double*)a.getDataPointer(), mA, nA);
    ...
    //=============================================================================
    static ArrayOf dcomplex_mtimes(ArrayOf a, ArrayOf  b)
    {
        Dimensions Cdim;
        ...
        if (a.isScalar() && b.isScalar())
        {
            ...
            if ((cxa.real() == 0.) && (cxa.imag() == 0.) ||
                    (cxb.real() == 0.) && (cxb.imag() == 0.))
            {
                double *pd = (double*)Cp;
                delete[] pd;
                pd = nullptr;
    ...
    ArrayOf double_mtimes_double(ArrayOf a, ArrayOf b)
    {
        ...
        if (a.isEmpty() || b.isEmpty())
        {
            ...
            if ((dimsA[1] == 0) && (dimsB[0] == 0) && (dimsB.getLength() < 3))
            {
                ...
                double *pDouble = (double *)ArrayOf::allocateArrayOf(NLS_DOUBLE, dimsC.getElementCount());
        ...
        bool isVector = ((a.isVector() && b.isScalar()) ||
                         (b.isVector() && a.isScalar()) ||
                         (a.isRowVector() && b.isColumnVector()) ||
                         (b.isRowVector() && a.isColumnVector()));
        if (!(SameSizeCheck(a.getDimensions(), b.getDimensions()) || a.isScalar() || b.isScalar()) && !isVector && a.getDimensions().getColumns() != b.getDimensions().getRows())
        {
            throw Exception(_W("Size mismatch on arguments to arithmetic operator ") + L"*");
        }
        if (a.isEmpty())

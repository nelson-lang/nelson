...
namespace Nelson {
    //=============================================================================
    template <class T> ArrayOf ToDouble(ArrayOf A)
    {
        ...
        Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>> matC(pDouble, A.getLength(), 1);
        matC = matA.template cast<double>();
        ArrayOf r = ArrayOf(NLS_DOUBLE, A.getDimensions(), pDouble, A.isSparse());
        return r;
    }
    //=============================================================================
    ArrayOf ToDouble(ArrayOf A)
    {
        switch (A.getDataClass())
        {
            ...
            case NLS_STRUCT_ARRAY:
            {
                ...
                {
                    throw Exception(_("Undefined function 'double' for input arguments of type '") + A.getStructType() + "'.");
                }
                else
                {
            ...
            case NLS_LOGICAL:
            {
                if (A.isSparse())
                {
                    void* pDouble = TypeConvertSparseDynamicFunction(NLS_DOUBLE, A.getDimensions().getRows(), A.getDimensions().getColumns(), A.getSparseDataPointer(), NLS_LOGICAL);

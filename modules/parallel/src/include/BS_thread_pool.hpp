 ...
 * classes BS::multi_future, BS::blocks, BS:synced_stream, and BS::timer.
 */
...
public:
    ...
    /**
     * @brief Get the results from all the futures stored in this multi_future object, rethrowing
     * any stored exceptions.
     *
     * @return If the futures return void, this function returns void as well. Otherwise, it returns
 ...
 * @tparam T2 The type of the index after the last index in the range. Should be a signed or
...
public:
    ...
    {
        return (i == num_blocks - 1) ? index_after_last
...
public:
     ...
     *
     * @param thread_count_ The number of threads to use. The default value is the total number of
     ...
     * separately to the queue. Returns a multi_future object that contains the futures for all of
     * the blocks.
     *
     ...
     * to "for (T i = first_index; i < index_after_last; ++i)". Note that if index_after_last ==
     ...
     * @return A multi_future object that can be used to wait for all the blocks to finish. If the
     * loop function returns a value, the multi_future object can also be used to obtain the values
    ...
    [[nodiscard]] multi_future<R> parallelize_loop(
        const T1 first_index, const T2 index_after_last, F&& loop, const size_t num_blocks = 0)
    {
     ...
     * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block
     * separately to the queue. Returns a multi_future object that contains the futures for all of
     * the blocks. This overload is used for the special case where the first index is 0.
     *
     * @tparam F The type of the function to loop through.
     * @tparam T The type of the loop indices. Should be a signed or unsigned integer.
     * @tparam R The return value of the loop function F (can be void).
     * @param index_after_last The index after the last index in the loop. The loop will iterate
     * from 0 to (index_after_last - 1) inclusive. In other words, it will be equivalent to "for (T
     * i = 0; i < index_after_last; ++i)". Note that if index_after_last == 0, no blocks will be
     * submitted.
     * @param loop The function to loop through. Will be called once per block. Should take exactly
     * two arguments: the first index in the block and the index after the last index in the block.
     * loop(start, end) should typically involve a loop of the form "for (T i = start; i < end;
     ...
     * queue, although any tasks already executed will keep running until they are finished.
     */
    void pause() { paused = true; }

    /**
     * @brief Parallelize a loop by automatically splitting it into blocks and submitting each block
     * separately to the queue. Does not return a multi_future, so the user must use
     * wait_for_tasks() or some other method to ensure that the loop finishes executing, otherwise
     ...
     * separately to the queue. Does not return a multi_future, so the user must use
     * wait_for_tasks() or some other method to ensure that the loop finishes executing, otherwise
     * bad things will happen. This overload is used for the special case where the first index is
     * 0.
     *
     * @tparam F The type of the function to loop through.
     * @tparam T The type of the loop indices. Should be a signed or unsigned integer.
     ...
     * Does not return a future, so the user must use wait_for_tasks() or some other method to
     ...
     * class member function, the first argument must be a pointer to the object, i.e. &object (or
     * this), followed by the actual arguments.
     */
    ...
    /**
     * @brief Submit a function with zero or more arguments into the task queue. If the function has
     * a return value, get a future for the eventual returned value. If the function has no return
    ...
    void wait_for_tasks()
    {
        ...
        task_done_cv.wait(
            tasks_lock, [this] { return (tasks_total == (paused ? tasks.size() : 0)); });
        waiting = false;
    }
...
private:
    ...
    /**
     * @brief An atomic variable indicating whether the workers should pause. When set to true, the
     * workers temporarily stop retrieving new tasks out of the queue, although any tasks already
     * executed will keep running until they are finished. When set to false again, the workers
    ...
    /**
     * @brief An atomic variable indicating that wait_for_tasks() is active and expects to be
...
public:
     ...
     * to this stream simultaneously, as long as they all exclusively use the same synced_stream
     * object to print.
     *
    ...
    /**
     * @brief A stream manipulator to pass to a synced_stream (an explicit cast of std::endl).
     * Prints a newline character to the stream, and then flushes it. Should only be used if
     * flushing is desired, otherwise '\n' should be used instead.
     */
    inline static std::ostream& (&endl)(std::ostream&)
        = static_cast<std::ostream& (&)(std::ostream&)>(std::endl);

    /**
     * @brief A stream manipulator to pass to a synced_stream (an explicit cast of std::flush). Used
     * to flush the stream.
     */

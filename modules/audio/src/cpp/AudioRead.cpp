...
#define	BUFFER_FRAMES 8192
    //=============================================================================
    ArrayOfVector AudioRead(std::wstring filename, double dstart, double dend, std::wstring datatype, std::wstring &errorMessage)
    {
        ...
        switch (format)
        {
            ...
            {
                single *dataAsSingle = (single*)ArrayOf::allocateArrayOf(NLS_SINGLE, dims.getElementCount());
                ...
                if (readcount == sfinfo.frames * sfinfo.channels)
                {
                    ...
                    for (size_t k = 0; k < dims.getElementCount(); k++)
                    {
                        for (indexType i = 0; i < n_out; ++i)
                        {
                            for (indexType j = 0; j < m_out; ++j)
                            {
                                dataAsUInt8[i * m_out + j] = (uint8)(dataAsSingle[j * n_out + i] * 127 + 127);
                            }
                        }
                    }
                    delete[] dataAsSingle;
            ...
            {
                int16 *dataAsInt16RowMajor = (int16*)ArrayOf::allocateArrayOf(NLS_INT16, dims.getElementCount());
                readcount = sf_read_short(file, dataAsInt16RowMajor, dims.getElementCount());
                sf_close(file);
                if (readcount == sfinfo.frames * sfinfo.channels)
                {
                    int16 *dataAsInt16ColumnMajor = (int16*)ArrayOf::allocateArrayOf(NLS_INT16, dims.getElementCount());
                    for (indexType i = 0; i < n_out; ++i)
                    {
                        for (indexType j = 0; j < m_out; ++j)
                        {
            ...
            {
                int32 *dataAsInt32RowMajor = (int32*)ArrayOf::allocateArrayOf(NLS_INT32, dims.getElementCount());
                readcount = sf_readf_int(file, dataAsInt32RowMajor, BUFFER_FRAMES);
                sf_close(file);
                if (readcount == sfinfo.frames * sfinfo.channels)
                {
                    int32 *dataAsInt32ColumnMajor = (int32*)ArrayOf::allocateArrayOf(NLS_INT32, dims.getElementCount());
                    for (indexType i = 0; i < n_out; ++i)
                    {
                        for (indexType j = 0; j < m_out; ++j)
                        {
            ...
            {
                single *dataAsSingleRowMajor = (single*)ArrayOf::allocateArrayOf(NLS_SINGLE, dims.getElementCount());
                readcount = sf_read_float(file, dataAsSingleRowMajor, dims.getElementCount());
                sf_close(file);
                if (readcount == sfinfo.frames * sfinfo.channels)
                {
                    single *dataAsSingleColumnMajor = (single*)ArrayOf::allocateArrayOf(NLS_SINGLE, dims.getElementCount());
                    for (indexType i = 0; i < n_out; ++i)
                    {
                        for (indexType j = 0; j < m_out; ++j)
                        {
            ...
            default: // double
            {
                double *dataAsDoubleRowMajor = (double*)ArrayOf::allocateArrayOf(NLS_DOUBLE, dims.getElementCount());
                readcount = sf_read_double(file, dataAsDoubleRowMajor, dims.getElementCount());
                sf_close(file);
                if (readcount == sfinfo.frames * sfinfo.channels)
                {
                    double *dataAsDoubleColumnMajor = (double*)ArrayOf::allocateArrayOf(NLS_DOUBLE, dims.getElementCount());
                    for (indexType i = 0; i < n_out; ++i)
                    {
                        for (indexType j = 0; j < m_out; ++j)
                        {

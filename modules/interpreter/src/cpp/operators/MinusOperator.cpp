...
Evaluator::minusOperator(AbstractSyntaxTreePtr t)
{
    ...
    ArrayOf retval = this->minusOperator(args);
    callstack.popID();
    return retval;
}
//=============================================================================
ArrayOf
Evaluator::minusOperator(const ArrayOfVector& args)
{
    ...
    if (FindCommonType(args, commonType, isSparse, isComplex, commonTypeName)) {
        ...
        res = callOverloadedFunction(this,
            ...
            MINUS_OPERATOR_STR, commonTypeName, commonType, overloadWasFound);
        if (overloadWasFound) {
            return res;
        }
    }
    ...
    case NLS_DCOMPLEX: {
        if (isSparse) {
            ...
            params << B;
            bool overloadWasFound = false;
            res = callOverloadedFunction(this, NLS_OVERLOAD_ALL_TYPES, params, MINUS_OPERATOR_STR,
                NLS_SPARSE_DOUBLE_STR, commonType, overloadWasFound);
            if (overloadWasFound) {
                return res;
            } else {
                OverloadRequired(MINUS_OPERATOR_STR);
            }
        }

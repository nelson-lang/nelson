...
namespace Nelson {
//=============================================================================
// RAII guard for callstack management - ensures exception safety
...
public:
    explicit CallStackGuard(CallStack& cs, size_t id) : callstack_(cs), active_(true)
    {
        callstack_.pushID(id);
    }
...
//=============================================================================
// RAII guard for loop context - ensures proper enter/exit pairing
...
public:
    explicit LoopContextGuard(Context* ctx) : context_(ctx) { context_->enterLoop(); }

    ~LoopContextGuard() { context_->exitLoop(); }

    LoopContextGuard(const LoopContextGuard&) = delete;
    LoopContextGuard&
    operator=(const LoopContextGuard&)
        = delete;
};
//=============================================================================
//!
...
{
    CallStackGuard stackGuard(callstack, (size_t)t->getContext());
    ...
    // Debug/breakpoint logic before first condition evaluation
    ...
    LoopContextGuard loopGuard(context);
    while (conditionTrue && !breakEncountered) {
        ...
        if (!breakEncountered) {
            // Debug/breakpoint logic on each subsequent condition evaluation
            if (handleConditionBreakpoint()) {
                return;
            }
            condVar = expression(testCondition);
...
{
    ArrayOf switchVal;
    CallStackGuard guard(callstack, t->getContext());

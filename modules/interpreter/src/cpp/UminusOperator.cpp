...
Evaluator::uminusOperator(const ArrayOf& A)
{
    ...
    std::string overloadTypeName = overloadOperatorName(destinationTypeName, UMINUS_OP);
    if (A.isSparse()) {
    ...
    if (!this->isOverloadAllowed()) {
        ...
        if (context->lookupFunction(overloadTypeName, funcDef, !this->isOverloadAllowed())) {
            FunctionsInMemory::getInstance()->add(classA, UMINUS_OP, funcDef);
        }
    } else {
        if (isObject(classA, destinationTypeName)) {
            ...
            if (!FunctionsInMemory::getInstance()->findUnaryOperator(classA, UMINUS_OP, funcDef)) {
                Context* context = this->getContext();

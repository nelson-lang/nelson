...
template <typename T> using ConvFunc = std::function<void(const std::string& pStr, T& pVal)>;
...
public:
     ...
     */
    void
    Load(const std::string& pPath, const LabelParams& pLabelParams = LabelParams(),
        const SeparatorParams& pSeparatorParams = SeparatorParams(),
        const ConverterParams& pConverterParams = ConverterParams(),
        const LineReaderParams& pLineReaderParams = LineReaderParams())
    {
     ...
     */
    void
    Load(std::istream& pStream, const LabelParams& pLabelParams = LabelParams(),
        const SeparatorParams& pSeparatorParams = SeparatorParams(),
        const ConverterParams& pConverterParams = ConverterParams(),
        const LineReaderParams& pLineReaderParams = LineReaderParams())
    {
     ...
     */
    void
    Save(std::ostream& pStream) const
    {
        WriteCsv(pStream);
    }
            ...
            if (std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx) {
                if (dataColumnIdx < itRow->size()) {
                    ...
                    const std::string errStr = "requested column index "
                        ...
                        + std::to_string(
                            std::distance(mData.begin(), itRow) - (mLabelParams.mColumnNameIdx + 1))
                        + ")";
                    throw std::out_of_range(errStr);
                }
            }
        ...
        for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow) {
            ...
            mData
                .at(static_cast<size_t>(
                    std::distance(pColumn.begin(), itRow) + mLabelParams.mColumnNameIdx + 1))
                .at(dataColumnIdx)
                = str;
        }
    }
    ...
    template <typename T>
    void
    InsertColumn(const size_t pColumnIdx, const std::vector<T>& pColumn = std::vector<T>(),
    ...
    SetRow(const size_t pRowIdx, const std::vector<T>& pRow)
    {
        ...
        for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol) {
            ...
            mData.at(dataRowIdx)
                .at(static_cast<size_t>(
                    std::distance(pRow.begin(), itCol) + mLabelParams.mRowNameIdx + 1))
                = str;
        }
    }
    ...
    template <typename T>
    void
    InsertRow(const size_t pRowIdx, const std::vector<T>& pRow = std::vector<T>(),
    ...
    template <typename T>
    void
    SetCell(const size_t pColumnIdx, const std::string& pRowName, const T& pCell)
    {
        const int rowIdx = GetRowIdx(pRowName);
        if (rowIdx < 0) {
            throw std::out_of_range("row not found: " + pRowName);
    ...
    template <typename T>
    void
    SetCell(const std::string& pColumnName, const size_t pRowIdx, const T& pCell)
    {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
            throw std::out_of_range("column not found: " + pColumnName);
     ...
     */
    std::string
    GetColumnName(const size_t pColumnIdx) const
    {
        const size_t dataColumnIdx = GetDataColumnIndex(pColumnIdx);
        if (mLabelParams.mColumnNameIdx < 0) {
            throw std::out_of_range(
                "column name row index < 0: " + std::to_string(mLabelParams.mColumnNameIdx));
        }
     ...
     */
    std::string
    GetRowName(const size_t pRowIdx) const
    {
        const size_t dataRowIdx = GetDataRowIndex(pRowIdx);
        if (mLabelParams.mRowNameIdx < 0) {
            throw std::out_of_range(
                "row name column index < 0: " + std::to_string(mLabelParams.mRowNameIdx));
        }
...
private:
        ...
        while (p_FileLength > 0) {
            ...
            for (size_t i = 0; i < static_cast<size_t>(readLength); ++i) {
                if (buffer[i] == mSeparatorParams.mQuoteChar) {
                    ...
                    if (!quoted) {
                        std::string value = Unquote(Trim(cell));
                        if (!hasIJLastChar) {
    ...
    inline size_t
    GetDataRowIndex(const size_t pRowIdx) const
    {
        const size_t firstDataRow = static_cast<size_t>(
    ...
    inline size_t
    GetDataColumnIndex(const size_t pColumnIdx) const
    {
    ...
    std::string
    Unquote(const std::string& pStr) const
    {
...
#pragma warning(pop)
    ...
    static void
    ReplaceString(std::string& pStr, const std::string& pSearch, const std::string& pReplace)
    {
        size_t pos = 0;
    ...
    static bool
    LastCharIsIorJ(const std::string& content)
    {
        if (content.empty()) {
            return false;
        }
        char lastChar = content.back();
        return (lastChar == 'I' || lastChar == 'J' || lastChar == 'i' || lastChar == 'j');
    }

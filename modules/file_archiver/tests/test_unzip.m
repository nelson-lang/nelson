%=============================================================================
% Copyright (c) 2016-present Allan CORNET (Nelson)
%=============================================================================
% This file is part of Nelson.
%=============================================================================
% LICENCE_BLOCK_BEGIN
% SPDX-License-Identifier: LGPL-3.0-or-later
% LICENCE_BLOCK_END
%=============================================================================
assert_isequal(nargin('unzip'), -4);
assert_isequal(nargout('unzip'), -1);
%=============================================================================
TMPDIR = tempdir();
if ismac()
  TMPDIR = ['/private', TMPDIR];
end
%=============================================================================
temp_dest = [TMPDIR, createGUID()];
mkdir(temp_dest);
cd(temp_dest);
R = unzip([nelsonroot(), '/modules/file_archiver/tests/test_zip.zip']);
REF = { 'test_zip/', ...
'test_zip/dir_1/', ...
'test_zip/dir_1/dir_1_1/', ...
'test_zip/dir_1/dir_1_1/file_1_1_1.txt', ...
'test_zip/dir_1/dir_1_1/file_1_1_2.txt', ...
'test_zip/dir_1/dir_1_2/', ...
'test_zip/dir_1/file_1_1.txt', ...
'test_zip/dir_1/file_1_2.txt', ...
'test_zip/dir_2/', ...
'test_zip/dir_2/file_2_1.txt', ...
'test_zip/file_1.txt', ...
'test_zip/file_2.txt', ...
'file_0.txt', ...
'test_zip/1汉字2.PNG'};
assert_isequal(R, REF);
for r = R
  rr = r{1};
  if endsWith(rr, '/')
    assert_istrue(isdir(rr));
  else
    assert_istrue(isfile(rr));
    fileinfo = dir(rr);
    assert_istrue(fileinfo.bytes > 0);
  end
end
cd(nelsonroot());
rmdir(temp_dest, 's');
%=============================================================================
temp_dest = [TMPDIR, createGUID()];
R = unzip([nelsonroot(), '/modules/file_archiver/tests/test_zip.zip'], temp_dest);
REF = { [temp_dest, '/test_zip/'], ...
[temp_dest, '/test_zip/dir_1/'], ...
[temp_dest, '/test_zip/dir_1/dir_1_1/'], ...
[temp_dest, '/test_zip/dir_1/dir_1_1/file_1_1_1.txt'], ...
[temp_dest, '/test_zip/dir_1/dir_1_1/file_1_1_2.txt'], ...
[temp_dest, '/test_zip/dir_1/dir_1_2/'], ...
[temp_dest, '/test_zip/dir_1/file_1_1.txt'], ...
[temp_dest, '/test_zip/dir_1/file_1_2.txt'], ...
[temp_dest, '/test_zip/dir_2/'], ...
[temp_dest, '/test_zip/dir_2/file_2_1.txt'], ...
[temp_dest, '/test_zip/file_1.txt'], ...
[temp_dest, '/test_zip/file_2.txt'], ...
[temp_dest, '/file_0.txt'], ...
[temp_dest, '/test_zip/1汉字2.PNG']};
assert_isequal(R, REF);
%=============================================================================
for r = R
  rr = r{1};
  if endsWith(rr, '/')
    assert_istrue(isdir(rr));
  else
    assert_istrue(isfile(rr));
    fileinfo = dir(rr);
    assert_istrue(fileinfo.bytes > 0);
  end
end
cd(tempdir());
rmdir(temp_dest, 's');
%=============================================================================
temp_dest = [TMPDIR, createGUID()];
mkdir(temp_dest);
cd(temp_dest);
fd = fopen('a.txt', 'w');
fprintf(fd, 'hello');
fclose(fd);
zip('plain', 'a.txt');
R = unzip('plain');
assert_isequal(R, {'a.txt'});
mkdir('outrel');
R = unzip('plain', 'outrel');
assert_isequal(R, {'outrel/a.txt'});
outabs = [temp_dest, '/outabs'];
R = unzip('plain.zip', outabs);
assert_isequal(R, {[outabs, '/a.txt']});
zip('secret', 'a.txt', 'Password', 'secret', 'EncryptionMethod', 'zipcrypto');
R = unzip('secret', 'outsecret', 'Password', 'secret');
assert_isequal(R, {'outsecret/a.txt'});
[ok, msg] = assert_checkerror('unzip(''secret'', ''outwrong'', ''Password'', ''bad'');', _('Cannot read data.'));
if ~ok
  assert_checkerror('unzip(''secret'', ''outwrong'', ''Password'', ''bad'');', _('Cannot open file.'));
end
cd(tempdir());
rmdir(temp_dest, 's');
%=============================================================================

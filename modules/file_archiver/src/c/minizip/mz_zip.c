...
static int32_t
mz_zip_search_eocd(void* stream, int64_t* central_pos)
{
    int64_t file_size = 0;
    ...
    if (max_back <= 0 || max_back > file_size) // lgtm [cpp/useless-expression cpp/constant-comparison]
        max_back = file_size;
...
static int32_t
mz_zip_search_zip64_eocd(void* stream, const int64_t end_central_offset, int64_t* central_pos)
{
    int64_t offset = 0;
    uint32_t value32 = 0;
    int32_t err = MZ_OK;
...
mz_zip_entry_read_header(
    void* stream, uint8_t local, mz_zip_file* file_info, void* file_extra_stream)
{
        ...
        while ((err == MZ_OK) && (field_pos + 4 <= file_info->extrafield_size)) {
            ...
            /* Read NTFS extra field */
                ...
                while ((err == MZ_OK) && (field_length_read + 4 <= field_length)) {
                    ...
                    if ((err == MZ_OK) && (ntfs_attrib_id == 0x01) && (ntfs_attrib_size == 24)) {
                        ...
                        && (field_length_read + ntfs_attrib_size <= field_length)) {
            ...
            /* Read UNIX1 extra field */
                ...
                if ((err == MZ_OK) && (linkname_size > 0)) {
                    ...
                    if (linkname != NULL) {
                        if (mz_stream_read(file_extra_stream, linkname, linkname_size)
                            != linkname_size)
                            err = MZ_READ_ERROR;
                        if (err == MZ_OK) {
    ...
    mz_stream_mem_get_buffer_at(
        file_extra_stream, extrafield_pos, (const void**)&file_info->extrafield);
    mz_stream_mem_get_buffer_at(file_extra_stream, comment_pos, (const void**)&file_info->comment);
    mz_stream_mem_get_buffer_at(
        file_extra_stream, linkname_pos, (const void**)&file_info->linkname);
    ...
    if (err == MZ_OK) {
        mz_zip_print(
            "Zip - Entry - Read header - %s (local %" PRId8 ")\n", file_info->filename, local);
        mz_zip_print("Zip - Entry - Read header compress (ucs %" PRId64 " cs %" PRId64
                     " crc 0x%08" PRIx32 ")\n",
        ...
        mz_zip_print("Zip - Entry - Read header variable (fnl %" PRId32 " efs %" PRId32
                     " cms %" PRId32 ")\n",
...
static int32_t
mz_zip_entry_read_descriptor(void* stream, uint8_t zip64, uint32_t* crc32, int64_t* compressed_size,
...
static int32_t
mz_zip_entry_write_header(void* stream, uint8_t local, mz_zip_file* file_info)
{
    uint64_t ntfs_time = 0;
    uint32_t reserved = 0;
    uint32_t dos_date = 0;
    ...
    if (file_info->extrafield_size > 0) {
        ...
        mz_stream_mem_set_buffer(
            file_extra_stream, (void*)file_info->extrafield, file_info->extrafield_size);
...
#endif
    /* NTFS timestamps */
    if ((file_info->modified_date != 0) && (file_info->accessed_date != 0)
    ...
    if ((err == MZ_OK) && (!skip_aes) && (file_info->flag & MZ_ZIP_FLAG_ENCRYPTED)
    ...
    if ((err == MZ_OK) && (!local) && (file_info->comment != NULL)) {
        if (mz_stream_write(stream, file_info->comment, file_info->comment_size)
...
mz_zip_entry_write_descriptor(
    void* stream, uint8_t zip64, uint32_t crc32, int64_t compressed_size, int64_t uncompressed_size)
{
    int32_t err = MZ_OK;
...
mz_zip_read_cd(void* handle)
{
    ...
    if (err == MZ_OK) {
        mz_zip_print("Zip - Read cd (disk %" PRId32 " entries %" PRId64 " offset %" PRId64
                     " size %" PRId64 ")\n",
...
mz_zip_write_cd(void* handle)
{
    ...
    if (mz_stream_get_prop_int64(zip->stream, MZ_STREAM_PROP_DISK_SIZE, &disk_size) == MZ_OK
        && disk_size > 0)
    ...
    mz_zip_print("Zip - Write cd (disk %" PRId32 " entries %" PRId64 " offset %" PRId64
                 " size %" PRId64 ")\n",
        zip->disk_number_with_cd, zip->number_entry, zip->cd_offset, zip->cd_size);
    ...
    while (err == MZ_OK) {
        ...
        if (err == MZ_OK) {
            if (local_file_info.compressed_size > 0) {
                ...
                err = mz_stream_find(zip->stream, (const void*)descriptor_magic,
                    sizeof(descriptor_magic), INT64_MAX, &descriptor_pos);
            }
        }
        ...
        if ((err == MZ_OK) && (local_file_info.flag & MZ_ZIP_FLAG_DATA_DESCRIPTOR)) {
            if (mz_zip_extrafield_contains(local_file_info.extrafield,
                    local_file_info.extrafield_size, MZ_ZIP_EXTENSION_ZIP64, NULL)
                == MZ_OK)
                zip64 = 1;

            err = mz_zip_entry_read_descriptor(
                zip->stream, zip64, &crc32, &compressed_size, &uncompressed_size);
...
int32_t
mz_zip_open(void* handle, void* stream, int32_t mode)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
        ...
        if ((err == MZ_OK) && (mode & MZ_OPEN_MODE_APPEND)) {
            ...
            if (zip->disk_number_with_cd > 0) {
                ...
                mz_stream_set_prop_int64(
                    zip->stream, MZ_STREAM_PROP_DISK_NUMBER, zip->disk_number_with_cd - 1);
            }
        } else {
...
int32_t
mz_zip_get_comment(void* handle, const char** comment)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_set_comment(void* handle, const char* comment)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t comment_size = 0;
    if (zip == NULL || comment == NULL)
...
int32_t
mz_zip_get_version_madeby(void* handle, uint16_t* version_madeby)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_set_version_madeby(void* handle, uint16_t version_madeby)
{
    mz_zip* zip = (mz_zip*)handle;
    if (zip == NULL)
        return MZ_PARAM_ERROR;
...
int32_t
mz_zip_set_recover(void* handle, uint8_t recover)
{
    mz_zip* zip = (mz_zip*)handle;
    if (zip == NULL)
        return MZ_PARAM_ERROR;
...
int32_t
mz_zip_get_stream(void* handle, void** stream)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_set_cd_stream(void* handle, int64_t cd_start_pos, void* cd_stream)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_get_cd_mem_stream(void* handle, void** cd_mem_stream)
{
    mz_zip* zip = (mz_zip*)handle;
...
static int32_t
mz_zip_entry_close_int(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
...
static int32_t
mz_zip_entry_open_int(void* handle, uint8_t raw, int16_t compress_level, const char* password)
{
    mz_zip* zip = (mz_zip*)handle;
    ...
    if ((err == MZ_OK) && (use_crypt)) {
        ...
        if (zip->file_info.aes_version) {
            ...
            mz_stream_wzaes_set_encryption_mode(
                zip->crypt_stream, zip->file_info.aes_encryption_mode);
        } else
#endif
        {
#ifdef HAVE_PKCRYPT
            ...
            /* If bit 3 of the general purpose bit flag is set, it uses high byte of 16-bit File
             * Time. */
    ...
    if (err == MZ_OK) {
        if (zip->open_mode & MZ_OPEN_MODE_WRITE) {
            mz_stream_set_prop_int64(
                zip->compress_stream, MZ_STREAM_PROP_COMPRESS_LEVEL, compress_level);
        } else {
#ifndef HAVE_LIBCOMP
            if (zip->entry_raw || zip->file_info.compression_method == MZ_COMPRESS_METHOD_STORE
...
#endif
            {
                ...
                mz_stream_set_prop_int64(
                    zip->crypt_stream, MZ_STREAM_PROP_TOTAL_IN_MAX, max_total_in);

                if (mz_stream_get_prop_int64(
                        zip->crypt_stream, MZ_STREAM_PROP_HEADER_SIZE, &header_size)
                    == MZ_OK)
                    max_total_in -= header_size;
                if (mz_stream_get_prop_int64(
                        zip->crypt_stream, MZ_STREAM_PROP_FOOTER_SIZE, &footer_size)
                    == MZ_OK)
                    max_total_in -= footer_size;

                mz_stream_set_prop_int64(
                    zip->compress_stream, MZ_STREAM_PROP_TOTAL_IN_MAX, max_total_in);
            }
            if ((zip->file_info.compression_method == MZ_COMPRESS_METHOD_LZMA)
...
int32_t
mz_zip_entry_is_open(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
    if (zip == NULL)
        return MZ_PARAM_ERROR;
...
static int32_t
mz_zip_seek_to_local_header(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
    ...
    return mz_stream_seek(
        zip->stream, zip->file_info.disk_offset + zip->disk_offset_shift, MZ_SEEK_SET);
}

int32_t
mz_zip_entry_read_open(void* handle, uint8_t raw, const char* password)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
    ...
    if (err == MZ_OK)
        err = mz_zip_entry_read_header(
            zip->stream, 1, &zip->local_file_info, zip->local_file_info_stream);
    ...
    if (err == MZ_FORMAT_ERROR && zip->disk_offset_shift > 0) {
        ...
        if (err_shift == MZ_OK)
            err_shift = mz_zip_entry_read_header(
                zip->stream, 1, &zip->local_file_info, zip->local_file_info_stream);
        if (err_shift == MZ_OK) {
...
int32_t
mz_zip_entry_write_open(void* handle, const mz_zip_file* file_info, int16_t compress_level,
    uint8_t raw, const char* password)
{
    ...
    if (file_info->filename != NULL)
        mz_stream_write(
            zip->file_info_stream, file_info->filename, (int32_t)strlen(file_info->filename));
    ...
    if (file_info->linkname != NULL)
        mz_stream_write(
            zip->file_info_stream, file_info->linkname, (int32_t)strlen(file_info->linkname));
    mz_stream_write_uint8(zip->file_info_stream, 0);

    mz_stream_mem_get_buffer_at(
        zip->file_info_stream, filename_pos, (const void**)&zip->file_info.filename);
    mz_stream_mem_get_buffer_at(
        zip->file_info_stream, extrafield_pos, (const void**)&zip->file_info.extrafield);
    mz_stream_mem_get_buffer_at(
        zip->file_info_stream, comment_pos, (const void**)&zip->file_info.comment);
    mz_stream_mem_get_buffer_at(
        zip->file_info_stream, linkname_pos, (const void**)&zip->file_info.linkname);
...
int32_t
mz_zip_entry_read(void* handle, void* buf, int32_t len)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t read = 0;
...
int32_t
mz_zip_entry_write(void* handle, const void* buf, int32_t len)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t written = 0;
...
mz_zip_entry_read_close(
    void* handle, uint32_t* crc32, int64_t* compressed_size, int64_t* uncompressed_size)
{
    mz_zip* zip = (mz_zip*)handle;
    int64_t total_in = 0;
    int32_t err = MZ_OK;
    ...
    if ((zip->file_info.flag & MZ_ZIP_FLAG_DATA_DESCRIPTOR)
        ...
        if (mz_zip_extrafield_contains(zip->local_file_info.extrafield,
                zip->local_file_info.extrafield_size, MZ_ZIP_EXTENSION_ZIP64, NULL)
            == MZ_OK)
            zip64 = 1;
    ...
    if ((err == MZ_OK) && (total_in > 0) && (!zip->entry_raw)) {
...
#endif
        {
            if (zip->entry_crc32 != zip->file_info.crc) {
                mz_zip_print("Zip - Entry - Crc failed (actual 0x%08" PRIx32
    ...
    void* handle, uint32_t crc32, int64_t compressed_size, int64_t uncompressed_size)
{
    ...
    if ((err == MZ_OK) && (zip->file_info.flag & MZ_ZIP_FLAG_DATA_DESCRIPTOR)) {
        ...
        if (zip->file_info.flag & MZ_ZIP_FLAG_MASK_LOCAL_INFO)
            ...
            err = mz_zip_entry_write_descriptor(
                zip->stream, zip64, crc32, compressed_size, uncompressed_size);
    }
...
int32_t
mz_zip_entry_is_dir(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_entry_is_symlink(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_entry_get_info(void* handle, mz_zip_file** file_info)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_entry_get_local_info(void* handle, mz_zip_file** local_file_info)
{
    mz_zip* zip = (mz_zip*)handle;
    if (zip == NULL || mz_zip_entry_is_open(handle) != MZ_OK)
        return MZ_PARAM_ERROR;
...
int32_t
mz_zip_entry_set_extrafield(void* handle, const uint8_t* extrafield, uint16_t extrafield_size)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_entry_close(void* handle)
{
    return mz_zip_entry_close_raw(handle, UINT64_MAX, 0);
}

int32_t
mz_zip_entry_close_raw(void* handle, int64_t uncompressed_size, uint32_t crc32)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
...
static int32_t
mz_zip_goto_next_entry_int(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
...
int32_t
mz_zip_set_number_entry(void* handle, uint64_t number_entry)
{
    mz_zip* zip = (mz_zip*)handle;
    if (zip == NULL)
        return MZ_PARAM_ERROR;
...
int32_t
mz_zip_get_number_entry(void* handle, uint64_t* number_entry)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_set_disk_number_with_cd(void* handle, uint32_t disk_number_with_cd)
{
    mz_zip* zip = (mz_zip*)handle;
    if (zip == NULL)
        return MZ_PARAM_ERROR;
...
int32_t
mz_zip_get_disk_number_with_cd(void* handle, uint32_t* disk_number_with_cd)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_goto_entry(void* handle, int64_t cd_pos)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_goto_first_entry(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_goto_next_entry(void* handle)
{
    mz_zip* zip = (mz_zip*)handle;
...
int32_t
mz_zip_locate_entry(void* handle, const char* filename, uint8_t ignore_case)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
    int32_t result = 0;
...
int32_t
mz_zip_locate_first_entry(void* handle, void* userdata, mz_zip_locate_entry_cb cb)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
    int32_t result = 0;
...
int32_t
mz_zip_locate_next_entry(void* handle, void* userdata, mz_zip_locate_entry_cb cb)
{
    mz_zip* zip = (mz_zip*)handle;
    int32_t err = MZ_OK;
    int32_t result = 0;
...
int32_t
mz_zip_attrib_is_dir(uint32_t attrib, int32_t version_madeby)
{
...
int32_t
mz_zip_attrib_is_symlink(uint32_t attrib, int32_t version_madeby)
{
    uint32_t posix_attrib = 0;
    uint8_t system = MZ_HOST_SYSTEM(version_madeby);
    int32_t err = MZ_OK;
...
mz_zip_attrib_convert(
    uint8_t src_sys, uint32_t src_attrib, uint8_t target_sys, uint32_t* target_attrib)
{
...
int32_t
mz_zip_attrib_posix_to_win32(uint32_t posix_attrib, uint32_t* win32_attrib)
{
...
int32_t
mz_zip_attrib_win32_to_posix(uint32_t win32_attrib, uint32_t* posix_attrib)
{
...
int32_t
mz_zip_extrafield_find(void* stream, uint16_t type, uint16_t* length)
{
    int32_t err = MZ_OK;
    uint16_t field_type = 0;
    uint16_t field_length = 0;
...
mz_zip_extrafield_contains(
    const uint8_t* extrafield, int32_t extrafield_size, uint16_t type, uint16_t* length)
{
    void* file_extra_stream = NULL;
    int32_t err = MZ_OK;
...
int32_t
mz_zip_extrafield_read(void* stream, uint16_t* type, uint16_t* length)
{
    int32_t err = MZ_OK;
...
int32_t
mz_zip_extrafield_write(void* stream, uint16_t type, uint16_t length)
{
    int32_t err = MZ_OK;
...
static int32_t
mz_zip_invalid_date(const struct tm* ptm)
{
...
static void
mz_zip_dosdate_to_raw_tm(uint64_t dos_date, struct tm* ptm)
{
...
int32_t
mz_zip_dosdate_to_tm(uint64_t dos_date, struct tm* ptm)
{
    if (ptm == NULL)
        return MZ_PARAM_ERROR;
...
time_t
mz_zip_dosdate_to_time_t(uint64_t dos_date)
{
    struct tm ptm;
    mz_zip_dosdate_to_raw_tm(dos_date, &ptm);
    return mktime(&ptm);
...
int32_t
mz_zip_time_t_to_tm(time_t unix_time, struct tm* ptm)
{
    struct tm ltm;
    if (ptm == NULL)
        return MZ_PARAM_ERROR;
...
uint32_t
mz_zip_time_t_to_dos_date(time_t unix_time)
{
    struct tm ptm;
...
uint32_t
mz_zip_tm_to_dosdate(const struct tm* ptm)
{
    struct tm fixed_tm;
    ...
    return (((uint32_t)fixed_tm.tm_mday + (32 * ((uint32_t)fixed_tm.tm_mon + 1))
                + (512 * (uint32_t)fixed_tm.tm_year))
               << 16)
        | (((uint32_t)fixed_tm.tm_sec / 2) + (32 * (uint32_t)fixed_tm.tm_min)
            + (2048 * (uint32_t)fixed_tm.tm_hour));
}

int32_t
mz_zip_ntfs_to_unix_time(uint64_t ntfs_time, time_t* unix_time)
{
...
int32_t
mz_zip_unix_to_ntfs_time(time_t unix_time, uint64_t* ntfs_time)
{
...
int32_t
mz_zip_path_compare(const char* path1, const char* path2, uint8_t ignore_case)
{
    do {

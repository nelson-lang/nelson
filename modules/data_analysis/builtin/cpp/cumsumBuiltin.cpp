...
Nelson::DataAnalysisGateway::cumsumBuiltin(Evaluator* eval, int nLhs, const ArrayOfVector& argIn)
{
    ...
    if (eval->mustOverloadBasicTypes()) {
        retval = OverloadFunction(eval, nLhs, argIn, "cumsum", bSuccess);
    }
    if (!bSuccess) {
        ...
        ArrayOf res = CumSum(argIn[0], n, withNaN, reverse, needOverload);
        if (needOverload) {
            retval = OverloadFunction(eval, nLhs, argIn, "cumsum", bSuccess);
            if (!bSuccess) {
                OverloadRequired(eval, argIn, Overload::OverloadClass::FUNCTION, "cumsum");
            }
        } else {
            retval << res;
        }

...
if /i "%~1"=="arm64" (
    call arm-environment.bat env-only
    call "%MSBUILD%"\msbuild nelson.sln /p:Configuration=Release /p:Platform=ARM /target:"NelSon-gui" /m:4 /p:PreferredToolArchitecture=x64 /p:QTDIR="!QTDIR!" /p:PATH="!PATH!"
    exit /b !errorlevel!
)

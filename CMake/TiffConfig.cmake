...
if(WITHOUT_TIFF)
  ...
  if(DEFINED ENV{IN_NIX_SHELL})
    if(NOT NIX_EXIT_CODE AND EXISTS "$ENV{NIX_TIFFLIB_PREFIX}/include/tiffio.h")
      set(
        TIFF_INCLUDE_DIR
        "$ENV{NIX_TIFFLIB_PREFIX}/include"
        CACHE
          STRING
          "Path to libtiff include directory"
        FORCE
      )
    endif()
    if(NOT
      ...
      AND
      EXISTS
      "$ENV{NIX_TIFFLIB_PREFIX}/lib/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}"
    )
      set(
        TIFF_LIBRARIES
        "$ENV{NIX_TIFFLIB_PREFIX}/lib/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}"
  ...
  elseif(DEFINED ENV{CONDA_PREFIX})
    ...
    if(EXISTS "${CONDA_INC_DIR}/tiffio.h")
      set(
        ...
        CACHE
          STRING
          "Path to tiff library include directory"
        FORCE
      )
    endif()
    ...
    if(EXISTS "${CONDA_LIB_DIR}/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}")
      set(
        TIFF_LIBRARIES
        "${CONDA_LIB_DIR}/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}"
        CACHE
          STRING
  ...
  elseif(DEFINED ENV{HOMEBREW_PREFIX})
    ...
    if(EXISTS "${HOMEBREW_LIB_DIR}/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}")
      set(
        TIFF_LIBRARIES
        "${HOMEBREW_LIB_DIR}/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}"
        CACHE
          STRING
          "Path to tiff library"
        FORCE
    ...
    if(NOT DEFINED TIFF_INCLUDE_DIR OR NOT DEFINED TIFF_LIBRARIES)
      if(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
        ...
        if(NOT
          ...
          AND
          EXISTS
          "${BREW_TIFFLIB_PREFIX}/include/tiffio.h"
        )
          set(
            TIFF_INCLUDE_DIR
            "${BREW_TIFFLIB_PREFIX}/include"
            CACHE
              STRING
              "Path to tiff include directory"
            FORCE
          )
        endif()
        ...
        if(NOT
          ...
          AND
          EXISTS
          "${BREW_TIFFLIB_PREFIX}/lib/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}"
        )
          set(
            TIFF_LIBRARIES
            "${BREW_TIFFLIB_PREFIX}/lib/libtiff${CMAKE_SHARED_LIBRARY_SUFFIX}"
